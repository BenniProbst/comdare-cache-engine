...
class BinaryToCsv {
public:
    void convert(std::span<MeasurementRecord32 const> records, std::filesystem::path const& csv_path) const {
        ...
        for (auto const& r : records) {
            out << r.timestamp_ns << ',' << r.op_id << ',' << r.op_kind << ',' << r.flags << ',' << r.cycles_or_value
                << '\n';
        }
    }
};
