...
class BinaryToJson {
public:
    void convert(std::span<MeasurementRecord32 const> records, std::filesystem::path const& json_path) const {
        ...
        for (auto const& r : records) {
            if (!first) out << ",\n";
            first = false;
            out << "    {\"timestamp_ns\":" << r.timestamp_ns << ",\"op_id\":" << r.op_id
                << ",\"op_kind\":" << r.op_kind << ",\"flags\":" << r.flags
