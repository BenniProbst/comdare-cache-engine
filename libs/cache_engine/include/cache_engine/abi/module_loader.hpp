...
inline dl_handle_t dl_open(char const* path) { return ::LoadLibraryA(path); }
inline void* dl_sym(dl_handle_t h, char const* name) { return reinterpret_cast<void*>(::GetProcAddress(h, name)); }
...
inline dl_handle_t dl_open(char const* path) { return ::dlopen(path, RTLD_NOW | RTLD_LOCAL); }
inline void*       dl_sym(dl_handle_t h, char const* name) { return ::dlsym(h, name); }
inline void        dl_close(dl_handle_t h) {
    if (h) ::dlclose(h);
}
...
public:
    explicit PermutationModule(std::filesystem::path const& path) {
        ...
        if (!handle_) {
            throw std::runtime_error{"Could not load module: " + path.string() + " (" + dl_error_msg() + ")"};
        }
        auto get_fn = reinterpret_cast<comdare_get_module_v1_fn>(dl_sym(handle_, COMDARE_GET_MODULE_V1_SYMBOL));
        if (!get_fn) {
            dl_close(handle_);
            throw std::runtime_error{"Module does not export " COMDARE_GET_MODULE_V1_SYMBOL};
        }
        module_ = get_fn();
    ...
    PermutationModule(PermutationModule&& other) noexcept : handle_{other.handle_}, module_{other.module_} {
        other.handle_ = nullptr;
        other.module_ = nullptr;
    }

    [[nodiscard]] comdare_permutation_module_v1 const& api() const noexcept { return *module_; }

    [[nodiscard]] void* create_instance(comdare_cache_engine_v1* engine) const {
        return module_->create_instance(engine);
    }
    void destroy_instance(void* inst) const { module_->destroy_instance(inst); }
    ...
    void pull_live_counters(void* inst, comdare_hw_counters_v1* out) const { module_->pull_live_counters(inst, out); }
