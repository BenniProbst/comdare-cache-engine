...
concept ArtTrieTraversal =
    ArtTrieNodePool<Pool> && requires(Pool& p, Pool const& cp, typename Pool::key_type k, typename Pool::value_type v) {
...
struct ArtTrieTraversalOrgan {
    ...
    static void link_parent(Pool& p, std::size_t parent, std::uint8_t parent_byte, std::size_t new_ref) {
        if (parent == Pool::kNil)
            ...
            p.set_child(parent, parent_byte, new_ref);
    }
        ...
        for (;;) {
            if (p.is_leaf(ref)) {
                ...
                if (ek == key) {
                    ...
                    return;
                } // Update (keine neue Schluesselzahl)
    ...
    static bool erase_from(Pool& p, typename Pool::key_type key) {
        ...
        std::size_t  gp          = NIL;
        std::uint8_t gp_byte     = 0; // Grosselternknoten (fuer Parent-Shrink)
        std::size_t  parent      = NIL;
        std::uint8_t parent_byte = 0;
        unsigned     depth       = 0;
        for (;;) {
