...
template <class A2>
class OriginalHotSearchAlgoRebound;

namespace detail {

/// DIE SUBSTANZ des S05-Organs (HOT (Binna PVLDB 2018, HOTRowex)): die wrapper-EIGENEN Staging-Vektoren ueber die
...
class OriginalHotSearchAlgoCore : public SearchAlgoBase<Self>
#if defined(COMDARE_A03A_IS_ORIGINAL_CODEGEN)
    ,
...
public:
    ...
    explicit OriginalHotSearchAlgoCore(allocator_type a)
    ...
    OriginalHotSearchAlgoCore(OriginalHotSearchAlgoCore const& o)
    ...
    OriginalHotSearchAlgoCore& operator=(OriginalHotSearchAlgoCore const& o) {
        if (this != &o) {
            keys_   = o.keys_;
            values_ = o.values_;
#ifdef COMDARE_CE_ENABLE_STATISTICS
    ...
    ~OriginalHotSearchAlgoCore() = default;

    [[nodiscard]] bool operator==(OriginalHotSearchAlgoCore const& other) const noexcept {
...
/// DIE IDENTITAET -- das Registry-Organ S05. Nicht-Template, exakt der historische Typ-Name.
class OriginalHotSearchAlgo final
    : public detail::OriginalHotSearchAlgoCore<::comdare::cache_engine::alloc::ExgenAllocator, OriginalHotSearchAlgo> {
public:
    ...
    COMDARE_DEFINE_ORGAN_LOCATION("::comdare::cache_engine::lookup::OriginalHotSearchAlgo",
                                  "organ_axes/lookup/axis_03a_search_algo_original_hot.hpp");

    /// Der Migrations-Ausweis (composable::AllocatorRebindableSearchAlgo).
    template <class A2>
    using rebind_allocator = OriginalHotSearchAlgoRebound<A2>;

    using detail::OriginalHotSearchAlgoCore<default_allocator_type, OriginalHotSearchAlgo>::OriginalHotSearchAlgoCore;
};

/// DIE GEBUNDENE FORM -- traegt BEWUSST KEIN COMDARE_DEFINE_ORGAN_LOCATION.
template <class A2>
class OriginalHotSearchAlgoRebound final
    : public detail::OriginalHotSearchAlgoCore<A2, OriginalHotSearchAlgoRebound<A2>> {
public:
    /// Der EBENEN-AUSWEIS (s. composable::IsReboundSearchAlgoLeaf).
    using axis03a_rebound_tag = void;

    using detail::OriginalHotSearchAlgoCore<A2, OriginalHotSearchAlgoRebound<A2>>::OriginalHotSearchAlgoCore;
};
...
// ---------------------------------------------------------------------------------------------
static_assert(std::is_same_v<composable::search_algo_for_composition_t<OriginalHotSearchAlgo,
                                                                       ::comdare::cache_engine::alloc::ExgenAllocator>,
                             OriginalHotSearchAlgo>,
...
static_assert(
    composable::AllocatorRebindableSearchAlgo<OriginalHotSearchAlgo, ::comdare::cache_engine::alloc::ExgenAllocator>,
    "01c: OriginalHotSearchAlgo traegt keinen rebind_allocator mehr -- nicht migriert.");
static_assert(!composable::IsReboundSearchAlgoLeaf<OriginalHotSearchAlgo>,
...
static_assert(
    composable::IsReboundSearchAlgoLeaf<OriginalHotSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>>,
    "01c: der Rebound-Leaf traegt seinen Ausweis nicht -- der Identitaets-Pin kann nicht mehr greifen.");
static_assert(composable::search_algo_name_is_allocator_invariant_v<OriginalHotSearchAlgo,
                                                                    ::comdare::cache_engine::alloc::ExgenAllocator>,
              "01c name()-INVARIANZ (Level 0) verletzt.");
static_assert(OriginalHotSearchAlgo::name() ==
                  OriginalHotSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>::name(),
...
static_assert(
    concepts::SearchAlgoVariant<OriginalHotSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>>);
static_assert(concepts::CacheEngineSearchAlgoPermutationStrategy<
              OriginalHotSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>>);
static_assert(
    concepts::DensityClassifiedStrategy<OriginalHotSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>>);
} // namespace comdare::cache_engine::lookup
