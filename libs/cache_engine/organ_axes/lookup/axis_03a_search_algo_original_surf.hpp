...
template <class A2>
class OriginalSurfSearchAlgoRebound;

namespace detail {

/// DIE SUBSTANZ des S08-Organs (SuRF (Zhang/Lim/Andersen SIGMOD 2018)): die wrapper-EIGENEN Staging-Vektoren ueber die
...
class OriginalSurfSearchAlgoCore : public SearchAlgoBase<Self>
#if defined(COMDARE_A03A_IS_ORIGINAL_CODEGEN)
    ,
...
public:
    ...
    explicit OriginalSurfSearchAlgoCore(allocator_type a)
    ...
    OriginalSurfSearchAlgoCore(OriginalSurfSearchAlgoCore const& o)
    ...
    OriginalSurfSearchAlgoCore& operator=(OriginalSurfSearchAlgoCore const& o) {
        if (this != &o) {
            keys_   = o.keys_;
            values_ = o.values_;
#ifdef COMDARE_CE_ENABLE_STATISTICS
    ...
    ~OriginalSurfSearchAlgoCore() = default;
    [[nodiscard]] bool operator==(OriginalSurfSearchAlgoCore const& other) const noexcept {
...
/// DIE IDENTITAET -- das Registry-Organ S08. Nicht-Template, exakt der historische Typ-Name.
class OriginalSurfSearchAlgo final
    : public detail::OriginalSurfSearchAlgoCore<::comdare::cache_engine::alloc::ExgenAllocator,
                                                OriginalSurfSearchAlgo> {
public:
    ...
    COMDARE_DEFINE_ORGAN_LOCATION("::comdare::cache_engine::lookup::OriginalSurfSearchAlgo",
                                  "organ_axes/lookup/axis_03a_search_algo_original_surf.hpp");

    /// Der Migrations-Ausweis (composable::AllocatorRebindableSearchAlgo).
    template <class A2>
    using rebind_allocator = OriginalSurfSearchAlgoRebound<A2>;

    using detail::OriginalSurfSearchAlgoCore<default_allocator_type,
                                             OriginalSurfSearchAlgo>::OriginalSurfSearchAlgoCore;
};

/// DIE GEBUNDENE FORM -- traegt BEWUSST KEIN COMDARE_DEFINE_ORGAN_LOCATION.
template <class A2>
class OriginalSurfSearchAlgoRebound final
    : public detail::OriginalSurfSearchAlgoCore<A2, OriginalSurfSearchAlgoRebound<A2>> {
public:
    /// Der EBENEN-AUSWEIS (s. composable::IsReboundSearchAlgoLeaf).
    using axis03a_rebound_tag = void;

    using detail::OriginalSurfSearchAlgoCore<A2, OriginalSurfSearchAlgoRebound<A2>>::OriginalSurfSearchAlgoCore;
};
...
// ---------------------------------------------------------------------------------------------
static_assert(std::is_same_v<composable::search_algo_for_composition_t<OriginalSurfSearchAlgo,
...
static_assert(
    composable::AllocatorRebindableSearchAlgo<OriginalSurfSearchAlgo, ::comdare::cache_engine::alloc::ExgenAllocator>,
    "01c: OriginalSurfSearchAlgo traegt keinen rebind_allocator mehr -- nicht migriert.");
static_assert(!composable::IsReboundSearchAlgoLeaf<OriginalSurfSearchAlgo>,
...
static_assert(
    composable::IsReboundSearchAlgoLeaf<OriginalSurfSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>>,
    "01c: der Rebound-Leaf traegt seinen Ausweis nicht -- der Identitaets-Pin kann nicht mehr greifen.");
static_assert(composable::search_algo_name_is_allocator_invariant_v<OriginalSurfSearchAlgo,
                                                                    ::comdare::cache_engine::alloc::ExgenAllocator>,
              "01c name()-INVARIANZ (Level 0) verletzt.");
static_assert(OriginalSurfSearchAlgo::name() ==
                  OriginalSurfSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>::name(),
...
static_assert(
    concepts::SearchAlgoVariant<OriginalSurfSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>>);
static_assert(concepts::CacheEngineSearchAlgoPermutationStrategy<
              OriginalSurfSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>>);
static_assert(
    concepts::DensityClassifiedStrategy<OriginalSurfSearchAlgoRebound<::comdare::cache_engine::alloc::ExgenAllocator>>);
} // namespace comdare::cache_engine::lookup
