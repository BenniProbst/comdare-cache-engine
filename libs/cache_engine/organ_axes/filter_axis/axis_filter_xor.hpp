...
public:
    ...
    // F15-Pfad-A Treibe-Op (Spec Par.5 T16, Goldstandard analog axis_05 scan_field_sum / axis_10
    ...
    // Tabelle (1 Byte/Slot) der Laenge `n` interpretiert. Der Aufwand ist REAL und strategie-abhaengig
    // (Xor-Filter nach Graf+Lemire 2020 Par.3: Lookup = fp == B[h0(x)] XOR B[h1(x)] XOR B[h2(x)] -- GENAU 3
    // unabhaengige Tabellenzugriffe + 3-fach-XOR, KEIN frueher Abbruch), KEINE erfundene Zahl.
    // Charakteristisch vs Bloom/Cuckoo: immer exakt 3 (branch-freie) Slot-Reads -> andere, gleichmaessige
    // Latenz-Signatur ohne datenabhaengigen early-exit. Punkt-Query only, immutable nach Build.
    [[nodiscard]] static std::uint64_t filter_probe_scan(unsigned char const* buf, std::size_t n,
        ...
        std::uint64_t     hits  = 0;
        std::size_t const third = n / 3u;         // garantiert >= 1 (n >= 3), Rest faellt in Drittel 3
        ...
        for (std::size_t i = 0; i < q; ++i) {
            ...
            // 3 unabhaengige Hash-Positionen (Graf+Lemire: drei disjunkte Tabellen-Drittel)
