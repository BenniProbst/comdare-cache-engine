...
template <class F>
concept SurfFilterOrgan = requires { typename F::key_type; } &&
                          ...
                          && requires(F& f, F const& cf, std::uint64_t k, std::uint64_t lo, std::uint64_t hi,
                                 ...
                                 {
                                     f.build_from_sorted_keys(sorted)
                                 } -> std::same_as<void>; // einzige Bulk-API (SuRF read-only/bulk-load)
                                 { cf.contains(k) } -> std::same_as<bool>; // may-contain (FP erlaubt, KEINE FN)
                                 {
                                     cf.range_may_exist(lo, hi)
                                 } -> std::same_as<bool>; // [lo,hi] may-overlap (FP erlaubt, KEINE FN)
