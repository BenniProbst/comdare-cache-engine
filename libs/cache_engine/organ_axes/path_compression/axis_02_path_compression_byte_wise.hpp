...
public:
    ...
    COMDARE_DEFINE_ORGAN_LOCATION("::comdare::cache_engine::path_compression::ByteWisePathCompression",
                                  "organ_axes/path_compression/axis_02_path_compression_byte_wise.hpp");
...
struct ByteWiseKeyPrefix {
    ...
    [[nodiscard]] constexpr unsigned     length() const noexcept { return static_cast<unsigned>(packed_ >> 56U); }
    ...
    constexpr void cut(unsigned cut_len) noexcept {
        packed_ = ((packed_ >> (cut_len * 8U)) & kKeyBytesMask) | length_to_word(length() - cut_len);
    }
    ...
    constexpr void prepend(ByteWiseKeyPrefix const& prefix1, std::uint8_t prefix2) noexcept {
        ...
        packed_ = shifted_prefix3 | shifted_prefix2 | masked_prefix1 | length_to_word(length() + prefix1.length() + 1U);
    }
};
