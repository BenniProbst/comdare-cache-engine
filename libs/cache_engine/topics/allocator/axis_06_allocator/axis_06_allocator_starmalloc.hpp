...
// Formal-verifizierter Allocator (F* + KaRaMeL extraction) — Beweis-getragene
// Memory-Safety + Functional Correctness. Build-Time-Eigenschaft, KEINE
// Runtime-Property (laeuft identisch wie andere Bins-Allokatoren).
//
// Dies ist die erste "Korrekt-by-Construction"-Variante in unserer Achse 6 —
// fuer F15-Forschungsmission (Vergleich zwischen verified vs unverified).
...
#include "vendor_includes/starmalloc_include.hpp"
...
class StarMallocAllocator : public AllocatorStrategyBase<StarMallocAllocator> {
public:
    static constexpr bool enabled = flags::starmalloc_enabled;
    ...
    [[nodiscard]] static constexpr std::string_view name() noexcept {
        if constexpr (enabled) { return "starmalloc"; }
    ...
    [[nodiscard]] static constexpr bool has_native_aligned_alloc()    noexcept { return true; }   // starmalloc_alloc(size, alignment)
    ...
    [[nodiscard]] void* allocate(std::size_t bytes, std::size_t alignment) {
        void* p;
        if constexpr (enabled) { p = ::starmalloc_alloc(bytes, alignment); }
    ...
    void deallocate(void* p, std::size_t bytes, std::size_t alignment) noexcept {
        if (p == nullptr) return;
        if constexpr (enabled) { ::starmalloc_free(p); }
    ...
    [[nodiscard]] void* zero_allocate(std::size_t n, std::size_t size) {
        ...
        void* p;
        if constexpr (enabled) { p = ::starmalloc_calloc(n, size); }
    ...
    [[nodiscard]] void* reallocate(void* p, std::size_t old_bytes, std::size_t new_bytes,
        ...
        if constexpr (enabled) {
            np = ::starmalloc_realloc(p, new_bytes);
            (void)alignment;
        } else {
...
namespace comdare::cache_engine::allocator::axis_06_allocator {
    static_assert(concepts::AllocatorStrategy<StarMallocAllocator>);
    static_assert(concepts::CacheEnginePermutationStrategy<StarMallocAllocator>);
    static_assert(concepts::ZeroingStrategy<StarMallocAllocator>);
    static_assert(concepts::ReallocatingStrategy<StarMallocAllocator>);
}
