...
public:
    ...
    [[nodiscard]] static constexpr std::string_view name()         noexcept {
        if constexpr (enabled) { return "original_art"; }
        else                   { return "original_art(disabled)"; }
    }
    [[nodiscard]] static constexpr std::string_view family_name()  noexcept { return "OriginalArtSearchAlgo (ART Leis ICDE 2013, unodb::db Paper-Bindung)"; }
    ...
    [[nodiscard]] bool operator==(OriginalArtSearchAlgo const& other) const noexcept {
    ...
    [[nodiscard]] std::optional<value_type> lookup(key_type k) const {
        ...
        observer_.notify(stats_);
#endif
        if constexpr (enabled) { return data_[k]; }   // s4: unodb::db::get(...)
