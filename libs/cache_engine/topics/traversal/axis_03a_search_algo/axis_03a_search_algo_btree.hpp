...
public:
    ...
    [[nodiscard]] static constexpr std::string_view family_name()  noexcept { return "BTreeSearchAlgo (balanced multi-way B-tree, t=4 — Bayer/McCreight Acta Inf. 1972 / CLRS Kap. 18)"; }
    ...
    [[nodiscard]] static constexpr bool has_cache_line_alignment() noexcept { return true; }   // Node alignas(64), block-orientiert

    BTreeSearchAlgo() noexcept = default;

    [[nodiscard]] bool operator==(BTreeSearchAlgo const& other) const noexcept {
    ...
    [[nodiscard]] std::optional<value_type> lookup(key_type k) const {
        ...
        while (cur != kNil) {
            ...
            if (i < n.n && n.key[i] == k) { result = n.val[i]; break; }
...
private:
    ...
    [[nodiscard]] std::uint32_t new_node(bool leaf) {
        ...
        else { nodes_.push_back(Node{}); idx = static_cast<std::uint32_t>(nodes_.size() - 1); }
        Node& nn = nodes_[idx];
        nn.leaf = leaf;
        nn.child.fill(kNil);
        return idx;
    ...
    [[nodiscard]] bool update_existing(key_type k, value_type v) {
        ...
        while (cur != kNil) {
            ...
            int i = 0;
            while (i < n.n && k > n.key[i]) ++i;
            if (i < n.n && n.key[i] == k) { n.val[i] = v; return true; }
            cur = n.leaf ? kNil : n.child[i];
        }
        return false;
    }
    ...
    void split_child(std::uint32_t x_idx, int i) {
        ...
        Node& y = nodes_[y_idx];
        z.n = kT - 1;
        for (int j = 0; j < kT - 1; ++j) { z.key[j] = y.key[j + kT]; z.val[j] = y.val[j + kT]; }
        if (!y_leaf) for (int j = 0; j < kT; ++j) z.child[j] = y.child[j + kT];
        ...
        x.child[i + 1] = z_idx;
        for (int j = x.n - 1; j >= i; --j) { x.key[j + 1] = x.key[j]; x.val[j + 1] = x.val[j]; }
        x.key[i] = med_k; x.val[i] = med_v;
        x.n += 1;
    }
    ...
    void insert_nonfull(std::uint32_t x_idx, key_type k, value_type v) {
        for (;;) {
            ...
            int i = x.n - 1;
            if (x.leaf) {
                while (i >= 0 && k < x.key[i]) { x.key[i + 1] = x.key[i]; x.val[i + 1] = x.val[i]; --i; }
                x.key[i + 1] = k; x.val[i + 1] = v; x.n += 1;
                return;
            }
    ...
    void remove_from(std::uint32_t x_idx, key_type k) {
        ...
        if (idx < x.n && x.key[idx] == k) {
            if (x.leaf) {
                for (int i = idx + 1; i < x.n; ++i) { x.key[i - 1] = x.key[i]; x.val[i - 1] = x.val[i]; }
                x.n -= 1;
            } else {
            ...
            if (last && idx > xr.n) remove_from(xr.child[idx - 1], k);
            else                    remove_from(xr.child[idx], k);
        }
    }
    ...
    void fill(std::uint32_t x_idx, int idx) {
        Node& x = nodes_[x_idx];
        if (idx != 0 && nodes_[x.child[idx - 1]].n >= kT)      borrow_from_prev(x_idx, idx);
        else if (idx != x.n && nodes_[x.child[idx + 1]].n >= kT) borrow_from_next(x_idx, idx);
        else {
            if (idx != x.n) merge(x_idx, idx);
            else            merge(x_idx, idx - 1);
        }
    }
    ...
    void borrow_from_prev(std::uint32_t x_idx, int idx) {
        ...
        for (int i = child.n - 1; i >= 0; --i) { child.key[i + 1] = child.key[i]; child.val[i + 1] = child.val[i]; }
        if (!child.leaf) for (int i = child.n; i >= 0; --i) child.child[i + 1] = child.child[i];
        child.key[0] = x.key[idx - 1]; child.val[0] = x.val[idx - 1];
        if (!child.leaf) child.child[0] = sib.child[sib.n];
        x.key[idx - 1] = sib.key[sib.n - 1]; x.val[idx - 1] = sib.val[sib.n - 1];
        child.n += 1; sib.n -= 1;
    }
    ...
    void borrow_from_next(std::uint32_t x_idx, int idx) {
        ...
        Node& child = nodes_[x.child[idx]];
        Node& sib   = nodes_[x.child[idx + 1]];
        child.key[child.n] = x.key[idx]; child.val[child.n] = x.val[idx];
        if (!child.leaf) child.child[child.n + 1] = sib.child[0];
        x.key[idx] = sib.key[0]; x.val[idx] = sib.val[0];
        for (int i = 1; i < sib.n; ++i) { sib.key[i - 1] = sib.key[i]; sib.val[i - 1] = sib.val[i]; }
    ...
    void merge(std::uint32_t x_idx, int idx) {
        ...
        child.key[kT - 1] = x.key[idx]; child.val[kT - 1] = x.val[idx];
        for (int i = 0; i < sib.n; ++i) { child.key[i + kT] = sib.key[i]; child.val[i + kT] = sib.val[i]; }
        if (!child.leaf) for (int i = 0; i <= sib.n; ++i) child.child[i + kT] = sib.child[i];
        for (int i = idx + 1; i < x.n; ++i) { x.key[i - 1] = x.key[i]; x.val[i - 1] = x.val[i]; }
