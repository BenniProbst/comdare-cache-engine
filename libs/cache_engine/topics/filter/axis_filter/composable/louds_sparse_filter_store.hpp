...
public:
    ...
    [[nodiscard]] std::uint32_t child_node_num(std::uint32_t pos) const noexcept { return child_indicator_.rank(pos); }  // +child_count_dense_(0)
    [[nodiscard]] std::uint32_t first_label_pos(std::uint32_t node) const noexcept { return louds_bits_.select(node + 1); }
    [[nodiscard]] std::uint32_t suffix_pos(std::uint32_t pos)    const noexcept { return pos - child_indicator_.rank(pos); }
    [[nodiscard]] std::uint32_t node_size(std::uint32_t pos)     const noexcept { return louds_bits_.distance_to_next_set_bit(pos); }
    ...
    [[nodiscard]] bool label_search(std::uint8_t target, std::uint32_t& pos, std::uint32_t search_len) const noexcept {
        if (search_len > 1 && labels_flat_[pos] == kSurfTerminator) { ++pos; --search_len; }
        if (search_len < 3) {
            for (std::uint32_t i = 0; i < search_len; ++i)
                if (labels_flat_[pos + i] == target) { pos += i; return true; }
            return false;
        }
        std::uint32_t l = pos, r = pos + search_len;
        while (l < r) {
            std::uint32_t const m = (l + r) >> 1;
            if (target < labels_flat_[m]) r = m;
            else if (target == labels_flat_[m]) { pos = m; return true; }
            else l = m + 1;
        }
        return false;
    }
    ...
    [[nodiscard]] bool label_search_greater_than(std::uint8_t target, std::uint32_t& pos, std::uint32_t search_len) const noexcept {
        if (search_len > 1 && labels_flat_[pos] == kSurfTerminator) { ++pos; --search_len; }
        ...
        while (l < r) {
            std::uint32_t const m = (l + r) >> 1;
            if (target < labels_flat_[m]) r = m;
            else if (target == labels_flat_[m]) { if (m < base + search_len - 1) { pos = m + 1; return true; } return false; }
            else l = m + 1;
        }
        if (l < base + search_len) { pos = l; return true; }
        return false;
    }
    ...
    [[nodiscard]] std::size_t bit_size() const noexcept {
        return labels_flat_.size() * 8                       // Labels (1 Byte/Item)
             + child_indicator_.bit_size_bits() + child_indicator_.lut_bits()
             + louds_bits_.bit_size_bits()       + louds_bits_.lut_bits()
             + suffix_.bit_size();
    }
    ...
    void clear() noexcept {
        labels_lv_.clear(); child_ind_lv_.clear(); louds_lv_.clear(); suffix_words_lv_.clear();
        node_counts_.clear(); is_last_term_.clear();
        labels_flat_.clear(); child_indicator_ = SurfRank{}; louds_bits_ = SurfSelect{};
        suffix_.clear(); key_count_ = 0;
    }
...
private:
    ...
    [[nodiscard]] std::uint32_t num_items(std::uint32_t level) const noexcept { return static_cast<std::uint32_t>(labels_lv_[level].size()); }
    ...
    void move_to_next_item_slot(std::uint32_t level) {
        if (num_items(level) % 64 == 0) { child_ind_lv_[level].push_back(0); louds_lv_[level].push_back(0); }
    }
    void insert_key_byte(std::uint8_t c, std::uint32_t level, bool is_start_of_node, bool is_term) {
        ...
        if (is_start_of_node) { set_bit(louds_lv_[level], num_items(level) - 1); ++node_counts_[level]; }
    ...
    [[nodiscard]] std::uint32_t insert_until_unique(key_bytes_t const& key, key_bytes_t const& next, unsigned next_len, std::uint32_t start_level) {
        ...
        if (prefix_same) for (unsigned i = 0; i < level; ++i) if (key[i] != next[i]) { prefix_same = false; break; }
    ...
    void build_sparse(std::vector<key_bytes_t> const& keys) {   // surf_builder.hpp Z.185-197
        for (std::size_t i = 0; i < keys.size(); ++i) {
            ...
            if (i < keys.size() - 1) level = insert_until_unique(keys[curpos], keys[i + 1], 8, level);
            else                     level = insert_until_unique(keys[curpos], key_bytes_t{}, 0, level);
