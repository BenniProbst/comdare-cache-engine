#pragma once
// V41.F.6.1.R7.1.d axis_04 Node16Type (ART medium node, SIMD-binary-search)
...
/// Node16Type — ART medium node mit 16 Slots (Leis ICDE 2013).
/// SIMD-binary-search optimal (SSE2 PCMPESTRM / AVX2 VPCMPESTRM).
class Node16Type : public NodeTypeStrategyBase<Node16Type> {
public:
    ...
    [[nodiscard]] static constexpr std::string_view family_name()  noexcept { return "Node16Type (ART medium, SIMD-binary-search 16-slot)"; }
...
namespace comdare::cache_engine::nodes::axis_04_node_type {
    static_assert(concepts::NodeTypeStrategy<Node16Type>);
    static_assert(concepts::CacheEnginePermutationStrategy<Node16Type>);
}
