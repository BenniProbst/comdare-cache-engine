...
struct FlushOriginalCodeMixin : ::comdare::cache_engine::concepts::OriginalCodeMixinBase<PaperManifest> {
    ...
    [[nodiscard]] static constexpr bool is_original_should_flush() noexcept {
        if constexpr (requires { PaperManifest::kIsOriginal_should_flush; })
            return PaperManifest::kIsOriginal_should_flush;
        else
            return false;
    }
    [[nodiscard]] static constexpr bool is_original_on_flush_complete() noexcept {
        if constexpr (requires { PaperManifest::kIsOriginal_on_flush_complete; })
