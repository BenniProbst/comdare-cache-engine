...
//
// Der Zustand lebt IN der Binary (container_algorithm_/ComposedStore/Disk-Files); der Host sieht den Memento NIE —
...
// reicht NICHT" → memento_all kapselt die im abi_adapter real gedeckten stateful Achsen (container_algorithm_,
...
/// Tier-Zustand — den materialisierten Träger container_algorithm_ (CoW Rev.2), die eager gesicherten Organe
/// container_algorithm_tier1_/flt_organ_/vh_organ_/pc_organ_ (R1-kritisch) plus den per-Achsen-MementoAxis-
/// Fallback (NICHT pauschal „alle" Achsen/„inkl. Disk": disk-persistierende Organe sind
/// nur abgedeckt, soweit sie über container_algorithm_ bzw. ihren MementoAxis materialisiert sind — G5-Audit
...
public:
    ...
    /// Kapselt den real gedeckten stateful Zustand (container_algorithm_ via CoW Rev.2, plus per-Achsen-
    ...
    /// container_algorithm_ bzw. MementoAxis materialisiert — kein pauschaler Disk-Flush/Checkpoint, G5-Audit).
