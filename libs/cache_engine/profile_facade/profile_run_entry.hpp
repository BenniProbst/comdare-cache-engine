...
[[nodiscard]] inline RunProfileResult run_profile(RunProfileArgs const& a) {
    ...
    ex::MarkerKontext const marker_kontext = [&bestand_zelle] {
        ...
        std::string const opt =
            bestand_zelle.opt.empty() ? std::string{cem::DefaultOptLevelOption::opt_level_id()} : bestand_zelle.opt;
    ...
    // LANDE-HARMONISIERUNG (W2 4/10, messfenster x s13): der rohe csv-Strom der HEAD-Seite
    // ("csv << r.resumed_csv_rows;") entfaellt nach S13-01 -- die resumed-Zeilen reisen unten
    // ueber mappe.blob_aus_csv in die Mappe (CSV = Kind, D-2(2)); mess_warnungen bleibt.
