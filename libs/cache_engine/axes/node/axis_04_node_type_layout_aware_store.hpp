...
public:
    ...
    static constexpr std::size_t record_phys_bytes() noexcept {
        if constexpr (kRep == RK::aos_interleaved_padded) return round_up_(kKvBytes, kLineBytes);  // 64
        else                                              return kKvBytes;                          // 16
    }
    ...
    [[nodiscard]] value_type  value_at(std::size_t i) const noexcept { return load_value_(chunks_[i / cap_].data, i % cap_); }
    ...
    std::uint64_t organ_observe_layout(LayoutOrgan& org) const {
        ...
        for (auto const& c : chunks_) {
            ...
            if constexpr (requires { org.observe_real_footprint(checksum, std::size_t{}, std::uint64_t{}, std::uint64_t{}); }) {
    ...
    std::uint64_t organ_migrate_step(MigOrgan const& org, LayoutAwareChunkedStore& tier1,
        ...
        for (auto const& s : flat) {
            ...
            bool const migrate = (max_moves == 0 || moved < max_moves)
                              && org.should_migrate_record(rec, sizeof(rec), prev_recency);
            if (migrate) { tier1.append_slot(s.first, s.second); ++moved; }
...
private:
    ...
    [[nodiscard]] static unsigned char* key_ptr_(unsigned char* base, std::size_t j) noexcept {
        if constexpr (kRep == RK::aos_interleaved_packed)      return base + j * kKvBytes;                 // [k|v] @ 16
        else if constexpr (kRep == RK::aos_interleaved_padded) return base + j * record_phys_bytes();      // [k|v|pad] @ 64
        else if constexpr (kRep == RK::soa_split_columns)      return base + j * kKeyBytes;                 // keys[] Spalte
    ...
    [[nodiscard]] static unsigned char* value_ptr_(unsigned char* base, std::size_t j) noexcept {
        if constexpr (kRep == RK::aos_interleaved_packed)      return base + j * kKvBytes + kKeyBytes;
        else if constexpr (kRep == RK::aos_interleaved_padded) return base + j * record_phys_bytes() + kKeyBytes;
        else if constexpr (kRep == RK::soa_split_columns)      return base + cap_ * kKeyBytes + j * kValBytes;   // values[] nach keys[]
        else if constexpr (kRep == RK::aosoa_blocked_columns) {
            std::size_t const b = j / kBlockW, w = j % kBlockW;
    ...
    static key_type load_key_(unsigned char const* base, std::size_t j) noexcept {
        if constexpr (kRep == RK::succinct_hot_cold_split) {
            std::uint16_t hot = 0; std::memcpy(&hot, key_ptr_(base, j), kHotBytes);
            std::uint64_t cold = 0; std::memcpy(&cold, cold_ptr_(base, j), kColdBytes);   // liest 6 B in low48
            return static_cast<key_type>(hot) | (cold << 16);                              // VERLUSTFREIE Rekonstruktion
        } else {
            key_type k; std::memcpy(&k, key_ptr_(base, j), kKeyBytes); return k;
        }
    }
    static value_type load_value_(unsigned char const* base, std::size_t j) noexcept {
        value_type v; std::memcpy(&v, value_ptr_(base, j), kValBytes); return v;
    }
    ...
    void key_scan_footprint_(Chunk const& c, std::uint64_t& checksum,
                             std::uint64_t& key_bytes, std::uint64_t& lines) const noexcept {
        checksum = 0; key_bytes = 0;
        ...
        std::uint64_t cnt = 0;
        for (std::size_t li = 0; li < kMaxLines; ++li) if (touched[li]) ++cnt;
        lines = cnt;
    }
