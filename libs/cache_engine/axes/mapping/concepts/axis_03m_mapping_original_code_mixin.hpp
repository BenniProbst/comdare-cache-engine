...
struct MappingOriginalCodeMixin : ::comdare::cache_engine::concepts::OriginalCodeMixinBase<PaperManifest> {
    ...
    [[nodiscard]] static constexpr bool is_original_register_slot() noexcept {
        if constexpr (requires { PaperManifest::kIsOriginal_register_slot; })
            return PaperManifest::kIsOriginal_register_slot;
        else
            return false;
    }
    [[nodiscard]] static constexpr bool is_original_resolve_offset() noexcept {
        if constexpr (requires { PaperManifest::kIsOriginal_resolve_offset; })
            return PaperManifest::kIsOriginal_resolve_offset;
        else
            return false;
    }
    [[nodiscard]] static constexpr bool is_original_reverse_lookup() noexcept {
        if constexpr (requires { PaperManifest::kIsOriginal_reverse_lookup; })
            return PaperManifest::kIsOriginal_reverse_lookup;
        else
            return false;
    }
    ...
    [[nodiscard]] static constexpr bool is_original_module() noexcept {
        return is_original_register_slot() && is_original_resolve_offset() && is_original_reverse_lookup() &&
               is_original_clear();
    }
};
