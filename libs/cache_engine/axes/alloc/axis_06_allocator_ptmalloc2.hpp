#pragma once
// V41.F.6.1 Batch 8 PtMalloc2Allocator A21 (2026-05-26)
//
// @topic allocator @achse 6 @family A21 (ptmalloc2 explizit — Wolfram Gloger)
// @subaxis AA2 size_class_schema (bins-basiert)
//
// Im Unterschied zu StdMalloc (A22 = was auch immer std::malloc liefert auf
// der Plattform) ruft dieser Wrapper EXPLIZIT __libc_malloc-Symbole auf —
// bypassed LD_PRELOAD-Interception (z.B. wenn libtcmalloc.so via LD_PRELOAD
// aktiv ist, liefert std::malloc trotzdem tcmalloc; __libc_malloc liefert
// garantiert ptmalloc2). Wichtig fuer A/B-Tests "explizit ptmalloc2 vs anderes".
...
#include "vendor_includes/ptmalloc2_include.hpp"
...
class PtMalloc2Allocator : public AllocatorStrategyBase<PtMalloc2Allocator> {
public:
    static constexpr bool enabled = flags::ptmalloc2_enabled;
    ...
    [[nodiscard]] static constexpr std::string_view name() noexcept {
        if constexpr (enabled) { return "ptmalloc2_explicit"; }
        else                   { return "ptmalloc2_explicit(real=std)"; }
    }
    [[nodiscard]] static constexpr std::string_view family_name() noexcept {
        return "ptmalloc2 explizit (Wolfram Gloger, glibc-Default __libc_malloc)";
    }
    [[nodiscard]] static constexpr std::string_view flag_suffix() noexcept { return "PTMALLOC2"; }
    ...
    [[nodiscard]] static constexpr bool supports_thread_local_cache() noexcept { return true; }   // per-arena thread-affinitaet
    ...
    [[nodiscard]] bool operator==(PtMalloc2Allocator const&) const noexcept { return true; }

    [[nodiscard]] void* allocate(std::size_t bytes, std::size_t alignment) {
        void* p;
        if constexpr (enabled) { p = ::__libc_memalign(alignment, bytes); }
    ...
    void deallocate(void* p, std::size_t bytes, std::size_t alignment) noexcept {
        if (p == nullptr) return;
        if constexpr (enabled) { ::__libc_free(p); }
    ...
    [[nodiscard]] void* zero_allocate(std::size_t n, std::size_t size) {
        ...
        void* p;
        if constexpr (enabled) { p = ::__libc_calloc(n, size); }
...
namespace comdare::cache_engine::alloc {
    static_assert(concepts::AllocatorStrategy<PtMalloc2Allocator>);
    static_assert(concepts::CacheEnginePermutationStrategy<PtMalloc2Allocator>);
    static_assert(concepts::ZeroingStrategy<PtMalloc2Allocator>);
    static_assert(concepts::ReallocatingStrategy<PtMalloc2Allocator>);
}
