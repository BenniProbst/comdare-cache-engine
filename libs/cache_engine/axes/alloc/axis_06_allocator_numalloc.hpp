...
public:
    ...
    [[nodiscard]] static constexpr bool has_native_aligned_alloc() noexcept {
        return true;
    } // numalloc_alloc(size, alignment, node)
    [[nodiscard]] static constexpr bool requires_explicit_init() noexcept { return false; }
    [[nodiscard]] static constexpr bool supports_numa_node_hint() noexcept {
        return true;
    } // SONDERFALL: Charakteristik dieser Familie
    [[nodiscard]] static constexpr bool supports_thread_local_cache() noexcept {
        return false;
    } // node-local statt thread-local
    ...
    [[nodiscard]] void* allocate(std::size_t bytes, std::size_t alignment) {
        void* p;
        if constexpr (enabled) {
            p = ::numalloc_alloc(bytes, alignment, numa_node_);
        } else {
    ...
    [[nodiscard]] void* zero_allocate(std::size_t n, std::size_t size) {
        ...
        void*       p;
        if constexpr (enabled) {
            p = ::numalloc_calloc(n, size, numa_node_);
        } else {
            p = std::calloc(n, size);
        }
#ifdef COMDARE_CE_ENABLE_STATISTICS
