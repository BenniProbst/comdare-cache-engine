...
// (z.B. Small-Object → Free-List, Large-Object → mmap-Pfad).
...
#include "vendor_includes/hmalloc_include.hpp"
...
class HMallocAllocator : public AllocatorStrategyBase<HMallocAllocator> {
public:
    static constexpr bool enabled = flags::hmalloc_enabled;
    ...
    [[nodiscard]] static constexpr std::string_view name() noexcept {
        if constexpr (enabled) { return "hmalloc"; }
    ...
    [[nodiscard]] void* allocate(std::size_t bytes, std::size_t alignment) {
        void* p;
        if constexpr (enabled) { p = ::hmalloc_alloc(bytes, alignment); }
    ...
    void deallocate(void* p, std::size_t bytes, std::size_t alignment) noexcept {
        if (p == nullptr) return;
        if constexpr (enabled) { ::hmalloc_free(p); }
    ...
    [[nodiscard]] void* zero_allocate(std::size_t n, std::size_t size) {
        ...
        void* p;
        if constexpr (enabled) { p = ::hmalloc_calloc(n, size); }
    ...
    [[nodiscard]] void* reallocate(void* p, std::size_t old_bytes, std::size_t new_bytes,
        ...
        if constexpr (enabled) {
            np = ::hmalloc_realloc(p, new_bytes);
            (void)alignment;
        } else {
...
namespace comdare::cache_engine::alloc {
    static_assert(concepts::AllocatorStrategy<HMallocAllocator>);
    static_assert(concepts::CacheEnginePermutationStrategy<HMallocAllocator>);
    static_assert(concepts::ZeroingStrategy<HMallocAllocator>);
    static_assert(concepts::ReallocatingStrategy<HMallocAllocator>);
}
