...
// Reduziert False-Sharing via Cache-Line-aligned Allocation per default.
...
#include "vendor_includes/cama_include.hpp"
...
class CAMAAllocator : public AllocatorStrategyBase<CAMAAllocator> {
public:
    static constexpr bool enabled = flags::cama_enabled;
    ...
    [[nodiscard]] static constexpr std::string_view name() noexcept {
        if constexpr (enabled) { return "cama"; }
    ...
    [[nodiscard]] static constexpr bool has_native_aligned_alloc()    noexcept { return true; }   // cama_alloc(size, alignment)
    ...
    [[nodiscard]] void* allocate(std::size_t bytes, std::size_t alignment) {
        void* p;
        if constexpr (enabled) { p = ::cama_alloc(bytes, alignment); }
    ...
    void deallocate(void* p, std::size_t bytes, std::size_t alignment) noexcept {
        if (p == nullptr) return;
        if constexpr (enabled) { ::cama_free(p); }
    ...
    [[nodiscard]] void* zero_allocate(std::size_t n, std::size_t size) {
        ...
        void* p;
        if constexpr (enabled) { p = ::cama_calloc(n, size); }
...
namespace comdare::cache_engine::alloc {
    static_assert(concepts::AllocatorStrategy<CAMAAllocator>);
    static_assert(concepts::CacheEnginePermutationStrategy<CAMAAllocator>);
    static_assert(concepts::ZeroingStrategy<CAMAAllocator>);
    static_assert(concepts::ReallocatingStrategy<CAMAAllocator>);
}
