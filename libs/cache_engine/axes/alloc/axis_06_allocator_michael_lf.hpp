...
public:
    ...
    [[nodiscard]] static constexpr std::string_view name() noexcept {
        if constexpr (enabled) {
            return "michael_lockfree";
        } else {
            return "michael_lockfree(real=std)";
        }
    }
    ...
    [[nodiscard]] static constexpr bool has_native_aligned_alloc() noexcept {
        return true;
    } // michael_lf_alloc(size, alignment)
    ...
    [[nodiscard]] static constexpr bool supports_thread_local_cache() noexcept {
        return false;
    } // single shared lock-free pool, kein per-thread
    ...
    [[nodiscard]] void* allocate(std::size_t bytes, std::size_t alignment) {
        void* p;
        if constexpr (enabled) {
            p = ::michael_lf_alloc(bytes, alignment);
        } else {
