#pragma once
// V41.F.6.1 Batch 3 MichaelLockFreeAllocator A03 (2026-05-26)
//
// @topic allocator
// @achse 6
...
// ohne Mutexe — Hazard-Pointers fuer Reclamation.
...
#include "vendor_includes/michael_lf_include.hpp"
...
class MichaelLockFreeAllocator : public AllocatorStrategyBase<MichaelLockFreeAllocator> {
public:
    static constexpr bool enabled = flags::michael_lf_enabled;
    ...
    [[nodiscard]] static constexpr std::string_view name() noexcept {
        if constexpr (enabled) { return "michael_lockfree"; }
    ...
    [[nodiscard]] static constexpr bool has_native_aligned_alloc()    noexcept { return true; }   // michael_lf_alloc(size, alignment)
    [[nodiscard]] static constexpr bool requires_explicit_init()      noexcept { return false; }  // CAS-Init lazy
    ...
    [[nodiscard]] static constexpr concepts::ProgressGuarantee progress_guarantee() noexcept { return concepts::ProgressGuarantee::LockFree; }  // CAS-only Allokation
    ...
    [[nodiscard]] void* allocate(std::size_t bytes, std::size_t alignment) {
        void* p;
        if constexpr (enabled) { p = ::michael_lf_alloc(bytes, alignment); }
    ...
    void deallocate(void* p, std::size_t bytes, std::size_t alignment) noexcept {
        if (p == nullptr) return;
        if constexpr (enabled) { ::michael_lf_free(p); }
    ...
    [[nodiscard]] void* zero_allocate(std::size_t n, std::size_t size) {
        ...
        void* p;
        if constexpr (enabled) { p = ::michael_lf_calloc(n, size); }
    ...
    [[nodiscard]] void* reallocate(void* p, std::size_t old_bytes, std::size_t new_bytes,
        ...
        if constexpr (enabled) {
            np = ::michael_lf_realloc(p, new_bytes);
            (void)alignment;
        } else {
...
namespace comdare::cache_engine::alloc {
    static_assert(concepts::AllocatorStrategy<MichaelLockFreeAllocator>);
    static_assert(concepts::CacheEnginePermutationStrategy<MichaelLockFreeAllocator>);
    static_assert(concepts::ZeroingStrategy<MichaelLockFreeAllocator>);
    static_assert(concepts::ReallocatingStrategy<MichaelLockFreeAllocator>);
}
