#pragma once
// V41.F.6.1 Batch 6 TCMallocWarehouseAllocator A14 (2026-05-26)
//
// @topic allocator @achse 6 @family A14 (TCMalloc-Warehouse, Google Cloud)
// @subaxis AA3 thread_locality (TCMalloc-Hyperscale-Erweiterung)
//
// TCMalloc-Variante fuer Hyperscale-Workloads: groessere Central-Caches,
// warehouse-style storage, optimiert fuer Cloud-Apps mit vielen Threads.
...
#include "vendor_includes/tcmalloc_wh_include.hpp"
...
class TCMallocWarehouseAllocator : public AllocatorStrategyBase<TCMallocWarehouseAllocator> {
public:
    static constexpr bool enabled = flags::tcmalloc_wh_enabled;
    ...
    [[nodiscard]] static constexpr std::string_view name() noexcept {
        if constexpr (enabled) { return "tcmalloc_warehouse"; }
        else                   { return "tcmalloc_warehouse(real=std)"; }
    }
    [[nodiscard]] static constexpr std::string_view family_name() noexcept {
        return "TCMalloc-Warehouse Hyperscale (Google Cloud, TCMalloc-Erweiterung)";
    }
    [[nodiscard]] static constexpr std::string_view flag_suffix() noexcept { return "TCMALLOC_WH"; }
    ...
    [[nodiscard]] static constexpr bool supports_thread_local_cache() noexcept { return true; }
    ...
    [[nodiscard]] bool operator==(TCMallocWarehouseAllocator const&) const noexcept { return true; }

    [[nodiscard]] void* allocate(std::size_t bytes, std::size_t alignment) {
        void* p;
        if constexpr (enabled) { p = ::tc_wh_alloc(bytes, alignment); }
    ...
    void deallocate(void* p, std::size_t bytes, std::size_t alignment) noexcept {
        if (p == nullptr) return;
        if constexpr (enabled) { ::tc_wh_free(p); }
    ...
    [[nodiscard]] void* zero_allocate(std::size_t n, std::size_t size) {
        ...
        void* p;
        if constexpr (enabled) { p = ::tc_wh_calloc(n, size); }
...
namespace comdare::cache_engine::alloc {
    static_assert(concepts::AllocatorStrategy<TCMallocWarehouseAllocator>);
    static_assert(concepts::CacheEnginePermutationStrategy<TCMallocWarehouseAllocator>);
    static_assert(concepts::ZeroingStrategy<TCMallocWarehouseAllocator>);
    static_assert(concepts::ReallocatingStrategy<TCMallocWarehouseAllocator>);
}
