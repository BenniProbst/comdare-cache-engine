...
public:
    ...
    [[nodiscard]] static constexpr std::string_view family_name()  noexcept { return "BinarySearchTreeSearchAlgo (unbalanced BST, Hibbard-Deletion — Knuth TAOCP 3 §6.2.2)"; }
    ...
    void insert(key_type k, value_type v) {
        if (root_ == kNil) { root_ = new_node(k, v); ++size_; notify_insert(); return; }
        std::uint32_t cur = root_;
        for (;;) {
            Node& n = nodes_[cur];
            if (k == n.key) { n.val = v; notify_insert(); return; }  // Update
            if (k < n.key) {
                if (n.left == kNil) { std::uint32_t const c = new_node(k, v); nodes_[cur].left = c; ++size_; notify_insert(); return; }
                cur = nodes_[cur].left;
            } else {
                if (n.right == kNil) { std::uint32_t const c = new_node(k, v); nodes_[cur].right = c; ++size_; notify_insert(); return; }
    ...
    bool erase(key_type k) {
        ...
        if (nodes_[cur].left != kNil && nodes_[cur].right != kNil) {
            ...
            while (nodes_[succ].left != kNil) { succ_parent = succ; succ = nodes_[succ].left; }
            ...
            if (nodes_[succ_parent].left == succ) nodes_[succ_parent].left = succ_child;
            else                                  nodes_[succ_parent].right = succ_child;
            free_node(succ);
        } else {
            ...
            if (parent == kNil)                       root_ = child;             // cur war Wurzel
            else if (nodes_[parent].left == cur)      nodes_[parent].left = child;
