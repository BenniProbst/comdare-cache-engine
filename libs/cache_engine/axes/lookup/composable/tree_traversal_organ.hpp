...
concept TreeTraversalOrgan = TreeNodePool<Pool> && requires(Pool& p, Pool const& cp,
...
struct BSTTraversalOrgan {
    ...
    static void insert_into(Pool& p, typename Pool::key_type k, typename Pool::value_type v) {
        ...
        while (cur != NIL) {
            ...
            if (k == ck) { p.set_node_value(cur, v); return; }   // Update
            parent = cur;
            if (k < ck) { cur = p.left(cur);  is_left = true; }
            else        { cur = p.right(cur); is_left = false; }
        }
        std::size_t const nn = p.allocate_node(k, v);
        if (parent == NIL)  p.set_root(nn);
        else if (is_left)   p.set_left(parent, nn);
        else                p.set_right(parent, nn);
    }
    ...
    static bool erase_from(Pool& p, typename Pool::key_type k) {
        ...
        while (cur != NIL && p.node_key(cur) != k) {
            parent = cur;
            if (k < p.node_key(cur)) { cur = p.left(cur);  is_left = true; }
            else                     { cur = p.right(cur); is_left = false; }
        }
        ...
        if (p.left(cur) != NIL && p.right(cur) != NIL) {
            ...
            while (p.left(succ) != NIL) { succ_parent = succ; succ = p.left(succ); }
        ...
        if (parent == NIL)  p.set_root(child);
        else if (is_left)   p.set_left(parent, child);
        else                p.set_right(parent, child);
        p.free_node(cur);
        return true;
    }
};
