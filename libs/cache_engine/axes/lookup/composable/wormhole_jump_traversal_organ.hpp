...
concept WormholeTraversal = WormholeLeafListPool<Pool> &&
...
struct WormholeJumpTraversalOrgan {
    ...
    static void reanchor(Pool& p, std::size_t L) {
        ...
        if (old == neu) return;
        p.index_erase(old);
        p.set_leaf_anchor(L, neu);
        p.index_insert(neu, L);
    }
    ...
    static void borrow_or_merge(Pool& p, std::size_t L) {
        ...
        if (Np != NIL && p.leaf_n(Np) > MRG) {
            ...
            for (int j = ln; j > 0; --j) {
                p.set_leaf_key_at(L, j, p.leaf_key_at(L, j - 1));
                p.set_leaf_value_at(L, j, p.leaf_value_at(L, j - 1));
            }
            p.set_leaf_key_at(L, 0, p.leaf_key_at(Np, pn - 1));
            p.set_leaf_value_at(L, 0, p.leaf_value_at(Np, pn - 1));
            p.set_leaf_n(Np, pn - 1);
            p.set_leaf_n(L, ln + 1);
        ...
        if (Nn != NIL && p.leaf_n(Nn) > MRG) {
            ...
            p.set_leaf_key_at(L, ln, p.leaf_key_at(Nn, 0));
            p.set_leaf_value_at(L, ln, p.leaf_value_at(Nn, 0));
            for (int j = 0; j < nn - 1; ++j) {
                p.set_leaf_key_at(Nn, j, p.leaf_key_at(Nn, j + 1));
                p.set_leaf_value_at(Nn, j, p.leaf_value_at(Nn, j + 1));
            }
            p.set_leaf_n(Nn, nn - 1);
            p.set_leaf_n(L, ln + 1);
        ...
        if (Np != NIL && p.leaf_n(Np) + p.leaf_n(L) <= KPN) {
            int const pn = p.leaf_n(Np), ln = p.leaf_n(L);
            for (int j = 0; j < ln; ++j) {
                p.set_leaf_key_at(Np, pn + j, p.leaf_key_at(L, j));
                p.set_leaf_value_at(Np, pn + j, p.leaf_value_at(L, j));
            }
        ...
        if (Nn != NIL && p.leaf_n(L) + p.leaf_n(Nn) <= KPN) {
            ...
            for (int j = 0; j < nn; ++j) {
                p.set_leaf_key_at(L, ln + j, p.leaf_key_at(Nn, j));
                p.set_leaf_value_at(L, ln + j, p.leaf_value_at(Nn, j));
            }
    ...
    static void insert_into(Pool& p, typename Pool::key_type key, typename Pool::value_type value) {
        ...
        if (p.root() == NIL) {
            ...
            p.set_leaf_key_at(L, 0, key);
            p.set_leaf_value_at(L, 0, value);
            p.set_leaf_n(L, 1);
            p.set_leaf_anchor(L, key);
            p.set_leaf_prev(L, NIL);
            p.set_leaf_next(L, NIL);
            p.set_root(L);
            p.index_insert(key, L);
            p.inc_size();
            return;
        }
        ...
        if (i < n && p.leaf_key_at(leaf, i) == key) {
            p.set_leaf_value_at(leaf, i, value);
            return;
        } // Update
        for (int j = n; j > i; --j) {
            p.set_leaf_key_at(leaf, j, p.leaf_key_at(leaf, j - 1));
            p.set_leaf_value_at(leaf, j, p.leaf_value_at(leaf, j - 1));
        }
        p.set_leaf_key_at(leaf, i, key);
        p.set_leaf_value_at(leaf, i, value);
        p.set_leaf_n(leaf, n + 1);
        p.inc_size();
    ...
    static bool erase_from(Pool& p, typename Pool::key_type key) {
        ...
        if (p.leaf_n(leaf) == 0) {                           // leeres Leaf entfernen
            ...
            if (pv != NIL)
                p.set_leaf_next(pv, nx);
            else
                p.set_root(nx); // root nachziehen wenn Kopf
