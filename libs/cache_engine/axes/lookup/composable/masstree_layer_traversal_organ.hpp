...
concept MasstreeLayerTraversal = MasstreeLayerNodePool<Pool> && requires(Pool& p, Pool const& cp,
...
struct MasstreeLayerTraversalOrgan {
    ...
    [[nodiscard]] static int leaf_lower_bound(Pool const& p, std::size_t leaf, std::uint64_t slice, int sz) {
        ...
        while (lo < hi) {
            int const m = (lo + hi) >> 1;
            if (p.leaf_slice_at(leaf, p.leaf_perm_at(leaf, m)) < slice) lo = m + 1; else hi = m;
        }
        return lo;
    }
    ...
    template <class Pool>
    static void bplus_find_or_insert(Pool& p, std::size_t root, std::uint64_t slice,
                                     std::size_t& out_leaf, int& out_phys, bool& was_new, std::size_t& new_root) {
        new_root = root;
        ...
        if (pos < sz && p.leaf_slice_at(lf, p.leaf_perm_at(lf, pos)) == slice) {
            was_new = false; out_leaf = lf; out_phys = p.leaf_perm_at(lf, pos);
            return;
        }
        was_new = true;
...
private:
    struct LEntry { std::uint64_t slice; std::uint8_t keylenx; std::uint64_t value; std::size_t layer; };
    ...
    static void split_leaf_and_insert(Pool& p, std::size_t lf, std::uint64_t slice,
        ...
        int ins = 0; while (ins < W && tmp[ins].slice < slice) ++ins;
        ...
        if (ins < mid) { out_leaf = lf; out_phys = ins; } else { out_leaf = rt; out_phys = ins - mid; }
    ...
    template <class Pool>
    static void propagate_split(Pool& p, std::vector<std::pair<std::size_t, int>> stack,
                                std::uint64_t up_slice, std::size_t left_node, std::size_t right_node,
                                std::size_t& new_root, std::size_t root) {
        constexpr int W = Pool::kWidth;
        for (;;) {
            ...
            auto const [inode, ci] = stack.back(); stack.pop_back();
            ...
            sl[ci] = up_slice; ch[ci + 1] = right_node;
            ...
            for (int j = 0; j < medi; ++j) { p.inode_set_slice_at(inode, j, sl[j]); p.inode_set_child_at(inode, j, ch[j]); }
            ...
            p.inode_set_n(ni, rn);
            for (int j = 0; j < rn; ++j) { p.inode_set_slice_at(ni, j, sl[medi + 1 + j]); p.inode_set_child_at(ni, j, ch[medi + 1 + j]); }
            p.inode_set_child_at(ni, rn, ch[tn]);
            up_slice = med_slice; left_node = inode; right_node = ni;   // weiter nach oben propagieren
        }
    }
...
public:
    ...
    static void insert_into(Pool& p, typename Pool::key_type k, typename Pool::value_type v) {
        ...
        std::size_t holder_leaf = NIL; int holder_phys = 0;

        for (unsigned layer = 0; layer < MaxLayers; ++layer) {
            std::uint64_t const slice = slice_at(k, layer);
            std::size_t leaf; int phys; bool was_new; std::size_t new_root;
            bplus_find_or_insert(p, layer_root, slice, leaf, phys, was_new, new_root);
            if (new_root != layer_root) {
                if (holder_is_pool_root) p.set_root(new_root);
                else                     p.leaf_set_layer_at(holder_leaf, holder_phys, new_root);
                layer_root = new_root;
            }
            if (was_new) {
                ...
                holder_is_pool_root = false; holder_leaf = leaf; holder_phys = phys;
                layer_root = sub;
            } else {
                if (layer + 1U == MaxLayers) { p.leaf_set_value_at(leaf, phys, v); return; }   // Update
    ...
    static bool erase_from(Pool& p, typename Pool::key_type k) {
        ...
        for (unsigned layer = 0; layer < MaxLayers; ++layer) {
            ...
            int const phys = p.leaf_perm_at(leaf, pos);
            if (layer + 1U == MaxLayers) { p.leaf_perm_remove(leaf, pos); p.dec_size(); return true; }   // I4: kein Kollaps
            layer_root = p.leaf_layer_at(leaf, phys);
        }
        return false;
    }
