...
[[nodiscard]] inline std::string to_csv_row(ExecutionResult const& r) {
    ...
    os << detail::csv_quote(r.engine_name) << ','
       << static_cast<int>(r.workload_kind) << ','
       << r.throughput_ops_per_sec << ','
       << r.latency_p50.count() << ','
       << r.latency_p99.count() << ','
       << r.total_cache_misses << ','
       << r.memory_footprint_bytes << ','
       << r.H1_clu_improvement << ','
       << r.H2_layout_score << ','
       << r.H3_inline_external_ratio << ','
       << r.latency_samples_ns.size() << ','
       << (r.success ? 1 : 0);
    return os.str();
}
...
[[nodiscard]] inline std::string to_json_object(ExecutionResult const& r) {
    std::ostringstream os;
    os << '{'
       << "\"engine_name\":\"" << detail::json_escape(r.engine_name) << "\","
       << "\"workload_kind\":" << static_cast<int>(r.workload_kind) << ','
       << "\"throughput_ops_per_sec\":" << r.throughput_ops_per_sec << ','
       << "\"latency_p50_ns\":" << r.latency_p50.count() << ','
       << "\"latency_p99_ns\":" << r.latency_p99.count() << ','
       << "\"total_cache_misses\":" << r.total_cache_misses << ','
       << "\"memory_footprint_bytes\":" << r.memory_footprint_bytes << ','
       << "\"H1_clu_improvement\":" << r.H1_clu_improvement << ','
       << "\"H2_layout_score\":" << r.H2_layout_score << ','
