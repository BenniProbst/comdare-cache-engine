...
inline WelchResult welch_t_test(std::span<const std::int64_t> samples_a,
                                std::span<const std::int64_t> samples_b) {
    WelchResult r {};
    ...
    const double denom = (se_a_sq * se_a_sq) / static_cast<double>(r.n_a - 1)
                       + (se_b_sq * se_b_sq) / static_cast<double>(r.n_b - 1);
