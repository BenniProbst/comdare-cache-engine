...
TEST(ExecuteEngineCommand, RealCallableInjection) {
    ...
    cmd::EngineCallable real_engine =
        [&observed_ops](std::size_t op, cmd::WorkloadKind, std::uint64_t seed) -> cmd::OperationOutcome {
            ++observed_ops;
...
TEST(ExecuteEngineCommand, CallableFailurePropagates) {
    ...
    cmd::EngineCallable failing =
        [](std::size_t op, cmd::WorkloadKind, std::uint64_t) -> cmd::OperationOutcome {
            cmd::OperationOutcome out {};
            out.success = (op != 5);
            return out;
        };
