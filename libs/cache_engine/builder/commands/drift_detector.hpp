...
[[nodiscard]] inline DriftVerdict assess_drift(std::span<const std::int64_t> samples,
    ...
    if (v.median_ns > 0) {                               // Division-durch-0-Schutz
        v.relative_drift = static_cast<double>(v.max_ns - v.min_ns) /
                           static_cast<double>(v.median_ns);
...
[[nodiscard]] DriftGateResult run_with_drift_gate(MeasureOne&& measure_one,
                                                  std::size_t reps = 3,
                                                  double threshold = 0.05,
                                                  std::size_t max_reruns = 3,
                                                  std::ostream* warn = nullptr,
                                                  std::string const& label = "measurement") {
    ...
    for (std::size_t attempt = 1; attempt <= max_attempts; ++attempt) {
        ...
        for (std::size_t r = 0; r < reps; ++r)
            group.push_back(static_cast<std::int64_t>(measure_one()));
        ...
        if (warn != nullptr) {
            ...
            (*warn) << "[chaos:drift] WARN " << label << ": Wiederholungs-Drift "
                    << (v.relative_drift * 100.0) << "% > " << (v.threshold * 100.0) << "% "
                    << "(median=" << v.median_ns << "ns min=" << v.min_ns << " max=" << v.max_ns
                    << " n=" << v.samples << ") — "
                    << (will_retry ? ("Rerun " + std::to_string(attempt) + "/" +
                                      std::to_string(max_reruns))
    ...
    if (warn != nullptr) {
        (*warn) << "[chaos:drift] WARN " << label << ": nach " << max_reruns
                ...
                << "% > " << (threshold * 100.0)
                << "%) — Messung als unzuverlaessig markiert (advisory, kein Abbruch).\n";
    }
    return result;
}
