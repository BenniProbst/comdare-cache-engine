...
run_conformance_gate(anatomy::IDriveableTier& tier, std::uint64_t seed = 42, std::uint64_t n_random = 2000) noexcept {
    ConformanceResult r{};
    auto check = [&](bool ok) noexcept {
        ++r.cases_total;
        if (ok) { ++r.cases_passed; }
        else if (r.first_fail == 0) { r.first_fail = r.cases_total; }
    };
    try {
        ...
        check(tier.tier_insert(7, 70) == true);   oracle[7] = 70;
