...
class LoadProfileXmlBuilder {
  public:
    LoadProfileXmlBuilder& id(std::string v)            { id_ = std::move(v);           return *this; }
    LoadProfileXmlBuilder& paper_ref(std::string v)     { paper_ref_ = std::move(v);    return *this; }
    LoadProfileXmlBuilder& pretty_name(std::string v)   { pretty_name_ = std::move(v);  return *this; }
    LoadProfileXmlBuilder& records(std::uint64_t v)     { records_ = v;                 return *this; }
    LoadProfileXmlBuilder& num_operations(std::uint64_t v) { num_operations_ = v;       return *this; }
    LoadProfileXmlBuilder& config(WorkloadConfig const& c) { config_ = c;               return *this; }

    [[nodiscard]] std::string build() const {
        WorkloadConfig const& c = config_;
        std::ostringstream o;
        o << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n";
        o << "<!-- comdare_load_profile (auto-extrahiert via write_load_profile_xml, #175). Schema = parse_load_profile. -->\n";
        o << "<comdare_load_profile id=\"" << detail::xml_encode(id_)
          << "\" paper_ref=\"" << detail::xml_encode(paper_ref_)
          << "\" schema_version=\"1\">\n";
        o << "  <metadata>\n";
...
[[nodiscard]] inline std::string lpw_focus_of(std::string const& binary_id) {
    ...
    if (v.rfind("search_algo=", 0) == 0) v.remove_prefix(std::string_view{"search_algo="}.size());
    else if (v.rfind("sota_tier=", 0) == 0) v.remove_prefix(std::string_view{"sota_tier="}.size());
    auto slash = v.find('/');
...
[[nodiscard]] inline std::optional<LoadProfile>
extract_load_profile_from_measurements(std::filesystem::path const& csv, std::string const& search_algo) {
    std::ifstream f{csv};
    ...
    while (std::getline(f, line)) {
        ...
        if (has_ws && fld.size() > i_ws) {
            ...
            if (w > 0) { working_set = w; ws_seen = true; }  // Working-Set je Fokus konstant (Pilot: 16384)
        }
    }
