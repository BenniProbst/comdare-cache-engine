...
WorkloadGenerator::WorkloadGenerator(WorkloadConfig config) : config_{config}, state_{config.seed}, generated_{0} {
    if (!config.is_valid()) {
        throw std::invalid_argument{"WorkloadGenerator: WorkloadConfig is invalid "
                                    "(seed=0, num_operations=0, key_max<=key_min, "
                                    "negative or zero-sum op-mix?)"};
    }

    // Op-Mix normalisieren (Summe -> 1.0) — inkl. Scan/RMW (V5-#49-E/F)
    double const sum = config_.pct_insert + config_.pct_lookup + config_.pct_erase + config_.pct_clear +
...
void WorkloadGenerator::precompute_zipfian() noexcept {
    ...
    double zetan = 0.0;
    for (std::uint64_t i = 1; i <= zipf_n_; ++i) zetan += 1.0 / std::pow(static_cast<double>(i), zipf_theta_);
    zipf_zetan_    = zetan;
    zipf_half_pow_ = std::pow(0.5, zipf_theta_);
    zipf_zeta2_    = 1.0 + zipf_half_pow_;
    zipf_alpha_    = 1.0 / (1.0 - zipf_theta_);
    zipf_eta_ =
        (1.0 - std::pow(2.0 / static_cast<double>(zipf_n_), 1.0 - zipf_theta_)) / (1.0 - zipf_zeta2_ / zipf_zetan_);
}
...
std::uint64_t WorkloadGenerator::sample_zipf_rank() noexcept {
    ...
    auto rank = static_cast<std::uint64_t>(static_cast<double>(zipf_n_) *
...
WorkloadOpKind WorkloadGenerator::sample_op_kind() noexcept {
    ...
    auto const r = static_cast<double>(next_random()) / static_cast<double>(static_cast<std::uint64_t>(-1));
...
WorkloadOp WorkloadGenerator::next() noexcept {
    ...
    if (config_.negative_query_pct > 0.0 && (kind == WorkloadOpKind::Lookup || kind == WorkloadOpKind::Scan)) {
    ...
    if (kind == WorkloadOpKind::Insert) { k = (config_.key_max * 2ULL) + 2ULL + generated_; }
...
std::vector<WorkloadOp> WorkloadGenerator::generate_all() {
    ...
    for (std::size_t i = 0; i < config_.num_operations; ++i) { out.push_back(next()); }
    return out;
}

std::size_t WorkloadGenerator::remaining() const noexcept {
    return (generated_ < config_.num_operations) ? (config_.num_operations - generated_) : 0;
}
