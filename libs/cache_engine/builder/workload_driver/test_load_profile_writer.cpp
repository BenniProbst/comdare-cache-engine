...
template <class... A> void emit(char const* fmt, A... a) {
    std::printf(fmt, a...);
    std::fflush(stdout);
    if (g_log) { std::fprintf(g_log, fmt, a...); std::fflush(g_log); }
}
...
bool fields_identical(wd::LoadProfile const& a, wd::LoadProfile const& b, std::string const& tag) {
    ...
    auto cmp_s = [&](char const* f, std::string const& x, std::string const& y) {
        if (x != y) { emit("    DIFF %s: '%s' != '%s'\n", f, x.c_str(), y.c_str()); ok = false; }
    };
    auto cmp_u = [&](char const* f, std::uint64_t x, std::uint64_t y) {
        if (x != y) { emit("    DIFF %s: %llu != %llu\n", f,
            (unsigned long long)x, (unsigned long long)y); ok = false; }
    };
    auto cmp_d = [&](char const* f, double x, double y) {
        if (!near(x, y)) { emit("    DIFF %s: %.9f != %.9f\n", f, x, y); ok = false; }
    };
    cmp_s("id", a.id, b.id);
...
int main(int argc, char** argv) {
    ...
    for (auto const& name : samples) {
        ...
        if (!fs::exists(xml)) { emit("  (skip, fehlt) %s\n", name.c_str()); continue; }
    ...
    std::vector<std::string> const foci = {
        "k_ary",
        "sota::A::ArtComposition",
        "sota::A::PrtArtComposition",
    ...
    for (auto const& focus : foci) {
        ...
        if (!lp) { emit("  [FAIL] extract '%s' -> nullopt\n", focus.c_str()); ++g_fail; continue; }
        emit("  focus=%-42s ops=%llu ws(records)=%llu  mix[i/l/e/c/s/r]=%.3f/%.3f/%.3f/%.3f/%.3f/%.3f\n",
            focus.c_str(),
            (unsigned long long)lp->num_operations, (unsigned long long)lp->records,
            lp->config.pct_insert, lp->config.pct_lookup, lp->config.pct_erase,
            lp->config.pct_clear, lp->config.pct_scan, lp->config.pct_rmw);
        // op-mix spiegelt Daten: Summe ~ 1.0
        double const sum = lp->config.pct_insert + lp->config.pct_lookup + lp->config.pct_erase
                         + lp->config.pct_clear + lp->config.pct_scan + lp->config.pct_rmw;
        check(near(sum, 1.0), (std::string("op-mix sums to 1.0 [") + focus + "]").c_str());
        // Pilot ist lookup-lastig -> pct_lookup dominiert
        check(lp->config.pct_lookup >= lp->config.pct_insert
              && lp->config.pct_lookup >= lp->config.pct_erase,
              (std::string("lookup dominates [") + focus + "]").c_str());

        // sanitize Fokus-Name für Dateinamen (':' / '/' -> '_')
        std::string fn = focus;
        for (char& ch : fn) if (ch == ':' || ch == '/' || ch == ' ') ch = '_';
        fs::path const xml_out = out_dir / (std::string("extracted_") + fn + ".xml");
        check(wd::write_load_profile_xml(*lp, xml_out),
              (std::string("write extracted XML [") + focus + "]").c_str());
        ...
        if (reparsed) check(fields_identical(*lp, *reparsed, std::string("extracted ") + focus),
