...
inline long perf_event_open(struct ::perf_event_attr* attr, ::pid_t pid,
                            int cpu, int group_fd, unsigned long flags) noexcept {
...
public:
    ...
    PerfCounter& operator=(PerfCounter&& o) noexcept {
        if (this != &o) { reset_fd(o.fd_); o.fd_ = -1; }
        return *this;
    }
    ...
    void reset()   const noexcept { if (fd_ >= 0) (void)::ioctl(fd_, PERF_EVENT_IOC_RESET,   0); }
    void enable()  const noexcept { if (fd_ >= 0) (void)::ioctl(fd_, PERF_EVENT_IOC_ENABLE,  0); }
    void disable() const noexcept { if (fd_ >= 0) (void)::ioctl(fd_, PERF_EVENT_IOC_DISABLE, 0); }
    ...
    std::uint64_t read_scaled(bool& ok, bool& scaled) const noexcept {
        ...
        scaled = false;
        if (fd_ < 0) return 0;
        struct { std::uint64_t value; std::uint64_t t_enabled; std::uint64_t t_running; } d{};
        ...
        if (d.t_running < d.t_enabled) {  // Multiplexing → ehrlich hochrechnen.
            ...
            return static_cast<std::uint64_t>(
                static_cast<double>(d.value) * static_cast<double>(d.t_enabled)
...
public:
    LinuxPerfPmcSource() noexcept {
        ...
        l1d_ok_  = c_l1d_.open(PERF_TYPE_HW_CACHE,
                       cache_cfg(PERF_COUNT_HW_CACHE_L1D,  PERF_COUNT_HW_CACHE_OP_READ,
                                 PERF_COUNT_HW_CACHE_RESULT_MISS));
        ll_ok_   = c_ll_.open(PERF_TYPE_HW_CACHE,
                       cache_cfg(PERF_COUNT_HW_CACHE_LL,   PERF_COUNT_HW_CACHE_OP_READ,
                                 PERF_COUNT_HW_CACHE_RESULT_MISS));
        dtlb_ok_ = c_dtlb_.open(PERF_TYPE_HW_CACHE,
                       cache_cfg(PERF_COUNT_HW_CACHE_DTLB, PERF_COUNT_HW_CACHE_OP_READ,
                                 PERF_COUNT_HW_CACHE_RESULT_MISS));
    ...
    void begin() noexcept override {
        ...
        if (l1d_ok_)  { c_l1d_.reset();  c_l1d_.enable();  }
        if (ll_ok_)   { c_ll_.reset();   c_ll_.enable();   }
        if (dtlb_ok_) { c_dtlb_.reset(); c_dtlb_.enable(); }
    ...
    [[nodiscard]] PmcCounters end() noexcept override {
        ...
        if (l1d_ok_) {
            ...
            if (ok) { c.cache_misses_l1 = v; any = true; if (scaled) scaled_ = true; }
        }
        if (ll_ok_) {
            ...
            if (ok) { c.cache_misses_l3 = v; any = true; if (scaled) scaled_ = true; }
        }
        if (dtlb_ok_) {
            std::uint64_t const v = c_dtlb_.read_scaled(ok, scaled);
            if (ok) { c.dtlb_misses = v; any = true; if (scaled) scaled_ = true; }
        }
