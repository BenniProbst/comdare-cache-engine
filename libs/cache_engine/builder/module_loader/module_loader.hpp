...
public:
    ...
    explicit ModuleHandle(void* native_handle,
                          comdare_permutation_module_v1 const* module) noexcept
    ...
    ModuleHandle(ModuleHandle&& other) noexcept
        : native_{other.native_}, module_{other.module_} {
    ...
    [[nodiscard]] std::uint64_t fingerprint() const noexcept {
        return module_ ? module_->permutation_fingerprint : 0;
    }
...
public:
    ...
    load(std::filesystem::path const& dll_path, ModuleHandle& handle_out) noexcept;

    // Loadet alle .dll/.so in einem Verzeichnis (Pattern: comdare_perm_*.dll/.so)
    [[nodiscard]] static int
    load_all(std::filesystem::path const& dir,
