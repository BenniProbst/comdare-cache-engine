...
[[nodiscard]] inline std::string render_adhoc_module_source(int idx, std::string_view macro_args,
    ...
    if (!organ_stamp.empty()) {
        ...
        // Mess-Tooling-HAUPT-Stempel-Zeile (kMeasurementAxisVersionLine, anatomy_version_stamp.hpp::measurement_stamp_
        // line). Beide LEER (Default: ce-only/Identitaets-Fall + [all]/leere Mess-Tooling-Combo) -> EXAKT die bisherige
        // 2-arg-Makro-Zeile -> der emittierte Quelltext bleibt byte-identisch (golden-CRC 0xF1C1F26A1232073B
        // unberuehrt; Katalog/binary_id haengen NICHT am Tooling, der Stempel != binary_id). Nur bei expliziter
        // Tooling-/Merge-Wahl fuellt sich der jeweilige Slot -> DLL-Bytes je Combo verschieden, binary_id/CRC
        // UNBERUEHRT. NAHT-REALITAET (S6-P1b, ehrlich dokumentiert): die gewaehlte Combo ist im gefilterten Planer-
        // Walk (select_measurement_combo, --measurement-combo je ceb:emit ab count>1) bekannt; der CLI-Konsument, der
        // den Slug->Tooling in DIESEN Emitter durchreicht, ist die CLI-Haertung (POST-v3 #34) -- bis dahin ist die
        // Durchreichung ein APPEND-ONLY-Parameter mit Default "" (byte-stabil), den der Direkttest scharf treibt.
        bool const has_meas  = !measurement_stamp.empty();
        bool const has_merge = !merge_stamp.empty();
        if (!has_meas && !has_merge) {
            // Default (kein Mess-Tooling, kein Merge) -> die bisherige 2-arg-Form (byte-identisch).
            src += "COMDARE_ANATOMY_VERSION_STAMP(\"";
            src += organ_stamp;
            src += "\", \"";
            src += system_stamp;
            src += "\")\n";
        } else if (has_meas && !has_merge) {
            // Nur Mess-Tooling gewaehlt -> 3-arg _M-Form (measurement gefuellt, merge leer via _M-Weiterleitung).
            ...
            // Merge gesetzt (mit/ohne Mess-Tooling) -> 4-arg _MERGE-Form. measurement_stamp LEER (Default) => EXAKT die
            // bisherige _MERGE("", "", "", merge)-Zeile (byte-identisch); gefuellt => Mess-Tooling im 3. Slot.
            src += "COMDARE_ANATOMY_VERSION_STAMP_MERGE(\"";
            src += organ_stamp;
            src += "\", \"";
            src += system_stamp;
            src += "\", \"";
