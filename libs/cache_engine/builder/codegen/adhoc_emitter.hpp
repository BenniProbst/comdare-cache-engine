...
[[nodiscard]] inline std::string render_adhoc_module_source(int idx, std::string_view macro_args,
    ...
    if (!organ_stamp.empty()) {
        ...
        // anatomy_version_stamp.hpp::merge_stamp_line). LEER (ce-only/Identitaets-Fall) -> EXAKT die bisherige
        // 2-arg-Makro-Zeile -> der emittierte Quelltext bleibt byte-identisch (golden-CRC unberuehrt). NICHT-LEER
        // (ein Pruefling-Merge) -> die 4-arg _MERGE-Form (measurement HIER leer; das Mess-Tooling ist ein separater
        // Stempel, S4/P-MESSTOOL). Speisung aus K6a (merge_stamp_line).
        if (merge_stamp.empty()) {
