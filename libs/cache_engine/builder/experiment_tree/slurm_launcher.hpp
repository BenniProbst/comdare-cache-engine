...
[[nodiscard]] inline std::string generate_arch_setup(ArchRuntimeConfig const& a) {
    ...
    if (a.smt == 0)
        s += "echo off | sudo tee /sys/devices/system/cpu/smt/control\n";
    else if (a.smt == 1)
        s += "echo on  | sudo tee /sys/devices/system/cpu/smt/control\n";
    if (!a.hw_prefetcher_msr_hex.empty())
        s += "wrmsr -a 0x1a4 " + a.hw_prefetcher_msr_hex + "   # HW-Prefetcher (CAP_SYS_RAWIO)\n";
    return s;
}
...
[[nodiscard]] inline std::string generate_sbatch_script(ZihJobConfig const& z, ArchRuntimeConfig const& a) {
    ...
    s += "# main() → der mitgebrachte perm_runner ist das Executable, das die DLL via AnatomyModuleLoader lädt + "
         "misst).\n";
