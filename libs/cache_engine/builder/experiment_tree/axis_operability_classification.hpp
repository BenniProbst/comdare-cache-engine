...
inline constexpr std::array<AxisOperabilityEntry, 17> kAxisOperability = {{
    ...
    {"memory_layout",      AxisOperability::OperativeCapable, "scan_field_sum runtime-operativ; ABER wall-clock sub-noise -> PMC (R5.D)"},
...
[[nodiscard]] constexpr std::size_t count_operability(AxisOperability k) noexcept {
    std::size_t n = 0;
    for (auto const& e : kAxisOperability) if (e.operability == k) ++n;
    return n;
}
/// Heute real getriebene Achsen (== observable_axis_count-Untergrenze; ehrlich, kein Fake).
[[nodiscard]] constexpr std::size_t operative_axis_count() noexcept { return count_operability(AxisOperability::Operative); }
