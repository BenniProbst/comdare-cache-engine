...
struct NodeObserverSnapshot {
    ...
    // layout-identisch zu anatomy::ComdareTierObserverSnapshot ohne den Typ zu importieren). 18x8 = kV3-Schema
    // (Bau-INC-2c: telemetry / Bau-INC-2d: isa raus -- war [19] bei Konsolidierung; STRUKT-R ORG-18:
    // persistence_target kommt als T17 hinzu -> wieder [18], aber mit anderem Achsen-Satz als vor INC-2d).
    std::uint64_t axis_stats[18][8] = {}; // T0..T17 x 8 Felder (Schema = kV3AxisSchema, STRUKT-R ORG-18)
    std::int64_t  seg_ns[18]        = {}; // Pfad-B Per-Achsen-Timing (ns, ALGORITHMISCHE Organ-Zeit)
