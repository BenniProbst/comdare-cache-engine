#pragma once
// BR-2 (2026-06-02, Doc 27 §3) -- CompositionRegistry: Baum-Blatt-Pfad <-> reale AdHocComposition<18>.
//
...
// zu einer realen AdHocComposition<18> materialisiert (CompositionFromPermTuple<P>) und unter ihrem serialisierten
...
// serialize_composition_from_slots<CompositionFromPermTuple<P>>() (aus den 18 named Slots) == Baum-`binary_id`.
//
...
public:
    /// Befuellt aus einem PILOT-PermutationEngine: jede Permutation -> reale AdHocComposition<18>.
    ...
    void register_from_engine() {
        PilotEngine::for_each_permutation([this]<class P>() {
            using Comp = anatomy::CompositionFromPermTuple<P>; // materialisiert AdHocComposition<18> (compile-time)
            CompositionRecord r;
