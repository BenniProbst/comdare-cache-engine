...
public:
    ...
    [[nodiscard]] void* raw_allocate_with_class(std::size_t bytes, std::size_t alignment,
                                                  MemoryClass mclass) {
    ...
    [[nodiscard]] std::uint64_t local_count()  const noexcept { return local_alloc_count_.load(std::memory_order_relaxed); }
    [[nodiscard]] std::uint64_t shared_count() const noexcept { return shared_alloc_count_.load(std::memory_order_relaxed); }
