...
struct PermutationFlags {
    ...
    [[nodiscard]] constexpr bool any_set() const noexcept {
        return (page_bank | node_bank | traversal_bank | value_handle_bank |
                memory_layout_bank | allocator_bank | prefetch_bank |
                concurrency_bank | isa_bank | telemetry_bank) != 0;
    }
    ...
    [[nodiscard]] constexpr bool is_valid_combination() const noexcept {
        ...
        constexpr uint64_t art_pages =
            flags::page_bank::DENSEBYTE_ART256 | flags::page_bank::SPARSE_NODE4_ART;
        constexpr uint64_t art_nodes =
            flags::node_bank::ART_NODE4 | flags::node_bank::ART_NODE16 |
            flags::node_bank::ART_NODE48 | flags::node_bank::ART_NODE256;
        if ((page_bank & art_pages) && !(node_bank & art_nodes))
            return false;
    ...
    [[nodiscard]] inline std::string to_identifier() const {
        ...
        std::string s;
        s.reserve(176);
        s += to_hex(page_bank);          s += '.';
        s += to_hex(node_bank);          s += '.';
        s += to_hex(traversal_bank);     s += '.';
        s += to_hex(value_handle_bank);  s += '.';
        s += to_hex(memory_layout_bank); s += '.';
        s += to_hex(allocator_bank);     s += '.';
        s += to_hex(prefetch_bank);      s += '.';
        s += to_hex(concurrency_bank);   s += '.';
        s += to_hex(isa_bank);           s += '.';
        s += to_hex(telemetry_bank);
        return s;
    }

    [[nodiscard]] friend constexpr bool operator==(PermutationFlags const&,
                                                   PermutationFlags const&) = default;
};
