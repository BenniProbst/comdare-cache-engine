...
public:
    ...
    void configure(NodeTreeConfig const& config) override {
        config_ = config;
    }
    ...
    [[nodiscard]] Decision evaluate(PageRelocationEvent const& event,
        ...
        if (event.load_factor < 0.20) { ++state_.total_executes; return Decision::EXECUTE; }
        if (event.load_factor > 0.95) { ++state_.total_executes; return Decision::EXECUTE; }
        // Lambda 2: Block AN Cache-Coherence-Cost
        if (ctx.cost_threshold > 0.0 &&
            event.load_factor * event.load_factor > ctx.cost_threshold) {
            ++state_.total_delays;
            return Decision::DELAY;
        }
        // Lambda 3: Anti-Last-Spirale (F2)
        if (config_.max_per_interval > 0 &&
            ctx.recent_relocations_count > config_.max_per_interval) {
            ++state_.total_skips;
