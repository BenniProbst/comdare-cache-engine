...
namespace cache_engine {
    ...
    template <> struct baustein_t<tag::PageAxisTag>        { using type = baustein::PageAxis; };
    template <> struct baustein_t<tag::NodeAxisTag>        { using type = baustein::NodeAxis; };
    template <> struct baustein_t<tag::TraversalAxisTag>   { using type = baustein::TraversalAxis; };
    template <> struct baustein_t<tag::ValueHandleAxisTag> { using type = baustein::ValueHandleAxis; };
    template <> struct baustein_t<tag::ConcurrencyAxisTag> { using type = baustein::ConcurrencyAxis; };
    template <> struct baustein_t<tag::AllocatorAxisTag>   { using type = baustein::AllocatorAxis; };
    template <> struct baustein_t<tag::PrefetchAxisTag>    { using type = baustein::PrefetchAxis; };
    template <> struct baustein_t<tag::TelemetryAxisTag>   { using type = baustein::TelemetryAxis; };
    template <> struct baustein_t<tag::IsaAxisTag>         { using type = baustein::IsaAxis; };
    template <> struct baustein_t<tag::LayoutAxisTag>      { using type = baustein::LayoutAxis; };
    template <> struct baustein_t<tag::ReclamationAxisTag> { using type = baustein::ReclamationAxis; };
}
