...
class ZipfianHelper {
public:
    ZipfianHelper(std::uint64_t n, double theta)
        : n_{n}, theta_{theta} {
...
std::vector<Operation> WorkloadGenerator::generate_ycsb(YcsbWorkload workload) {
    ...
    for (std::uint64_t i = 0; i < config_.num_operations; ++i) {
        ...
        switch (workload) {
            ...
            case YcsbWorkload::E:
            case YcsbWorkload::F:
                op.key_id = zipf.next(gen);
                break;
            case YcsbWorkload::D:
                // "latest" distribution
                op.key_id = (op.op == OperationKind::Insert)
                    ? config_.num_keys + i
                    : zipf.next(gen);
                break;
        }
        if (op.op == OperationKind::Scan) {
            op.scan_length = 1 + (gen() % 100);
        }
        ops.push_back(op);
    }
    return ops;
...
comdare_workload_descriptor_v1 WorkloadGenerator::to_abi_descriptor(
...
OperationKind WorkloadGenerator::sample_op_kind(YcsbWorkload workload, std::mt19937_64& gen) const {
    ...
    switch (workload) {
        case YcsbWorkload::A:
            return (r < 0.5) ? OperationKind::Read : OperationKind::Update;
        case YcsbWorkload::B:
            return (r < 0.95) ? OperationKind::Read : OperationKind::Update;
        case YcsbWorkload::C:
            return OperationKind::Read;
        case YcsbWorkload::D:
            return (r < 0.95) ? OperationKind::Read : OperationKind::Insert;
        case YcsbWorkload::E:
            return (r < 0.95) ? OperationKind::Scan : OperationKind::Insert;
        case YcsbWorkload::F:
            return (r < 0.5) ? OperationKind::Read : OperationKind::ReadModifyWrite;
    }
    return OperationKind::Read;
}
