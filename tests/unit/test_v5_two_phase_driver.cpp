...
struct MockTier final : an::IObservableTier, an::IRollbackableTier {
    ...
    bool tier_lookup(std::uint64_t k, std::uint64_t* out) const noexcept override {
        ...
        if (it != data_.end()) { ++hit_; if (out) *out = it->second; return true; }
        ++miss_; return false;
    }
    ...
    void tier_observe(an::ComdareTierObserverSnapshotV1* o) const noexcept override {
        if (!o) return;
        an::ComdareTierObserverSnapshotV1 t{};
    ...
    void tier_rollback_all() noexcept override {
        if (saved_) {
            data_ = saved_->data; ins_ = saved_->ins; ers_ = saved_->ers; peak_ = saved_->peak;
...
TEST(V5TwoPhaseDriver, TwoPhaseEqualsSinglePhaseFinalState) {
    ...
    MockTier single; ac::drive_tier_observe_trace_abi(single, cfg);
    MockTier twophase; ac::drive_two_phase_tier_trace_abi(twophase, &twophase, cfg);
...
TEST(V5TwoPhaseDriver, ColdFallbackEqualsSinglePhase) {
    auto const cfg = small_cfg();
    MockTier single; ac::drive_tier_observe_trace_abi(single, cfg);
    MockTier cold;   ac::drive_two_phase_tier_trace_abi(cold, nullptr, cfg);
...
TEST(V5TwoPhaseDriver, FillLevelTrajectoryMatches) {
    auto const cfg = small_cfg();
    MockTier single; auto const ts = ac::drive_tier_observe_trace_abi(single, cfg);
    MockTier twophase; auto const tt = ac::drive_two_phase_tier_trace_abi(twophase, &twophase, cfg);
