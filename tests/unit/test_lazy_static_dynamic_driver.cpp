...
int main() {
    ...
    std::vector<std::string> const defs = {"/DCOMDARE_ANATOMY_MODULE_BUILD=1",
                                           "/DCOMDARE_MEASUREMENT_ON=1",
                                           "/DCOMDARE_CE_ENABLE_STATISTICS=1",
                                           "/DCOMDARE_EXPERIMENT_MODE_ON=1",
                                           "/DCOMDARE_OS_WINDOWS=1",
                                           "/DCOMDARE_ARCH_X86_64=1",
                                           "/DCOMDARE_CACHE_LINE_SIZE=64",
                                           "/DWIN32",
                                           "/D_WINDOWS"};
    ...
    std::cout << "SmallSourceCatalog::Engine::count() = " << tlz::SmallSourceCatalog::Engine::count()
              << "  tree.binary_count() = " << tree.binary_count() << "  dyn_dims = " << tree.dynamic_filter().size()
              << "\n";
    ...
    // fälschlich resumed wird (sonst würde eine stale Messung als gültig übernommen). Stamp-Format = resume-v2.
    {
        ...
        ex::LazyRunConfig cfg_env            = cfg;
        cfg_env.env_limits.prefetch_distance = 7;
        check_true("K8: env_limits-Aenderung invalidiert den Stamp",
                   ex::lazy_resume_stamp_prefix(cfg_env, no_dims) != base);
        ex::LazyRunConfig cfg_x1 = cfg;
        {
            wd::WorkloadConfig c{};
            c.pct_insert                    = 0.5;
            c.pct_lookup                    = 0.5;
            cfg_x1.workload_configs["LP01"] = c;
        }
        ex::LazyRunConfig cfg_x2 = cfg;
        {
            wd::WorkloadConfig c{};
            c.pct_insert                    = 0.9;
            c.pct_lookup                    = 0.1;
            cfg_x2.workload_configs["LP01"] = c;
        }
    ...
    // Vorab-Diagnose: Source-Gen liefert für das erste Blatt reale Anatomie? (flush vor dem Build-Loop)
    {
        ...
        std::cout << "Source-Gen[0]: " << s0.size() << " B  enthält ADHOC-Makro: "
                  << (s0.find("COMDARE_DEFINE_ANATOMY_MODULE_ADHOC") != std::string::npos ? "ja" : "NEIN") << std::endl;
    }

    std::cout << "\n=== run_lazy_static_then_dynamic (baut " << N
              << " DLLs real, dann dyn-Loop + Messung) ===" << std::endl;
    ...
    {
        std::error_code ec;
        std::cout << "DIAG source_dir exists=" << fs::exists(cfg.source_dir, ec) << " files="
                  << (fs::exists(cfg.source_dir, ec)
                          ? std::distance(fs::directory_iterator(cfg.source_dir), fs::directory_iterator{})
                          : 0)
                  << "\n";
        std::cout << "DIAG output_dir exists=" << fs::exists(cfg.output_dir, ec) << " files="
                  << (fs::exists(cfg.output_dir, ec)
                          ? std::distance(fs::directory_iterator(cfg.output_dir), fs::directory_iterator{})
                          : 0)
                  << "\n";
    }
    ...
    std::uint64_t sum_lookup = 0, sum_insert = 0;
    std::size_t   rows_real = 0, rows_pos = 0;
    ...
    std::cout << "  Observer-Summen: lookup=" << sum_lookup << " insert=" << sum_insert << "  rows_real=" << rows_real
              << " rows_pos=" << rows_pos << "\n";
    ...
    for (std::size_t i = 0; i < r.csv_rows.size() && i < 6; ++i) {
        ...
        std::cout << "  " << row.setting_id << "  -> lookup=" << row.observer.search_lookup_count
                  << " insert=" << row.observer.search_insert_count << " fill=" << row.observer.tier_fill_level
