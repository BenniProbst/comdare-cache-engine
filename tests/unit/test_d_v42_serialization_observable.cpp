...
int main() {
    ...
    static_assert(std::is_standard_layout_v<s::SerializationSnapshot> &&
                  std::is_trivially_copyable_v<s::SerializationSnapshot>, "Snapshot ABI-tauglich");
    ...
    std::cout << "OK: serialization-Achse ist echte getriebene ObservableAxis (serialize_scan durchgereicht + getrackt).\n";
    return 0;
}
