...
void check_stamp_injected(std::vector<std::string> const& g320_ids) {
    ...
    check_true("(a2) Meta-Meta-Klammer-Anhang [simd=code@1.0.0.c] im emittierten Quelltext",
               ls.find(";[simd=code@1.0.0.c]") != std::string::npos);
...
//    explizite wallclock/macro-Combo => 3-arg _M-Form mit verschiedenen measurement_tooling=..@1.0.0.c-Zeilen. Der
...
void check_measurement_stamp_wiring(std::vector<std::string> const& g320_ids) {
    ...
    // wallclock: 3-arg _M-Form mit measurement_tooling=wallclock@1.0.0.c.
    ...
    check_true("(e) wallclock-Quelle traegt measurement_tooling=wallclock@1.0.0.c",
               src_wc.find("measurement_tooling=wallclock@1.0.0.c") != std::string::npos);

    // macro: verschiedene measurement-Zeile => 2 Combos => 2 verschiedene DLL-Quellen.
    check_true("(e) macro-Quelle traegt measurement_tooling=macro@1.0.0.c",
               src_ma.find("measurement_tooling=macro@1.0.0.c") != std::string::npos);
...
void check_measurement_combo_env_bridge(std::vector<std::string> const& g320_ids) {
    ...
    check_true("(f) full_set traegt wallclock+macro+micro + [load_framework] am Ende (4 Eintraege, 3 Trenner)",
               full.rfind("measurement_tooling=wallclock@1.0.0.c", 0) == 0 &&
                   full.find("measurement_tooling=macro@1.0.0.c") != std::string::npos &&
                   full.find("measurement_tooling=micro@1.0.0.c") != std::string::npos &&
                   full.ends_with(";[load_framework=ycsb@1.0.0.c]") && std::count(full.begin(), full.end(), ';') == 3);
    ...
    check_true("(f) Env ungesetzt -> traegt die volle 3-Tool-Vollmenge",
               src_unset.find("measurement_tooling=wallclock@1.0.0.c") != std::string::npos &&
                   src_unset.find("measurement_tooling=macro@1.0.0.c") != std::string::npos &&
                   src_unset.find("measurement_tooling=micro@1.0.0.c") != std::string::npos);
