...
TEST(MasstreeKpermuter, Kperm15Invariants) {
    ...
    // make_empty: size 0; Allokationsreihenfolge 0,1,2,... via insert_from_back(0) (Prepend -> [k-1..0]).
    {
        ...
        ASSERT_EQ(p.size(), 0);
        for (int k = 0; k < 5; ++k) {
            int const phys = p.insert_from_back(0);
            ASSERT_EQ(phys, k) << "alloc order k=" << k;
        }
        ASSERT_EQ(p.size(), 5);
...
TEST(Axis03aTierOrgan, Uint64MasstreeMatchesStdMap) {
    ...
    ts::verify_matches_std_map<MasstreeOrgan8>(
        60000u, 60000u); // Single-Layer-Degenerationsanker (SliceBytes=8 == reiner B+Baum)
...
TEST(Axis03aTierOrgan, MasstreeLayerBoundaryMatchesStdMap) {
    ...
    auto                                   mk = [](std::uint64_t i) {
        return (std::uint64_t{0x0042} << 48) | ((i * 2654435761u) % 0x0000FFFFFFFFFFFFull);
    };
    for (std::uint64_t i = 0; i < 1500; ++i) {
        std::uint64_t const k = mk(i);
        organ.insert(k, k * 11 + 1);
        ref[k] = k * 11 + 1;
    }
    ASSERT_EQ(organ.occupied_count(), ref.size());
    for (auto const& kv : ref) {
        auto o = organ.lookup(kv.first);
        ASSERT_TRUE(o.has_value()) << kv.first;
        ASSERT_EQ(*o, kv.second);
    }
    EXPECT_FALSE(organ.lookup((std::uint64_t{0x0043} << 48)).has_value()); // anderer slice[0] -> nicht da
    for (std::uint64_t i = 0; i < 1500; i += 2) {
        std::uint64_t const k = mk(i);
        organ.erase(k);
        ref.erase(k);
    }
...
TEST(Axis03aTierOrgan, MasstreeChurnStressMatchesStdMap) {
    ...
    auto                                   check = [&](char const* ph) {
        ...
        for (auto const& kv : ref) {
            auto o = organ.lookup(kv.first);
            ASSERT_TRUE(o.has_value()) << ph << " k=" << kv.first;
            ASSERT_EQ(*o, kv.second);
        }
    };
    for (std::uint64_t i = 0; i < 2000; ++i) {
        std::uint64_t const k = (i * 2654435761u) % 40000u;
        organ.insert(k, k * 7 + 1);
        ref[k] = k * 7 + 1;
    }
    check("fill");
    ...
    for (std::uint64_t i = 0; i < 2000; i += 3) {
        std::uint64_t const k = (i * 2654435761u) % 40000u;
        organ.insert(k, k * 13 + 2);
        ref[k] = k * 13 + 2;
    }
    check("reinsert");
    for (std::uint64_t i = 0; i < 800; ++i) {
        std::uint64_t const k = (std::uint64_t{0x00AB} << 48) | ((i * 40503u) % 0xFFFFFFFFu);
        organ.insert(k, k | 1);
        ref[k] = k | 1;
    } // shared slice[0] -> Multi-Layer
    check("layer-block");
...
void btree_cross_check(ce_cmp::BTreeSearchOrgan const& organ, std::map<std::uint64_t, std::uint64_t> const& ref,
    ...
    for (std::uint64_t q = 0; q <= query_max; ++q) {
        ...
        if (it == ref.end()) {
            ASSERT_FALSE(o.has_value()) << "phantom key " << q;
        } else {
            ASSERT_TRUE(o.has_value()) << "lost key " << q;
            ASSERT_EQ(*o, it->second) << "value mismatch key " << q;
        }
    }
}
} // namespace
...
TEST(Axis03aTierOrgan, HotPatriciaMultiLevelStressMatchesStdMap) {
    ...
    for (std::uint64_t k = 0; k < 1200; ++k) {
        organ.insert(k, k * 3 + 1);
        ref[k] = k * 3 + 1;
    }
    ...
    for (std::uint64_t hi = 1; hi <= 8; ++hi) {
        std::uint64_t const k = (hi << 16) | 0x0102u;
        organ.insert(k, k);
        ref[k] = k;
    }
    ASSERT_EQ(organ.occupied_count(), ref.size());
    ...
    for (std::uint64_t k = 0; k < 1200; k += 3) {
        organ.insert(k, k * 7);
        ref[k] = k * 7;
    }
    art_cross_check(organ, ref, 1300);
    // (4) Erase jeden 2. -> Collapse + Doppel-free + Free-List-Recycling.
    for (std::uint64_t k = 0; k < 1200; k += 2) {
        organ.erase(k);
        ref.erase(k);
    }
    art_cross_check(organ, ref, 1300);
    ...
    ASSERT_EQ(organ.occupied_count(), 0u);
    for (std::uint64_t k = 100; k < 400; ++k) {
        organ.insert(k, k + 9);
        ref[k] = k + 9;
    }
    ...
    ref[0] = 1;
    organ.insert(1, 2);
    ref[1] = 2; // crit_bit=63 (tiefste Stelle)
    organ.insert(0x8000000000000000ULL, 3);
    ref[0x8000000000000000ULL] = 3; // crit_bit=0 (Wurzel-Split)
    for (int s = 0; s < 16; ++s) {
        std::uint64_t const k = (1ULL << s) - 1;
        organ.insert(k, k + 1);
        ref[k] = k + 1;
    } // 0,0x1,0x3,0x7,... Praefix-Kette
    for (auto const& kv : ref) {
        auto o = organ.lookup(kv.first);
        ASSERT_TRUE(o.has_value()) << kv.first;
        ASSERT_EQ(*o, kv.second);
...
TEST(Axis03aTierOrgan, ArtTrieN48EraseReinsertMatchesStdMap) {
    ...
    for (std::uint64_t k = 130; k <= 255; ++k) {
        organ.insert(k, k);
        ref[k] = k;
    }
    art_cross_check(organ, ref, 300);
    for (std::uint64_t k = 130; k <= 255; k += 2) {
        organ.erase(k);
        ref.erase(k);
    }
    art_cross_check(organ, ref, 300);
...
TEST(Axis03aTierOrgan, WormholeMultiLevelStressMatchesStdMap) {
    ...
    for (std::uint64_t hi = 1; hi <= 16; ++hi) {
        std::uint64_t const k = (hi << 48) | 0x0102u;
        organ.insert(k, k);
        ref[k] = k;
    }
    ASSERT_EQ(organ.occupied_count(), ref.size());
...
TEST(Axis03aTierOrgan, StartTrieMultiByteStressMatchesStdMap) {
    ...
    for (std::uint64_t hi = 0; hi < 64; ++hi) {
        std::uint64_t const k = (hi << 8) | 0x07u;
        organ.insert(k, k + 5);
        ref[k] = k + 5;
    }
    for (std::uint64_t hi = 1; hi <= 12; ++hi) {
        std::uint64_t const k = (hi << 16) | 0x0203u;
        organ.insert(k, k);
        ref[k] = k;
    }
    art_cross_check(organ, ref, 1300);
    ...
    ASSERT_EQ(organ.occupied_count(), 0u);
    for (std::uint64_t k = 100; k < 500; ++k) {
        organ.insert(k, k + 9);
        ref[k] = k + 9;
    }
...
TEST(Axis03aTierOrgan, StartSpan3AndHighByteMatchesStdMap) {
    ...
    for (std::uint64_t v = 1; v <= 300; ++v) {
        std::uint64_t const k = (v << 48) | 0x0102u;
        organ.insert(k, k);
        ref[k] = k;
    } // span-Kette ueber Byte 6/7
    organ.insert(0xAA00000000000001ull, 1);
    ref[0xAA00000000000001ull] = 1; // Byte-7-Divergenz
    organ.insert(0xBB00000000000001ull, 2);
    ref[0xBB00000000000001ull] = 2;
    ASSERT_EQ(organ.occupied_count(), ref.size());
    for (auto const& kv : ref) {
        auto o = organ.lookup(kv.first);
        ASSERT_TRUE(o.has_value()) << kv.first;
    ...
    for (std::uint64_t v = 1; v <= 300; v += 2) {
        std::uint64_t const k = (v << 48) | 0x0102u;
        organ.erase(k);
        ref.erase(k);
    } // Erase ueber span-Disk
    ASSERT_EQ(organ.occupied_count(), ref.size());
    for (auto const& kv : ref) {
        auto o = organ.lookup(kv.first);
        ASSERT_TRUE(o.has_value());
