...
struct ScannableMockTier final : an::IObservableTier, an::IRollbackableTier, an::IScannableTier {
    ...
    void                        tier_observe(
        an::ComdareTierObserverSnapshot* o) const noexcept override { // konsolidierter Observer: search → axis_stats[0]
        if (!o) return;
...
struct OpCounts {
    std::size_t insert = 0, lookup = 0, erase = 0, clear = 0, scan = 0, rmw = 0;
};
...
TEST(V5YcsbOpSet, YcsbE_MostlyScan_PlusInsert) {
    ...
    for (auto const& op : ops)
        if (op.kind == wd::WorkloadOpKind::Scan) {
