...
int  g_fail = 0;
void tr(std::string const& w, bool c) {
    std::cout << (c ? "  [OK]  " : "  [ERR] ") << w << "\n";
    if (!c) ++g_fail;
}
...
[[nodiscard]] std::uint64_t qpc_freq() {
    LARGE_INTEGER f;
...
[[nodiscard]] std::uint64_t scan_records(unsigned char const* buf) {
    ...
    for (std::size_t i = 0; i < kN; ++i) {
        std::uint32_t v;
        std::memcpy(&v, buf + i * kRecordSize, sizeof(v));
        s += v;
    }
    return s;
}
...
ModeResult run_buffered(std::uint64_t freq) {
    ...
    r.name = "Buffered";
    ScopedFile sf;
    sf.path = make_temp_path(L"io5b");
    if (sf.path.empty()) {
        tr("Buffered: temp-Pfad", false);
        return r;
    }

    std::vector<unsigned char> src(kTotalBytes);
    fill_block(src.data());

    // Schreiben (gepuffert).
    {
        ScopedHandle h(
            CreateFileW(sf.path.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_TEMPORARY, nullptr));
        if (!h.valid()) {
            tr("Buffered: CreateFileW(write)", false);
            return r;
        }
        DWORD wr = 0;
        if (!WriteFile(h.h, src.data(), static_cast<DWORD>(kTotalBytes), &wr, nullptr) || wr != kTotalBytes) {
            tr("Buffered: WriteFile", false);
            return r;
        }
    }
    ...
    std::uint64_t const        t0  = now_ticks();
    {
        ScopedHandle h(CreateFileW(sf.path.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                                   FILE_ATTRIBUTE_TEMPORARY, nullptr));
        if (!h.valid()) {
            tr("Buffered: CreateFileW(read)", false);
            return r;
        }
        DWORD rd = 0;
        if (!ReadFile(h.h, dst.data(), static_cast<DWORD>(kTotalBytes), &rd, nullptr) || rd != kTotalBytes) {
            tr("Buffered: ReadFile", false);
            return r;
        }
...
ModeResult run_direct(std::uint64_t freq) {
    ...
    r.name = "Direct";
    ScopedFile sf;
    sf.path = make_temp_path(L"io5d");
    if (sf.path.empty()) {
        tr("Direct: temp-Pfad", false);
        return r;
    }
    ...
    if (!vbuf.p) {
        tr("Direct: VirtualAlloc(aligned)", false);
        return r;
    }
    ...
    {
        ScopedHandle h(CreateFileW(sf.path.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS,
                                   FILE_FLAG_NO_BUFFERING | FILE_FLAG_WRITE_THROUGH, nullptr));
        if (!h.valid()) {
            tr("Direct: CreateFileW(NO_BUFFERING,write)", false);
            return r;
        }
        DWORD wr = 0;
        if (!WriteFile(h.h, abuf, static_cast<DWORD>(kTotalBytes), &wr, nullptr) || wr != kTotalBytes) {
            tr("Direct: WriteFile(unbuffered)", false);
            return r;
        }
    }
    ...
    std::uint64_t const t0  = now_ticks();
    {
        ...
        if (!h.valid()) {
            tr("Direct: CreateFileW(NO_BUFFERING,read)", false);
            return r;
        }
        DWORD rd = 0;
        if (!ReadFile(h.h, abuf, static_cast<DWORD>(kTotalBytes), &rd, nullptr) || rd != kTotalBytes) {
            tr("Direct: ReadFile(unbuffered)", false);
            return r;
        }
...
ModeResult run_mmap(std::uint64_t freq) {
    ...
    r.name = "Mmap";
    ScopedFile sf;
    sf.path = make_temp_path(L"io5m");
    if (sf.path.empty()) {
        tr("Mmap: temp-Pfad", false);
        return r;
    }
    ...
    // Datei mit dem Block befuellen (gepuffert genuegt zum Anlegen — das mmap kommt danach).
    {
        ...
        ScopedHandle h(
            CreateFileW(sf.path.c_str(), GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_TEMPORARY, nullptr));
        if (!h.valid()) {
            tr("Mmap: CreateFileW(write)", false);
            return r;
        }
        DWORD wr = 0;
        if (!WriteFile(h.h, src.data(), static_cast<DWORD>(kTotalBytes), &wr, nullptr) || wr != kTotalBytes) {
            tr("Mmap: WriteFile", false);
            return r;
        }
    }

    // File-backed Mapping anlegen + View mappen.
    ScopedHandle fh(CreateFileW(sf.path.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
                                FILE_ATTRIBUTE_TEMPORARY, nullptr));
    if (!fh.valid()) {
        tr("Mmap: CreateFileW(read)", false);
        return r;
    }
    ScopedHandle mh(CreateFileMappingW(fh.h, nullptr, PAGE_READONLY, 0, 0, nullptr));
    if (!mh.valid()) {
        tr("Mmap: CreateFileMappingW", false);
        return r;
    }
    ScopedView view{MapViewOfFile(mh.h, FILE_MAP_READ, 0, 0, kTotalBytes)};
    if (!view.p) {
        tr("Mmap: MapViewOfFile", false);
        return r;
    }
...
int main() {
    ...
    for (auto const& m : all) {
        std::cout << "  " << m.name << std::string(m.name.size() < 9 ? 9 - m.name.size() : 1, ' ')
                  << ": ns/batch=" << m.ns_per_batch << "  syscalls=" << m.syscall_count
                  << "  page_faults=" << m.page_faults_delta << "  checksum=" << m.checksum << "  ("
                  << (m.ok ? "ok" : "FEHLER") << ")\n";
    }
    std::cout << "\n";
    ...
    tr("UNTERSCHEID: Buffered ns/batch > InMemory (echtes IO ist langsamer als RAM)",
       buf.ns_per_batch > inmem.ns_per_batch);
    tr("UNTERSCHEID: Direct ns/batch > InMemory (unbuffered Block-IO ist langsamer als RAM)",
       dir.ns_per_batch > inmem.ns_per_batch);
    tr("UNTERSCHEID: Mmap ns/batch > InMemory (page-fault-getrieben ist langsamer als RAM)",
       mmp.ns_per_batch > inmem.ns_per_batch);
    ...
    bool const profiles_differ = !(inmem.syscall_count == buf.syscall_count && buf.syscall_count == dir.syscall_count &&
    ...
    std::cout << "\n==== P3 ECHTES IO Fixture: " << (g_fail == 0 ? "ALLE OK" : (std::to_string(g_fail) + " FEHLER"))
              << " ====\n";
    return g_fail == 0 ? 0 : 1;
}
