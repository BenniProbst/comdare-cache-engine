...
static ex::ExperimentTree make_tree(std::shared_ptr<ex::ExperimentNodeFactory> const& f, std::size_t n) {
    ...
    t.build({ ex::AxisLevel{"traversal", {"ART"}, true, ""},
              ex::AxisLevel{"node", nodes, true, ""} });
    return t;
}
...
int main() {
    ...
    check_eq("parallel_jobs(4 je Build, 16 Kerne)", (ex::BuildConfig{4,16,{},{}}).parallel_jobs(), std::size_t{4});  // 4×4=16
    check_eq("parallel_jobs(4 je Build, 8 Kerne)",  (ex::BuildConfig{4, 8,{},{}}).parallel_jobs(), std::size_t{2});  // 2×4=8
    check_eq("parallel_jobs(4 je Build, 6 Kerne)",  (ex::BuildConfig{4, 6,{},{}}).parallel_jobs(), std::size_t{1});  // 1×4=4≤6
    // Oversubscription-Schutz: cores_per_build > Kerne → gekappt auf Kerne
    check_eq("effective_cores_per_build(4, 2 Kerne) gekappt", (ex::BuildConfig{4,2,{},{}}).effective_cores_per_build(), std::size_t{2});
    check_eq("parallel_jobs(4, 2 Kerne) == 1 (1×2=2≤2, keine Oversubscription)", (ex::BuildConfig{4,2,{},{}}).parallel_jobs(), std::size_t{1});
    ...
    // ── (B) RAM-Admission: 10 GB frei, 4 GB/Build → max 2 gleichzeitig (RAM-gebunden, nicht CPU) ──
    {
        ...
        check_true("RAM-Low-Water-Mark gemessen (≤ 10GB, > 0)", st.min_free_ram_bytes <= 10*GB && st.min_free_ram_bytes > 0);
    }
    ...
    // ── (C) Inkrementell/resumierbar: versions-aktuelle DLLs überspringen ──
    {
        ...
        auto compile_touch = [](ex::BuildJob const& j) -> int {
            std::ofstream f{j.output, std::ios::binary | std::ios::trunc}; f << "DLL";
            return 0;
        };
        ...
        ex::BuildStats s1; orch1.provision_all(view, &s1);
        ...
        ex::BuildStats s2; orch2.provision_all(view, &s2);
        ...
        ex::BuildConfig cfg2 = cfg; cfg2.build_version = "v2";
        ex::BuildOrchestrator orch3{cfg2, compile_touch, gen};
        ex::BuildStats s3; orch3.provision_all(view, &s3);
