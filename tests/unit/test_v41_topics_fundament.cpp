...
TEST(PhaseF1_Nodes, Node256NodeTypeAxisBase) {
    static_assert(ce_topics::AxisBaseConcept<Node256NodeType>);
    static_assert(ce_concepts::LegacyOriginalCodePflicht<Node256NodeType>);
    static_assert(Node256NodeType::max_capacity() == 256);
    SUCCEED();
}
...
TEST(PhaseF1_CrossTopic, AllUseAxisBaseDefaultCompiler) {
    ...
    static_assert(Node256NodeType::get_compiler() == std::string_view{"original"});
...
TEST(PhaseF1_CrossTopic, AllUseAxisBaseDefaultIsOriginalModule) {
    ...
    static_assert(!Node256NodeType::is_original_module());
