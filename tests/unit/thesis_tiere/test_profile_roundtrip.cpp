...
int main(int argc, char** argv) {
    ...
    fs::path const golden_txt = (argc >= 3) ? fs::path(argv[2])
                                            : (fs::path("tests") / "unit" / "thesis_tiere" /
    ...
    for (auto const& g : golden_ids) {
        bool found = false; for (auto const& p : prof_ids) if (p == g) { found = true; break; }
        if (!found) only_golden.push_back(g);
    }
    for (auto const& p : prof_ids) {
        bool found = false; for (auto const& g : golden_ids) if (g == p) { found = true; break; }
    ...
    for (std::size_t i = 0; i < golden_ids.size() && i < prof_ids.size(); ++i)
        if (golden_ids[i] != prof_ids[i]) { ++mism; if (position_identical) position_identical = false; }
    ...
    for (std::size_t i : {std::size_t{0}, std::size_t{1}, golden_ids.size() - 1}) {
        ...
        if (!same) std::cout << "       profile[" << i << "]: " << (i < prof_ids.size() ? prof_ids[i] : std::string{"<fehlt>"}) << "\n";
    }

    // ── (5) DynamicDims aendern die binary_id NICHT (with_dynamic) ──
    {
        std::vector<ex::AxisLevel> const prof_full = tlz::build_profile_basis_levels(*m3, "m3v2_base", /*with_dynamic=*/true);
    ...
    // ════════════════════════════════════════════════════════════════════════════════════════════════════
    {
        ...
        // (6b) PER-ACHSEN-SWEEP node_type: profile_axis_level == 4; die Sweep-binary_ids variieren NUR node_type.
        {
            ...
            for (std::size_t i = 0; i < pf.selection.indices.size() && i < 4; ++i)
                std::cout << "  Sweep[node_type][" << i << "] = " << view7[pf.selection.indices[i]].binary_id.substr(0, 96) << "...\n";
        }
