...
[[nodiscard]] inline bool cl_log_has_transient_lock(std::filesystem::path const& log) {
    ...
    return content.find("C1083") != std::string::npos || content.find("Permission denied") != std::string::npos ||
...
[[nodiscard]] inline int run_cl_with_c1083_retry(std::string const& cmd, std::filesystem::path const& log) {
    ...
    for (int attempt = 0; attempt < kRetries && rc != 0; ++attempt) {
        ...
        std::cerr << "  [C1083-retry] transienter Datei-Lock (OneDrive-Sync) erkannt — Versuch " << (attempt + 2) << "/"
                  << (kRetries + 1) << " nach " << kBackoffMs[attempt] << "ms Backoff\n";
...
int main(int argc, char** argv) {
    ...
    //    Exit 0 + Zusammenfassung bei OK; Exit != 0 + klare Meldung (Achse + ungueltiger Wert) bei Fehler. ──
    {
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (arg == "--validate" || arg == "-validate" || arg == "--check") {
                want_validate = true;
                continue;
            }
        ...
        if (want_validate) {
            ...
            std::optional<comdare::builder::xml::ThesisProfile> const tp = tlz::load_thesis_profile(validate_path);
            if (!tp) {
    ...
    std::uint32_t const n_repeats = (argc >= 10) ? static_cast<std::uint32_t>(std::strtoul(argv[9], nullptr, 10)) : 3u;
    ...
    if (profile_path.empty()) {
        char const* e = std::getenv("COMDARE_THESIS_PROFILE");
        if (e != nullptr) profile_path = e;
    }
    ...
    if (char const* bv = std::getenv("COMDARE_BUILD_VERSION"); bv != nullptr && *bv != '\0')
        a.build_version_tag_override = bv;
