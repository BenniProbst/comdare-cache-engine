...
using QueueTier    = cea::AdapterAnatomy<cea::AdapterComposition<D,D,D,D,D,D,D,D,D,D,D,D, cea::DequeInner<>>>;  // FIFO
using StackTier    = cea::AdapterAnatomy<cea::AdapterComposition<D,D,D,D,D,D,D,D,D,D,D,D, cea::DequeInner<>>>;  // LIFO
using PriorityTier = cea::AdapterAnatomy<cea::AdapterComposition<D,D,D,D,D,D,D,D,D,D,D,D, cea::HeapInner<>>>;    // Max-Heap
...
static void run(char const* disc, char const* inner, Disc d, std::uint64_t n_ops, int reps) {
    ...
    for (int r = 0; r < reps; ++r) {
        ...
        for (std::uint64_t i = 0; i < half; ++i) {
            if (d == Disc::Lifo) (void)tier.pop_back();   // stack
            else                 (void)tier.pop_front();  // queue (FIFO) + priority (Extract-Max)
        }
        auto const t1 = std::chrono::steady_clock::now();
        auto const ns = static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(t1 - t0).count());
        if (ns < best_ns) { best_ns = ns; obs = tier.observe_all(); }
    }
    double const total_ops = static_cast<double>(n_ops + n_ops / 2);
    double const ns_per_op = static_cast<double>(best_ns) / total_ops;
    std::cout << disc << ';' << inner << ';' << n_ops << ';'
              << obs.push_count << ';' << obs.pop_count << ';' << obs.front_reads << ';' << obs.back_reads << ';'
              << obs.peak_occupancy << ';' << obs.current_occupancy << ';'
              << best_ns << ';' << ns_per_op << '\n';
}
