...
int main(int argc, char** argv) {
    ...
    check("Stufe2_PrueflingReplace → abstract", tlz::derive_pruefling_type("B", "Stufe2_PrueflingReplace", "") == "abstract");
    ...
    std::string hdr_noeol = hdr; if (!hdr_noeol.empty() && hdr_noeol.back() == '\n') hdr_noeol.pop_back();
    ...
    std::cout << "  ZEILE full     : ..." << "pruefling_type=" << last_col(csv_full)     << "  (binary_id=" << row_full.binary_id     << ")\n";
    std::cout << "  ZEILE abstract : ..." << "pruefling_type=" << last_col(csv_abstract) << "  (binary_id=" << row_abstract.binary_id << ")\n";
