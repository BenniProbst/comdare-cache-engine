...
// "per-Host-Verbund2-Kompositionen noetig", bau-relevant mit F/G): ctest-Gate der Verbund2_Replace-
// Materialisierung im sota_catalog.
//
// HISTORIE + KORREKTUR: FRUeHER (2026-07-13) war Verbund2 KONZEPTIONELL EINE Binary (der HOT-Pilot,
// HotPrtVerbund2ReplaceComposition), weil nur der HOT-Host als Verbund2-Komposition existierte -- N <sota_series
// merge="Verbund2_.." lebewesen=X> materialisierten daher DIESELBE HOT-Messung und wurden per (view_binary_id,
...
// Verbund2 je SOTA-Host eine EIGENE reale <Host>PrtVerbund2ReplaceComposition (prt_art_merge_reference.hpp,
// symmetrisch zu Verbund3) -- die binary_ids sind damit GENUINE per-Host distinkt (verschiedene Hosts = verschiedene
...
//   (1) PER-HOST-DISTINKT: mehrere V2-Deklarationen mit verschiedenen lebewesen ? je Host EIN distinkter Pass
//       (build_sota_passes), NICHT mehr zu 1 kollabiert.
//   (2) H2-ATTRIBUTION host-dominant, host == lebewesen: der art-V2-Pass traegt h2_lebewesen == "art" (sein realer
//       Host), NICHT "hot"; über die committete Akte aufgelöst ergibt das den ART-Score.
//   (3) DISTINKT-ZAeHLUNG: das gepinnte m3v2_study (7 St1 + 7 V2 + 7 St3-Deklarationen) liefert 19 Paesse mit
//       19 DISTINKTEN view_binary_ids (7 St1 + 6 V2 [prt_art degeneriert] + 6 St3 [prt_art degeneriert]).
...
//   (5) ECHTE DEDUP bleibt: zwei V2-Deklarationen mit DEMSELBEN lebewesen (gleicher Host) ? 1 Pass.
//
...
// Ein Verbund2-Pass (per-Host): die view_binary_id traegt den <Host>PrtVerbund2ReplaceComposition-Marker.
bool is_st2(tlz::SotaPass const& p) { return p.view_binary_id.find("PrtVerbund2Replace") != std::string::npos; }
...
// Auffaecherung ueber ein synthetisches Profil mit 3 verschiedenen V2-Lebewesen (art/masstree/surf).
TEST(SotaSt2Dedup, MultipleSt2DeclarationsArePerHostDistinctWithOwnHostH2) {
    auto const tp = parse_temp("  <sota_series_set>\n"
                               "    <sota_series id=\"A\" lebewesen=\"art\"      merge=\"Verbund2_Replace\"/>\n"
                               "    <sota_series id=\"A\" lebewesen=\"masstree\" merge=\"Verbund2_Replace\"/>\n"
                               "    <sota_series id=\"A\" lebewesen=\"surf\"     merge=\"Verbund2_Replace\"/>\n"
                               "  </sota_series_set>\n");
    ASSERT_TRUE(tp.has_value());
    ASSERT_EQ(tp->sota_series.size(), 3u) << "3 V2-Deklarationen im Fixture";

    std::vector<tlz::SotaPass> const passes = tlz::build_sota_passes(*tp);
    ASSERT_EQ(passes.size(), 3u) << "M-CE-10 per-Host: 3 verschiedene V2-Lebewesen = 3 distinkte Paesse";
    ...
    for (auto const& p : passes) {
        ...
        EXPECT_EQ(p.h2_lebewesen, p.lebewesen) << "V2 traegt seinen eigenen Host (host == lebewesen)";
    }
    ...
    ASSERT_NE(art_pass, nullptr) << "der art-V2-Pass existiert (per-Host)";
    EXPECT_EQ(tlz::h2_score_for(*akte, art_pass->h2_lebewesen), art->score) << "art-V2-Score == ART-Host-Score";
    EXPECT_NE(tlz::h2_score_for(*akte, art_pass->h2_lebewesen), hot->score)
        << "der art-V2-Pass darf NIE den HOT-Score tragen (die M-CE-10-Fehl-Attribution ist behoben)";
}

// (3) DISTINKT-ZAeHLUNG am gepinnten m3v2_study (7 St1 + 7 V2 + 7 St3): 19 Paesse / 19 distinkte view_binary_ids.
TEST(SotaSt2Dedup, M3v2StudyYieldsNineteenDistinctSotaBinaries) {
    ...
    std::vector<tlz::SotaPass> const passes = tlz::build_sota_passes(*tp);
    // 7 St1 (prt_art + 6 SOTA isoliert) + 6 V2 (per-Host; prt_art degeneriert = nullopt) + 6 St3 (prt_art
    // degeneriert = nullopt) = 19 distinkte reale Kompositionen (M-CE-10 per-Host: vor der Auffächerung 14).
    EXPECT_EQ(passes.size(), 19u) << "M-CE-10 per-Host: 7 St1 + 6 V2 + 6 St3 = 19 Paesse";
    ...
    // GENAU 6 V2-Paesse (art/hot/masstree/surf/start/wormhole), je host-dominant auf DEN EIGENEN Host attribuiert.
    std::set<std::string> st2_hosts;
    for (auto const& p : passes)
        if (is_st2(p)) {
            st2_hosts.insert(p.h2_lebewesen);
            EXPECT_EQ(p.h2_lebewesen, p.lebewesen) << "jeder V2-Pass traegt seinen eigenen Host-Score";
            EXPECT_EQ(p.pruefling_type, "abstract");
            EXPECT_NE(p.h2_lebewesen, "prt_art") << "prt_art-Host ist in V2 degeneriert (nullopt)";
        }
    EXPECT_EQ(st2_hosts.size(), 6u) << "die 7 V2-Deklarationen ergeben 6 reale per-Host-Paesse (prt_art degeneriert)";
}
...
TEST(SotaSt2Dedup, FairnessVariantsAreNotCollapsedBySameBinaryId) {
    auto const tp = parse_temp(
        "  <sota_series_set>\n"
        "    <sota_series id=\"A\" lebewesen=\"hot\" merge=\"Verbund1_CeOnly\" fairness=\"common_denominator\"/>\n"
        "    <sota_series id=\"A\" lebewesen=\"hot\" merge=\"Verbund1_CeOnly\" fairness=\"native\"/>\n"
        "  </sota_series_set>\n");
    ASSERT_TRUE(tp.has_value());
...
// (5) ECHTE DEDUP bleibt: zwei V2-Deklarationen mit DEMSELBEN lebewesen (gleicher Host, gleicher fairness_mode)
