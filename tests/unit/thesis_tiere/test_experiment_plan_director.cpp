...
// (E-04-P1, §61-Dual-Weg) SliceKanalLaneReachesBareMetalPath: der bare-metal-Weg (CMake-Targets) traegt DIESELBE
//   Lane-Aussage wie die CI-Emission. Fehlte sie dort, stuende der lokale Dual-Weg-Beweis mit lane=unbelegt gegen
//   die CI-Zeilen -- die beiden Wege waeren nicht mehr vergleichbar (§61 verlangt genau diese Vergleichbarkeit).
TEST(TierCmakeGraphBuilder, SliceKanalLaneReachesBareMetalPath) {
    ...
    EXPECT_EQ(count_occurrences(txt, "\"COMDARE_LANE="), simd_zeilen)
        << "JEDER Treiber-COMMAND des bare-metal-Wegs traegt seine Lane (§61-Dual-Weg-Symmetrie)";
