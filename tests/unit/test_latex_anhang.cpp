...
int write_latex(std::string const& path, std::vector<CsvRow> const& rows,
                std::string const& caption, std::string const& label);
int write_latex_full_metrics(std::string const& path, std::vector<CsvRow> const& rows,
                              std::string const& caption, std::string const& label);
}
...
public:
    explicit TempFile(std::string const& suffix) {
        path_ = std::filesystem::temp_directory_path()
              / ("comdare_latex_test_" + std::to_string(rand()) + suffix);
    }
...
void write_csv_with_header(std::filesystem::path const& p,
                            std::vector<std::string> const& body_lines) {
    std::ofstream out{p};
...
TEST(LatexAnhang, EscapesBraces) {
    EXPECT_EQ(latex_anhang::escape_latex("{ok}"), "\\{ok\\}");
}
...
TEST(LatexAnhang, WriteLatexHasBooktabsStructure) {
    ...
    TempFile tex{".tex"};
    EXPECT_EQ(latex_anhang::write_latex(tex.path().string(), rows,
                                         "Test caption", "tab:test"), 0);
    ...
    EXPECT_NE(content.find("ce\\_test:\\_x"),   std::string::npos)
        << "Underscores must be escaped in LaTeX output";
...
TEST(LatexAnhang, WriteLatexFullMetricsHasAllColumns) {
    ...
    TempFile tex{".tex"};
    EXPECT_EQ(latex_anhang::write_latex_full_metrics(tex.path().string(), rows,
                                                      "Full", "tab:full"), 0);
    ...
    EXPECT_NE(content.find(" & 7 "),       std::string::npos)
        << "dTLB-Wert sollte in der Tabelle erscheinen";
    EXPECT_NE(content.find(" & 4 "),       std::string::npos)
        << "Coherence-Wert sollte in der Tabelle erscheinen";
}
...
TEST(LatexAnhang, WriteLatexSkipsFailedRows) {
    ...
    EXPECT_EQ(content.find("fail\\_row"), std::string::npos)
        << "Failed rows should be skipped in summary table";
}
