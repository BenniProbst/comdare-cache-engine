// S7-3 (#261/#234) -- SkipList-Pool allocator parameter + T6 DEG hook.
...
using Shape        = slshape::SkipListMax16P50;
using DefaultStore = lkc::SkipListNodePoolStore<>;
using DefaultOrgan = lkc::SkipListOrgan;
using SkipHull     = lkc::ObservableComposedContainer<DefaultOrgan>;

static_assert(std::is_same_v<DefaultStore, lkc::SkipListNodePoolStore<Shape>>);
static_assert(lkc::SkipListNodePool<DefaultStore>);
...
static_assert(requires(SkipHull const& hull) { hull.store_allocator_statistics(); });
...
struct PoolFlipComposition {
    ...
    static constexpr std::string_view paper_id = "S7-3 SkipList pool allocator DEG";
    static constexpr std::string_view name     = "S73SkipListPoolComposition";
};
...
TEST(S73SkipListPoolAllocatorDeg, DirectPoolStatsAreReal) {
    SkipHull               hull;
    std::vector<U64> const keys{40u, 12u, 90u, 7u, 25u, 70u, 110u, 1u, 3u, 6u, 9u, 13u};
...
TEST(S73SkipListPoolAllocatorDeg, AbiObserverRoutesPoolAllocatorStatsToT6) {
    using C       = PoolFlipComposition<lk::SkipListSearchAlgo>;
    ...
    std::cout << "S7-3 T6-DEG alloc_cnt=" << snap.axis_stats[6][2] << " bytes_alloc=" << snap.axis_stats[6][0]
...
TEST(S73SkipListPoolAllocatorDeg, DefaultStoreRoundtripIsUnchanged) {
    DefaultOrgan           organ;
...
TEST(S73SkipListPoolAllocatorDeg, NeutralityGuardsStayIntact) {
