// S7-5 (#261/#234) -- ART-Pool allocator parameter + T6 DEG hook.
...
using DefaultStore = lkc::ArtTrieNodePoolStore<>;
using DefaultOrgan = lkc::ArtTrieOrgan;
using ArtHull      = lkc::ObservableComposedContainer<DefaultOrgan>;

static_assert(lkc::ArtTrieNodePool<DefaultStore>);
...
static_assert(requires(ArtHull const& hull) { hull.store_allocator_statistics(); });
...
struct PoolFlipComposition {
    ...
    static constexpr std::string_view paper_id = "S7-5 ART pool allocator DEG";
    static constexpr std::string_view name     = "S75ArtPoolComposition";
};
...
TEST(S75ArtPoolAllocatorDeg, DirectPoolStatsAreReal) {
    ArtHull                hull;
...
TEST(S75ArtPoolAllocatorDeg, AbiObserverRoutesPoolAllocatorStatsToT6) {
    using C       = PoolFlipComposition<lk::OriginalArtSearchAlgo>;
    ...
    std::cout << "S7-5 T6-DEG alloc_cnt=" << snap.axis_stats[6][2] << " bytes_alloc=" << snap.axis_stats[6][0]
...
TEST(S75ArtPoolAllocatorDeg, DefaultStoreRoundtripIsUnchanged) {
    DefaultOrgan           organ;
...
TEST(S75ArtPoolAllocatorDeg, NeutralityGuardsStayIntact) {
