...
struct LinearScanComposition {
    ...
    COMDARE_DEFINE_COMPOSITION_LOCATION("::comdare::cache_engine::compositions::LinearScanComposition",
                                        "tests/unit/test_m8_storetrav_segment.cpp");
};
} // namespace comdare::cache_engine::compositions
...
int main() {
    ...
    chk("tier_search_routes_through_store()==true (LinearScan store-traversierbar)",
    ...
    std::uint64_t v   = 0;
    bool          hit = drv->tier_lookup(1234, &v);
    ...
    std::cout << "    seg_ns[T0,T4,T5,T6] = " << u.seg_ns[0] << "," << u.seg_ns[4] << "," << u.seg_ns[5] << ","
              << u.seg_ns[6] << "\n";
