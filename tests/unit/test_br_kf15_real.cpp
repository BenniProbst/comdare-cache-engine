...
int main() {
    ...
    tree.for_each_binary([&](std::string const& b, std::string const& p, ex::TreeNode const&) {
        if (first_bin.empty()) { first_bin = b; paper_sig = p; }
    });
    ...
    nv.observer.search_insert_count = 256; nv.observer.tier_fill_level = 256; nv.observer_real = true;
    nv.has_result = true; nv.measured_setting_count = 1; nv.sum_op_count = 512;
    ...
    for (auto const& b : bins) if (b.find("node_type=") == std::string::npos || b.find("memory_layout=") == std::string::npos) real_paths = false;
