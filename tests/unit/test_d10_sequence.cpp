...
static_assert(SC::slot_count == 11, "Sequence = 10 geteilte + axis_growth (§28, K-B)");
...
int main() {
    ...
    eq("SC::slot_count == 11 (10 + axis_growth)", SC::slot_count, std::size_t{11});
    tr("genus() == Sequence (Reptil)", SAnat::genus() == cea::AnatomyGenus::Sequence);
    eq("organ_count() == 11", SAnat::organ_count(), std::size_t{11});
    ...
    if (st) {
        ...
        eq("observe: organ_count == 11", pod.organ_count, std::uint64_t{11});
