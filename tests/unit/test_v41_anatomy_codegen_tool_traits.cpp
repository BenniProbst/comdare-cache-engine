...
TEST(R5G_CompositionLocation, AdHocCompositionDoesNotConform) {
    ...
    using AdHocArt = ana::AdHocComposition<
        comp::ArtComposition::search_algo,
...
TEST(R5G_DescriptorFromComposition, HotPaperBindingDescriptorIsCorrect) {
    ...
    static_assert(desc.header_include ==
        std::string_view{"compositions/hot_paper_binding_reference.hpp"});
    SUCCEED();
}
...
TEST(R5G_TableConsistency, AllSixCeReimplMatchHardcoded) {
    ...
    for (auto const& p : pairs) {
        ...
        EXPECT_EQ(entry->cpp_type_name,  p.cpp_type_name)
            << "Mismatch fuer short_name=" << p.short_name;
        EXPECT_EQ(entry->header_include, p.header_include)
            << "Mismatch fuer short_name=" << p.short_name;
    }
}
