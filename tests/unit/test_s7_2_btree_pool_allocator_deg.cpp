// S7-2 (#261/#234) -- BTree-Pool allocator parameter + T6 DEG hook.
...
using Shape        = ord::BtreeOrderKt4;
using DefaultStore = lkc::BTreeNodePoolStore<>;
using DefaultOrgan = lkc::BTreeSearchOrgan;
using BTreeHull    = lkc::ObservableComposedContainer<DefaultOrgan>;
...
struct PoolFlipComposition {
    ...
    static constexpr std::string_view paper_id = "S7-2 BTree pool allocator DEG";
    static constexpr std::string_view name     = "S72BTreePoolComposition";
};
...
TEST(S72BTreePoolAllocatorDeg, DirectPoolStatsAreReal) {
    BTreeHull              hull;
...
TEST(S72BTreePoolAllocatorDeg, AbiObserverRoutesPoolAllocatorStatsToT6) {
    using C       = PoolFlipComposition<lk::BTreeSearchAlgo>;
    ...
    std::cout << "S7-2 T6-DEG alloc_cnt=" << snap.axis_stats[6][2] << " bytes_alloc=" << snap.axis_stats[6][0]
...
TEST(S72BTreePoolAllocatorDeg, DefaultStoreRoundtripIsUnchanged) {
    DefaultOrgan           organ;
...
TEST(S72BTreePoolAllocatorDeg, NeutralityGuardsStayIntact) {
