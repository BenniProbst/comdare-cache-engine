...
//      drift_faktor        = 3*(5+1)                    = 18   (Eingang nennt max_reruns=5 EXPLIZIT)
//      paar_faktor         = 2                                 (KON47-04, konstant)
//      t15b_retry          = 5                                 (Default MessRetryKonfig)
//      arena_gesamt_faktor = 18*2*5                     = 180
//      zeilen_je_op_batch  = 10*4*180                   = 7200
//      arena_bytes         = 7200*32                    = 230400
//   B  ops_gesamt          = 10*8192*6*18*2             = 17694720   (paar x2; retry NICHT: Fehlerpfad)
//      dauer_s             = 17694720*0.001             = 17694.72
//      dauer_tage          = 17694.72/86400             = 0.2048
//   H  DIE 160-MiB-REKONSTRUKTION (der eigentliche Anlass dieses Kommandos), FORTGESCHRIEBEN 20.08.:
//      131072*40*32                                     = 167772160 B  = 160.0000 MiB (der historische
//                                                         EIN-Durchlauf-Anker; heute arena/gesamt)
//      Gate AUS + retry=1: gesamt = 1*2*1 = 2           => 335544320 B  = 320 MiB
//      Defaults (drift 18 im Eingang, paar 2, retry 5) = 180 => 30198988800 B = 28800 MiB = 28.125 GiB
//      Der verschluckte Faktor ist damit beziffert statt behauptet -- frueher 18, seit KON47-04/
//      KON26-04 bis zu 180 (bzw. 120 mit dem neuen Drift-Default 3: 12*2*5).
//
...
int main() {
    ...
    // == A: DIE KERN-RECHNUNG gegen die von Hand eingefrorenen Zahlen ==============================
    {
        ...
        pruefe_gleich(s.paar_faktor, 2u, "A: paar_faktor == 2 (KON47-04)");
        pruefe_gleich(s.t15b_retry_faktor, 5u, "A: t15b_retry == 5 (MessRetryKonfig-Default)");
        pruefe_gleich(s.arena_gesamt_faktor, 180u, "A: arena_gesamt_faktor == 18*2*5");
        pruefe_gleich(s.zeilen_je_op_batch, 7200u, "A: zeilen_je_op_batch == 10*4*180");
        pruefe_gleich(s.arena_bytes_je_op_batch, 230400u, "A: arena_bytes == 7200*32");
        pruefe_gleich(s.ops_gesamt, 17694720u, "A: ops_gesamt == 10*8192*6*18*2 (paar, OHNE retry)");
    ...
    // T-2: nicht "der Bericht ist nicht leer", sondern: genau diese Faktoren, mit ihrem Nenner.
    {
        ...
        pruefe_enthaelt(b, "paar_faktor", "A': Faktor paar_faktor steht einzeln im Bericht (KON47-04)");
        pruefe_enthaelt(b, "t15b_retry_max_versuche", "A': Faktor t15b_retry steht einzeln im Bericht");
        pruefe_enthaelt(b, "arena_gesamt_faktor", "A': der Arena-Gesamt-Faktor steht einzeln im Bericht");
        ...
        pruefe_enthaelt(b, "mess_warmup_paar.hpp", "A': der Nenner des Paar-Faktors steht im Bericht");
        pruefe_enthaelt(b, "mess_retry_klammer.hpp", "A': der Nenner des Retry-Faktors steht im Bericht");
        pruefe_enthaelt(b, "D.7", "A': der D.7-Entscheid ist im Gesamt-Faktor-Nenner benannt");
    ...
    // == B: DIE DAUER -- nur mit ausdruecklicher Kalibrierung ======================================
    {
        ...
        pruefe(std::fabs(s.dauer_s - 17694.72) < 1e-6, "B: dauer_s == 17694720*0.001 (paar x2)");
        pruefe(std::fabs(s.dauer_tage - 0.2048) < 1e-9, "B: dauer_tage == 17694.72/86400");
    }

    // == H: DIE 160-MiB-REKONSTRUKTION und ihre Gesamt-Faktor-Fortschreibung =======================
    // Das ist der Grund, warum es dieses Kommando gibt: dieselbe Konfiguration, einmal mit dem
    // kleinsten rechenbaren Fenster (Gate AUS, retry=1 -- es bleibt das PAAR x2, das per KON47-04
    // JEDEN Lauf faehrt) und einmal mit allen Vervielfachern -- 320 MiB gegen 28800 MiB. Der
    // historische EIN-Durchlauf-Anker 160 MiB (2^17 Ops x 40 Zeilen/Op x 32 B) ist weiterhin exakt
    // rekonstruierbar: arena_ohne / paar_faktor.
    {
        auto e               = eingang_a();
        ...
        auto e_ohne_gate              = e;
        e_ohne_gate.drift_reps        = 1; // reps < 2 == GATE AUS => genau EIN Drift-Durchlauf
        e_ohne_gate.t15b_max_versuche = 1; // Retry-worst-case ausgeblendet => nur das Paar bleibt
        ...
        pruefe_gleich(s_ohne.arena_gesamt_faktor, 2u, "H: Gate AUS + retry=1 => nur das Paar (2)");
        pruefe_gleich(s_ohne.arena_bytes_je_op_batch, 335544320u, "H: 2^17*40*32*2 == 320 MiB");
        pruefe_gleich(s_ohne.arena_bytes_je_op_batch / s_ohne.paar_faktor, 167772160u,
                      "H: der historische EIN-Durchlauf-Anker 160 MiB == arena/paar");

        auto const s_mit = pl::mengen_rechnen(e); // drift 18 (Eingang), paar 2, retry 5 => 180
        pruefe_gleich(s_mit.drift_faktor, 18u, "H: Gate AN => 18");
        pruefe_gleich(s_mit.arena_gesamt_faktor, 180u, "H: Gesamt == 18*2*5");
        pruefe_gleich(s_mit.arena_bytes_je_op_batch, 30198988800u, "H: 2^17*40*32*180 == 28800 MiB");
        // Die Aussage, um die es geht: die Vervielfacher zusammen sind exakt 90x das kleinste Fenster.
        pruefe_gleich(s_mit.arena_bytes_je_op_batch, s_ohne.arena_bytes_je_op_batch * 90u,
                      "H: der Faktor zwischen beiden ist exakt 90 (180/2)");
    }

    // == T-4 GEGENEINGANG 1: der Speicher-Deckel REISST ===========================================
    {
        auto e         = eingang_a();
        e.deckel_bytes = 230399; // ein Byte unter dem Bedarf von 230400
        auto const s   = pl::mengen_rechnen(e);
        pruefe(s.speicher_urteil == pl::Deckelurteil::Gerissen, "G1: 230399 < 230400 => GERISSEN");
    ...
    {
        auto e         = eingang_a();
        e.deckel_bytes = 230400; // exakt der Bedarf
        auto const s   = pl::mengen_rechnen(e);
        pruefe(s.speicher_urteil == pl::Deckelurteil::Haelt, "G2: 230400 == 230400 => haelt");
    ...
    // == T-4 GEGENEINGANG 4: Zeit-Deckel MIT Kalibrierung, beide Richtungen =======================
    {
        ...
        auto e2           = e;
        e2.sekunden_je_op = 1.0; // => 17694720 s == 204.8 Tage
        auto const s2     = pl::mengen_rechnen(e2);
        pruefe(s2.zeit_urteil == pl::Deckelurteil::Gerissen, "G4: 204.8 > 4.5 => GERISSEN");
    ...
    {
        // 5h (20.08.2026) -- die GESAMT-FAKTOR-Wache allein: drift_faktor selbst passt noch in
        // uint64 (2*(2^62-1+1) = 2^63), erst drift * paar laeuft ueber. Die Arena- und Kampagnen-
        // Wachen koennen diesen Eingang nicht faengen -- die Rechnung bricht in Abschnitt 4b ab.
        auto e             = eingang_a();
        e.drift_reps       = 2;
        e.drift_max_reruns = 4611686018427387903u; // 2^62-1 => drift = 2*(2^62) = 2^63
        auto const s       = pl::mengen_rechnen(e);
        pruefe(s.ueberlauf, "G5h: die Gesamt-Faktor-Wache erkennt den Ueberlauf");
        pruefe(s.grund.find("drift_faktor * paar_faktor") != std::string::npos,
               "G5h: es war die GESAMT-FAKTOR-Wache, nicht eine spaetere");
    }
    ...
    // == T-4 GEGENEINGANG 7: nur Makro-Ebene => 2 Zeilen je Op, die Achszahl faellt weg ===========
    {
        ...
        pruefe_gleich(s.zeilen_je_op_batch, 3600u, "G7: 10*2*180 == 3600");
    }
    ...
    // normal -- der Unbestimmbar-Zweig verschluckt das echte Urteil nicht.
    {
        ...
        e.tooling_leer = false;
        e.ebene_macro  = true;
        e.ebene_micro  = false; // zeilen_je_op = 2 => arena = 10*2*180*32 = 115200
        e.deckel_bytes = 1;
        auto const s   = pl::mengen_rechnen(e);
        pruefe_gleich(s.arena_bytes_je_op_batch, 115200u, "G9b: arena == 10*2*180*32");
        pruefe(s.speicher_urteil == pl::Deckelurteil::Gerissen, "G9b: 115200 > 1 => GERISSEN, nicht unbestimmbar");
    }
