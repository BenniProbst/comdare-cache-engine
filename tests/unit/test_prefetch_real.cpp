...
static void test_real_address_in_backing() {
    ...
    bool all_in = true; bool addr_eq_slot = true;
...
static void test_four_strategies_differ() {
    ...
    tr("(B) Hardware: genau 1 Prefetch auf den AKTUELLEN Slot (dist==0)", rh.prefetches_issued == 1 && rh.last_distance == 0);
    ...
    tr("(B) Path: MEHRERE Prefetches (Bundle, issued>1), groesste issued-Summe",
       rp.prefetches_issued > 1 && rp.prefetches_issued >= rh.prefetches_issued && rp.prefetches_issued >= rd.prefetches_issued);
...
static std::uint64_t fuzz_strategy(RealStore const& store, std::string_view name) {
    ...
    std::uint64_t total_issued = 0; std::uint64_t oob = 0;
    ...
    for (std::uint64_t t = 0; t < N; ++t) {
        ...
        if (res.last_address != nullptr) {
            ...
            bool found = false;
            for (std::size_t j = 0; j < n && !found; ++j) if (store.slot_address(j) == res.last_address) found = true;
            if (!found) ++oob;
        }
    }
...
static an::ComdareTierObserverSnapshot drive_tier_and_observe(std::uint64_t n_keys) {
    ...
    if (!drv || !obs) { tr("(D) IDriveableTier + IObservableTier vorhanden", false); return snap; }
...
static void test_hotpath_end_to_end() {
    ...
    std::cout << "     hardware-Tier: real_prefetches_issued=" << hw_issued
...
int main() {
    std::cout << "==== K9-Fix: REALER prefetch (_mm_prefetch auf reale Store-Adressen, 4 Strategien differenzieren) ====\n";
    test_real_address_in_backing();
    test_four_strategies_differ();
    test_fuzz_no_crash();
    test_hotpath_end_to_end();
    std::cout << "==== K9-Fix REALER prefetch: "
