...
static void test_strategy_direct(std::string_view name, double max_fp_rate) {
    ...
    for (std::uint64_t i = 0; i < N; ++i) if (f.probe_key(ins_key(i))) { empty_all_neg = false; break; }
    ...
    for (std::uint64_t i = 0; i < N; ++i) if (f.probe_key(ins_key(i))) ++pos_hits;
    ...
    std::uint64_t fp = 0;
    for (std::uint64_t i = 0; i < N; ++i) if (f.probe_key(miss_key(i))) ++fp;
    double const fp_rate = static_cast<double>(fp) / static_cast<double>(N);
    std::cout << "     false-positives: " << fp << "/" << N << " (rate=" << fp_rate << ")\n";
    tr(std::string{name} + ": (a) nicht-eingefuegte Keys ueberwiegend negativ (FP-Rate plausibel)", fp_rate <= max_fp_rate);
    ...
    for (std::uint64_t i = 0; i < N; ++i) if (f.probe_key(ins_key(i))) { cleared_all_neg = false; break; }
...
int main() {
    ...
    // ─────────────────────────────────────────────────────────────────────────────────────────────────────────
    {
        ...
        for (std::uint64_t i = 0; i < kBase; ++i) if (!flt_inst_before.strategy_instance().probe_key(i)) { base_all_pos = false; break; }
        ...
        for (std::uint64_t i = kBase; i < kBase + kWarm; ++i) if (!tier.filter_instance().strategy_instance().probe_key(i)) { warm_in_filter = false; break; }
        tr("(c) nach Warmup-Insert: Warmup-Keys sind im REALEN Filter (mutiert, kein no-op)", warm_in_filter);
        tr("(c) nach Warmup-Insert: Filter != Pre-Save-Stand (echte Mutation)", !(tier.filter_instance() == flt_copy_before));
        ...
        for (std::uint64_t i = 0; i < kBase; ++i) if (!tier.filter_instance().strategy_instance().probe_key(i)) { base_still_pos = false; break; }
        ...
        for (std::uint64_t i = kBase; i < kBase + kWarm; ++i) if (!tier.filter_instance().strategy_instance().probe_key(i)) ++warm_back_neg;
        std::cout << "  (c) Warmup-Keys nach Rollback wieder negativ: " << warm_back_neg << "/" << kWarm << "\n";
        tr("(c) Memento: Warmup-Keys nach Rollback ueberwiegend wieder negativ (kein Leck)", warm_back_neg >= (kWarm * 80) / 100);
    }

    std::cout << "==== P5 REALER Filter aus Keys: "
