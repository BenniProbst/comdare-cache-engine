...
#include <organ_axes/node/axis_04_node_type_chunked_store.hpp> // NodeChunkedStore (organ_observe_*)
...
static_assert(std::is_standard_layout_v<vh::ValueHandleSnapshot> &&
              std::is_trivially_copyable_v<vh::ValueHandleSnapshot>);
...
static vh::ValueHandleSnapshot drive_vh(char const* name, std::uint64_t& indirect_out) {
    ...
    std::cout << "  vh " << name << ": access=" << s.total_access_count << " indirect=" << s.indirect_deref_count
              << " vtag=" << s.version_tag_strips << " depth=" << s.peak_chain_depth << "\n";
    return s;
}
...
int main() {
    ...
    std::cout << "==== Phase B ISO T11/T12: " << (g_fail == 0 ? "ALLE OK" : (std::to_string(g_fail) + " FEHLER"))
              << " ====\n";
    return g_fail == 0 ? 0 : 1;
}
