// S7-8 (#261/#234) -- Wormhole-Pool allocator parameter + T6 DEG hook.
...
using DefaultStore = lkc::WormholeLeafListPoolStore<>;
using DefaultOrgan = lkc::WormholeOrgan;
...
struct PoolFlipComposition {
    ...
    static constexpr std::string_view paper_id = "S7-8 Wormhole pool allocator DEG";
    static constexpr std::string_view name     = "S78WormholePoolComposition";
};
...
TEST(S78WormholePoolAllocatorDeg, DirectPoolStatsAreReal) {
    WormholeHull           hull;
    std::vector<U64> const keys{0u, 1u, 2u, 3u, 4u, 5u, 6u, 7u, 8u, 9u, 10u, 11u, 12u, 13u, 14u};
...
TEST(S78WormholePoolAllocatorDeg, AbiObserverRoutesPoolAllocatorStatsToT6) {
    using C       = PoolFlipComposition<lk::OriginalWormholeSearchAlgo>;
    ...
    std::cout << "S7-8 T6-DEG alloc_cnt=" << snap.axis_stats[6][2] << " bytes_alloc=" << snap.axis_stats[6][0]
...
TEST(S78WormholePoolAllocatorDeg, DefaultStoreRoundtripIsUnchanged) {
    DefaultOrgan           organ;
    std::vector<U64> const keys{5u, 2u, 8u, 1u, 3u, 7u, 9u, 4u, 6u, 10u, 11u, 12u};
...
TEST(S78WormholePoolAllocatorDeg, NeutralityGuardsStayIntact) {
