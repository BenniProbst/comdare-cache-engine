...
static void check_eq(char const* w, A const& g, B const& e) {
    bool ok = (g == e);
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "usage: test_d4b_container_dll <perm_container.dll>\n";
        return 2;
    }
    ...
    if (st != loader::status_ok) {
        std::cerr << "  load-Status: " << loader::status_name(st) << "\n";
        std::cout << "\n==== D4b.2: FEHLER (load) ====\n";
        return 1;
    }
    ...
    if (ct) {
        ...
        ct->tier_observe_container(&pod);
        check_eq("tier_observe_container: front_reads == pop_count (get == pop_front je Entnahme)", pod.front_reads,
                 pod.pop_count);
    }

    std::cout << "\n==== D4b.2 Container-DLL-Round-Trip: "
