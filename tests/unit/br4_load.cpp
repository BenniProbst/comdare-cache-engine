...
void check_eq(char const* w, A const& g, B const& e) {
    bool ok = (g == e);
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cerr << "usage: br4_load <perm.dll> <path.txt>\n";
        return 2;
    }
    ...
    check_eq("composition_name == AdHocComposition", std::string{a->composition_name()},
             std::string{"AdHocComposition"});
