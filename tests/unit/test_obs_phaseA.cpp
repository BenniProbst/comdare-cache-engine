...
//     (4) filled_axis_count == kV3FilledAxisCount (schema-abgeleitet; 18 seit Bau-INC-2c).
...
static void check_one(char const* name, an::ComdareTierObserverSnapshot const& s, std::uint32_t expected_filled) {
    ...
    // kV3FilledAxisCount(18), sondern nur ihre Nicht-Store-Achsen (komposition-spezifisch, seit INC-2c ohne
    // auto-gekoppelte Telemetrie-Zeile: Art/Hot=9, Masstree=8).
