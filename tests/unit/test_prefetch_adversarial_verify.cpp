...
using RealStore = nd::LayoutAwareChunkedStore<nd::Node4NodeType, ml::CacheLineAlignedMemoryLayout, al::MimallocAllocator>;
static std::uint64_t spread_key(std::uint64_t i) { return (i * 2654435761ull) ^ (i << 17) ^ 0x9E3779B97F4A7C15ull; }
static RealStore make_store(std::size_t n) { RealStore s; for (std::size_t i=0;i<n;++i) s.append_slot(spread_key(i), i*7u+1u); return s; }
...
static bool addr_is_real_slot(RealStore const& s, unsigned char const* addr) {
    ...
    for (std::size_t j = 0; j < n; ++j) if (s.slot_address(j) == addr) return true;
    return false;
}
...
static void v2_clamp_no_oob() {
    ...
    for (std::uint64_t t = 0; t < 1000000ull; ++t) {
        ...
        auto rp = pf::PrefetchDescentPolicy<pf::PathOrientedPrefetch>::drive(store, i);
        if (rp.last_address && !addr_is_real_slot(store, rp.last_address)) { oob = true; break; }
        auto rh = pf::PrefetchDescentPolicy<pf::HardwarePrefetch>::drive(store, i);
        if (rh.last_address && !addr_is_real_slot(store, rh.last_address)) { oob = true; break; }
    }
    ...
    tr("(V2) leerer Store: 0 issued, nullptr (kein Deref n-1 bei n==0)", re.prefetches_issued == 0 && re.last_address == nullptr);
}

template <class PFStrategy>
struct PFComp : comp::HotComposition { using prefetch = PFStrategy; static constexpr std::string_view name = "PFComp"; };
...
static an::ComdareTierObserverSnapshot drive_tier(std::uint64_t n) {
    ...
    std::uint64_t v=0; for (std::uint64_t i=0;i<n;++i) (void)drv->tier_lookup(spread_key(i), &v);
...
static void v3_v4_hotpath_real_and_differ() {
    ...
    std::uint64_t i_no = no.axis_stats[7][5], i_hw = hw.axis_stats[7][5], i_di = di.axis_stats[7][5], i_pa = pa.axis_stats[7][5];
    std::uint64_t a_hw = hw.axis_stats[7][7];
    std::uint64_t d_di = di.axis_stats[7][6];
    std::cout << "     none="<<i_no<<" hw="<<i_hw<<" dist="<<i_di<<"(d="<<d_di<<") path="<<i_pa<<" hw_addr="<<a_hw<<"\n";
    ...
    for (std::uint64_t i = 0; i < N; ++i) if (static_cast<std::uintptr_t>(spread_key(i)) == a_hw) { addr_equals_some_key = true; break; }
    tr("(V3) HW-Adresse ist KEIN eingefuegter Schluessel-Wert (kein K9-Pseudo-Adress-Rueckfall)", !addr_equals_some_key);
...
static std::uint64_t big_fuzz(RealStore const& s, std::string_view nm) {
    ...
    std::uint64_t issued = 0, oob = 0; std::size_t const n = s.slot_count();
