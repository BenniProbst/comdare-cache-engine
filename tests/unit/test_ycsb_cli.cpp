...
TEST(YcsbCli, ParseWorkloadAllLetters) {
    wg::YcsbWorkload w;
    EXPECT_EQ(ycsb_cli::parse_workload("A", w), 0); EXPECT_EQ(w, wg::YcsbWorkload::A);
    EXPECT_EQ(ycsb_cli::parse_workload("B", w), 0); EXPECT_EQ(w, wg::YcsbWorkload::B);
    EXPECT_EQ(ycsb_cli::parse_workload("c", w), 0); EXPECT_EQ(w, wg::YcsbWorkload::C);
    EXPECT_EQ(ycsb_cli::parse_workload("F", w), 0); EXPECT_EQ(w, wg::YcsbWorkload::F);
}
...
TEST(YcsbCli, ParseFormatAllOptions) {
    ...
    EXPECT_EQ(ycsb_cli::parse_format("binary", f), 0); EXPECT_EQ(f, ycsb_cli::OutputFormat::Binary);
    EXPECT_EQ(ycsb_cli::parse_format("bin",    f), 0); EXPECT_EQ(f, ycsb_cli::OutputFormat::Binary);
    EXPECT_EQ(ycsb_cli::parse_format("tsv",    f), 0); EXPECT_EQ(f, ycsb_cli::OutputFormat::Tsv);
    EXPECT_EQ(ycsb_cli::parse_format("json",   f), 0); EXPECT_EQ(f, ycsb_cli::OutputFormat::Json);
}
...
TEST(YcsbCli, ParseKeyDistAllOptions) {
    ...
    EXPECT_EQ(ycsb_cli::parse_key_dist("uniform",    d), 0); EXPECT_EQ(d, wg::KeyDistribution::Uniform);
    EXPECT_EQ(ycsb_cli::parse_key_dist("zipfian",    d), 0); EXPECT_EQ(d, wg::KeyDistribution::Zipfian);
    EXPECT_EQ(ycsb_cli::parse_key_dist("sequential", d), 0); EXPECT_EQ(d, wg::KeyDistribution::Sequential);
    EXPECT_EQ(ycsb_cli::parse_key_dist("latest",     d), 0); EXPECT_EQ(d, wg::KeyDistribution::Latest);
}
...
TEST(YcsbCli, GenerateAndWriteAllYcsbVariants) {
    for (auto w : {wg::YcsbWorkload::A, wg::YcsbWorkload::B, wg::YcsbWorkload::C,
                   wg::YcsbWorkload::D, wg::YcsbWorkload::E, wg::YcsbWorkload::F}) {
        ycsb_cli::CliConfig cfg;
        cfg.workload = w;
        cfg.num_keys = 50;
        cfg.num_ops  = 25;
        ...
        EXPECT_EQ(ycsb_cli::generate_and_write(cfg), 0)
            << "Workload variant failed: " << static_cast<int>(w);
        std::filesystem::remove(cfg.output);
    }
}
