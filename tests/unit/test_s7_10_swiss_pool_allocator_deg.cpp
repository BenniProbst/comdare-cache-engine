// S7-10 (#261/#234) -- SwissTable-Pool allocator parameter + T6 DEG hook.
...
using DefaultStore = lkc::SwissGroupPoolStore<>;
using DefaultOrgan = lkc::SwissTableOrgan;
using SwissHull    = lkc::ObservableComposedContainer<DefaultOrgan>;

static_assert(lkc::SwissGroupPool<DefaultStore>);
...
static_assert(requires(SwissHull const& hull) { hull.store_allocator_statistics(); });
...
struct PoolFlipComposition {
    ...
    static constexpr std::string_view paper_id = "S7-10 SwissTable pool allocator DEG";
    static constexpr std::string_view name     = "S710SwissPoolComposition";
};
...
TEST(S710SwissPoolAllocatorDeg, DirectPoolStatsAreReal) {
    SwissHull              hull;
...
TEST(S710SwissPoolAllocatorDeg, AbiObserverRoutesPoolAllocatorStatsToT6) {
    using C       = PoolFlipComposition<lk::SwissTableSearchAlgo>;
    ...
    std::cout << "S7-10 T6-DEG alloc_cnt=" << snap.axis_stats[6][2] << " bytes_alloc=" << snap.axis_stats[6][0]
...
TEST(S710SwissPoolAllocatorDeg, DefaultStoreRoundtripIsUnchanged) {
    DefaultOrgan           organ;
...
TEST(S710SwissPoolAllocatorDeg, NeutralityGuardsStayIntact) {
