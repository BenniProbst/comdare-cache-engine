// (X) BUILD-VERIFIKATION: der per-Achsen-Timer auf ALLE 19 SearchAlgorithm-Achsen ausgeweitet
// (IMeasurableWorkloadV3::run_workload_segmented_v2 → ComdareSegmentLatencyV2.seg_ns[19]) + Layout-Fix
...
//
// Emittiert die 19-Spalten-CSV via die ECHTEN Writer (lazy_csv_header/format_csv_row, ComdareSegmentLatencyV2)
// nach build/thesis_tiere/all19_pilot.csv und prüft literal:
//   (1) ALLE 19 seg_*_ns > 0 (kein n/a, kein 0) bei jeder Komposition;
...
static an::ComdareSegmentLatencyV2 measure19(char const* name, std::string& csv_out) {
    ...
    for (int i = 0; i < 19; ++i) std::cout << seg.seg_ns[i] << (i < 18 ? "," : "");
...
int main() {
    ...
    std::cout << "==== 19-Segment-Timer + Layout-Fix: "
