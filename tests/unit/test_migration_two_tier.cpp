...
//   (d) keine Observer-Doppelzaehlung: axis_stats[13] (migration, seit INC-2c) vor == nach migrate (Migration NICHT im Observer-
...
static std::uint64_t mig_row_sum(an::ComdareTierObserverSnapshot const& s) {
    std::uint64_t v = 0;
    for (std::size_t f = 0; f < an::kV3FieldCount; ++f) v += s.axis_stats[13][f];
    return v;
}
...
int main() {
    ...
    // ──────────────────────────────────────────────────────────────────────────────────────────────────────────
    {
        ...
        if (obs) {
            ...
            tr("None-Pin: Observer T14 tier_moves-Feld (r[4]) == 0", snap.axis_stats[13][4] == 0);
        }
    }
    ...
    // ──────────────────────────────────────────────────────────────────────────────────────────────────────────
    // (d) keine Observer-Doppelzaehlung: axis_stats[13] vor == nach migrate (Migration NICHT im Observer-Pfad)
    // ──────────────────────────────────────────────────────────────────────────────────────────────────────────
    {
        using Anatomy = an::SearchAlgorithmAnatomy<MigHotColdComposition>;
        an::SearchAlgorithmAbiAdapter<Anatomy> tier;
        ...
        tr("Observer: T14 tier_moves-Feld (r[4]) bleibt im Observer-Scan 0 (decide-only)",
           before.axis_stats[13][4] == 0 && after.axis_stats[13][4] == 0);
    }
