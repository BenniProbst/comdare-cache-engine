...
static std::uint64_t mig_row_sum(an::ComdareTierObserverSnapshot const& s) {
    std::uint64_t v = 0; for (std::size_t f = 0; f < an::kV3FieldCount; ++f) v += s.axis_stats[15][f]; return v;
}
...
int main() {
    ...
    // ──────────────────────────────────────────────────────────────────────────────────────────────────────────
    {
        ...
        if (!mgr || !drv) { std::cout << "  ABBRUCH (Interface fehlt)\n"; return 1; }
    ...
    // ──────────────────────────────────────────────────────────────────────────────────────────────────────────
    {
        ...
        if (!mgr || !drv) { std::cout << "  ABBRUCH None (Interface fehlt)\n"; return 1; }
    ...
    // ──────────────────────────────────────────────────────────────────────────────────────────────────────────
    {
        ...
        if (!mgr || !drv || !rbk) { std::cout << "  ABBRUCH Memento (Interface fehlt)\n"; return 1; }
        ...
        for (std::uint64_t i = 0; i < kN; ++i) {
            std::uint64_t v = 0;
            if (!drv->tier_lookup(i, &v) || v != i * 7u + 1u) { all_back = false; break; }
        }
    ...
    // ──────────────────────────────────────────────────────────────────────────────────────────────────────────
    {
        ...
        auto* mgr  = dynamic_cast<an::IMigratableTier*>(base);
        auto* obs  = dynamic_cast<an::IObservableTier*>(base);
        if (!mgr || !drv || !obs) { std::cout << "  ABBRUCH Observer (Interface fehlt)\n"; return 1; }
        ...
        tr("Observer: T15 NICHT akkumuliert ueber den Move (after <= before, idempotenter Scan)", after_mig <= before_mig);
