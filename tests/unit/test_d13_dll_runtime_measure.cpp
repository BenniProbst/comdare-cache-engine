...
struct DllTierBridge final : ana::IResourceControllableTier {
    ...
    void tier_observe(ana::ComdareTierObserverSnapshot* o) const noexcept { obs->tier_observe(o); }   // I1: die EINE Observer-Methode
...
int main(int argc, char** argv) {
    if (argc < 2) { std::cerr << "usage: test_d13_dll_runtime_measure <searchalgo_adhoc.dll>\n"; return 2; }
    ...
    if (obs == nullptr || rc == nullptr) { std::cout << "  (DLL ohne L-MEAS-Steuer-Ebene → Abbruch)\n"; return 1; }
    ...
    for (auto const& p : pts) if (p.repeat_index < 3) ++rep_counts[p.repeat_index];
    tr("je 3 Punkte mit repeat_index 0/1/2 (KF-10 separat, kein Reload)", rep_counts[0] == 3 && rep_counts[1] == 3 && rep_counts[2] == 3);
    tr("tier_size() > 0 über DLL (Workload-Daten persistieren — kein Reload zwischen Einstellungen)", obs->tier_size() > 0);
