...
static an::ComdareTierObserverSnapshot measure(char const* name) {
    ...
    if (!drv) { tr(std::string(name) + ": IObservableTier via dynamic_cast vorhanden", false); return u; }
    ...
    bool corr = true;
    for (int t = 0; t < 19; ++t) {
        std::uint64_t rs = 0; for (int f = 0; f < 8; ++f) rs += u.axis_stats[t][f];
        if (rs > 0 && u.seg_ns[t] <= 0) corr = false;
    }
    tr(std::string(name) + ": Korrelation (Observer>0 ⇒ seg_ns>0)", corr);

    std::int64_t total = 0; for (int t = 0; t < 19; ++t) total += u.seg_ns[t];
    std::cout << "    " << name << " seg_ns[T0,T4,T5,T7,T16] = "
              << u.seg_ns[0] << "," << u.seg_ns[4] << "," << u.seg_ns[5] << ","
              << u.seg_ns[7] << "," << u.seg_ns[16] << "  total=" << total << "\n";
    return u;
}
...
static void measure_unified(char const* name) {
    ...
    for (std::uint64_t i = 0; i < kN; ++i) { (void)ad->tier_insert(i, i * 7u + 1u); (void)bd->tier_insert(i, i * 7u + 1u); }
    ...
    bool stats_eq = (sa.observable_axis_count == sb.observable_axis_count && sa.tier_fill_level == sb.tier_fill_level && sa.filled_axis_count == sb.filled_axis_count);
    for (int t = 0; t < 19 && stats_eq; ++t) for (int f = 0; f < 8; ++f) if (sa.axis_stats[t][f] != sb.axis_stats[t][f]) { stats_eq = false; break; }
    tr(std::string(name) + ": unified axis_stats deterministisch (Q1: kein Timing-Inflate)", stats_eq);
    bool seg_pos = true; for (int t = 0; t < 19; ++t) if (sa.seg_ns[t] <= 0) seg_pos = false;
