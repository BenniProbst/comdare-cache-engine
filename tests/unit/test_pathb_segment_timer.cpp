...
static an::ComdareTierObserverSnapshot measure(char const* name) {
    ...
    tr(std::string(name) + ": alle " + std::to_string(an::kV3AxisCount) + " seg_ns > 0" +
    ...
    for (std::size_t t = 0; t < an::kV3AxisCount; ++t) {
        std::uint64_t rs = 0;
        for (std::size_t f = 0; f < an::kV3FieldCount; ++f) rs += u.axis_stats[t][f];
        if (rs > 0 && u.seg_ns[t] <= 0) corr = false;
    }
    tr(std::string(name) + ": Korrelation (Observer>0 ⇒ seg_ns>0)", corr);

    std::int64_t total = 0;
    for (std::size_t t = 0; t < an::kV3AxisCount; ++t) total += u.seg_ns[t];
...
static void measure_unified(char const* name) {
    ...
    for (std::size_t t = 0; t < an::kV3AxisCount && stats_eq; ++t)
    ...
    tr(std::string(name) + ": unified seg_ns[0.." + std::to_string(an::kV3AxisCount - 1) +
           "] alle > 0 (Pfad-B-Timing im EINEN POD)",
       seg_pos);
}
