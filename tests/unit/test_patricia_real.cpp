...
template <class T> concept has_insert_key = requires (T t) { t.insert_key(std::uint64_t{1}); };
template <class T> concept has_descend    = requires (T t) { { t.descend(std::uint64_t{1}) } -> std::convertible_to<bool>; };
...
static void test_real_patricia() {
    ...
    // (a) leerer Trie: kein Key dereferenzierbar.
    {
        ...
        for (std::uint64_t i = 0; i < N; ++i) if (organ.descend(ins_key(i))) { empty_none = false; break; }
    ...
    for (std::uint64_t i = 0; i < N; ++i) {
        if (organ.descend(ins_key(i))) { ++hits; if (organ.real_trie().last_descent_depth() > max_depth) max_depth = organ.real_trie().last_descent_depth(); }
    }
    ...
    for (std::uint64_t i = 0; i < N; ++i) if (organ.descend(miss_key(i))) ++miss_hits;
    ...
    organ.clear_trie();
    tr("Patricia: (b) clear_trie() == leerer Default-Trie (bit-exakt)", organ == pc::ObservablePathCompression<pc::PatriciaPathCompression>{});
...
static void test_none_noop() {
    ...
    static_assert(has_insert_key<pc::real_trie_t<pc::PatriciaPathCompression>> && has_descend<pc::real_trie_t<pc::PatriciaPathCompression>>,
    ...
    tr("(b) None: compress() unveraendert verfuegbar (Pfad-A messneutral, kein Trie-Effekt)", organ.real_trie().key_count() == 0);
}
...
static void test_tier_memento_patricia() {
    ...
    for (std::uint64_t i = 0; i < kBase; ++i) if (!tier.path_compression_instance().real_trie().descend(ins_key(i))) { base_all_deref = false; break; }
    tr("Patricia: (c) Pre-Save: alle Original-Keys per Descent gefunden (echte Struktur)", base_all_deref);
    std::cout << "     Pre-Save key_count=" << pc_before.real_trie().key_count() << " node_count=" << pc_before.real_trie().node_count() << "\n";

    rbk->tier_save_all();                                   // Warmup-Vor-Zustand kapseln (eager pc-Snapshot)
    for (std::uint64_t i = kBase; i < kBase + kWarm; ++i) (void)drv->tier_insert(ins_key(i), i * 11u + 5u);   // Warmup-Mutation
    // nach dem Warmup-Insert MUSS der Trie die Warmup-Keys kennen (REAL mutiert, kein no-op).
    bool warm_in_pc = true;
    for (std::uint64_t i = kBase; i < kBase + kWarm; ++i) if (!tier.path_compression_instance().real_trie().descend(ins_key(i))) { warm_in_pc = false; break; }
    tr("Patricia: (c) nach Warmup-Insert: Warmup-Keys im REALEN Trie (mutiert, kein no-op)", warm_in_pc);
    tr("Patricia: (c) nach Warmup-Insert: Trie != Pre-Save-Stand (echte Mutation)", !(tier.path_compression_instance() == pc_copy_before));
    ...
    for (std::uint64_t i = 0; i < kBase; ++i) if (!tier.path_compression_instance().real_trie().descend(ins_key(i))) { base_still_deref = false; break; }
    ...
    for (std::uint64_t i = kBase; i < kBase + kWarm; ++i) if (!tier.path_compression_instance().real_trie().descend(ins_key(i))) ++warm_back_gone;
    std::cout << "     Warmup-Keys nach Rollback wieder weg: " << warm_back_gone << "/" << kWarm << "\n";
    tr("Patricia: (c) Memento: Warmup-Keys nach Rollback NICHT mehr per Descent gefunden (kein Leck)", warm_back_gone == kWarm);
}
...
static void test_tier_memento_none() {
    ...
    auto* drv  = dynamic_cast<an::IDriveableTier*>(base);
    auto* rbk  = dynamic_cast<an::IRollbackableTier*>(base);
    if (!drv || !rbk) { tr("(c)-None: Interface vorhanden", false); return; }
    ...
    tr("(c)-None: path_compression-Trie bleibt leer (key_count==0, No-Op)", tier.path_compression_instance().real_trie().key_count() == 0);
    tr("(c)-None: Memento des leeren None-Trie ist BIT-EXAKT (operator==)", tier.path_compression_instance() == pc_copy);
}
...
static void test_patricia_fuzz_realloc() {
    ...
    for (std::uint64_t k : truth) if (organ.descend(k)) ++found;
    tr("(e) Fuzz: ALLE distinct eingefuegten Keys gefunden (kein Crash/Verlust ueber Reallocations)", found == truth.size());
    ...
    std::uint64_t fp = 0;
    for (std::uint64_t i = 0; i < 5000; ++i) { std::uint64_t const m = miss_key(i); if (!truth.count(m) && organ.descend(m)) ++fp; }
    tr("(e) Fuzz: garantiert fremde Keys NICHT gefunden (0 False-Positive bei exaktem Trie)", fp == 0);
    std::cout << "     fuzz: inserted=" << N << " distinct=" << truth.size()
              << " found=" << found << " node_count=" << organ.real_trie().node_count() << "\n";
}

int main() {
    std::cout << "==== §4.3: REALER path_compression(Patricia) (materialisierter crit-bit-Trie + echter Descent) ====\n";
