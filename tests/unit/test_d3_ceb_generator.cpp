...
struct FakeComp {
    ...
    using mapping            = int;
    using path_compression   = int;
    using node_type          = int;
    using memory_layout      = int;
    using allocator          = int;
    using prefetch           = int;
    ...
    using io_dispatch        = int;
    using migration_policy   = int;
    using filter             = int;
...
struct FakeEngine {
    template <class F>
    static void for_each_composition_type(F&& f) {
        f.template operator()<FakeComp>();
    }
};
...
int main() {
    ...
    check(!has(shell, "COMDARE_DEFINE_ANATOMY_MODULE_ADHOC"),
          "Diagnose-Shell: KEIN ADHOC-Makro (keine reale Anatomie)");
    ...
    check(has(real, "COMDARE_DEFINE_ANATOMY_MODULE_ADHOC"),
          "reales Modul: COMDARE_DEFINE_ANATOMY_MODULE_ADHOC (reale Anatomie)");
