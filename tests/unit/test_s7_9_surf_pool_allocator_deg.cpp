// S7-9 (#261/#234) -- SuRF-Pool allocator parameter + T6 DEG hook.
...
using DefaultStore = lkc::SurfFstMapPoolStore<>;
using DefaultOrgan = lkc::SurfMapOrgan;
...
struct PoolFlipComposition {
    ...
    static constexpr std::string_view paper_id = "S7-9 SuRF pool allocator DEG";
    static constexpr std::string_view name     = "S79SurfPoolComposition";
};
...
TEST(S79SurfPoolAllocatorDeg, DirectPoolStatsAreReal) {
    SurfHull               hull;
...
TEST(S79SurfPoolAllocatorDeg, AbiObserverRoutesPoolAllocatorStatsToT6) {
    using C       = PoolFlipComposition<lk::OriginalSurfSearchAlgo>;
    ...
    std::cout << "S7-9 T6-DEG alloc_cnt=" << snap.axis_stats[6][2] << " bytes_alloc=" << snap.axis_stats[6][0]
...
TEST(S79SurfPoolAllocatorDeg, DefaultStoreRoundtripIsUnchanged) {
    DefaultOrgan           organ;
...
TEST(S79SurfPoolAllocatorDeg, NeutralityGuardsStayIntact) {
