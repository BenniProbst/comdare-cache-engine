...
int main() {
    ...
    std::size_t           leaves = 0, matched = 0;
    std::set<std::string> leaf_paths;
    tree.for_each_binary([&](std::string const& bin, std::string const&, ex::TreeNode const&) {
        ++leaves;
        leaf_paths.insert(bin);
        if (ex::CompositionRecord const* r = reg.lookup(bin)) {
            if (r->path == bin) ++matched;
        }
    });
    ...
    reg.for_each([&](ex::CompositionRecord const& r) {
        if (!leaf_paths.contains(r.path)) all_reg_in_tree = false;
    });
