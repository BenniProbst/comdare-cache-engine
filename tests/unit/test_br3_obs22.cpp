...
int main() {
    ...
    for (auto const& l : lv) {
        ...
        if (found) {
            ++classified;
            if (k == ex::AxisObserverKind::SearchAlgorithmObserver) ++sa;
            else if (k == ex::AxisObserverKind::DefinitionOnly)     ++def_only;
            else                                                    ++cont;
        }
        if (!l.values.empty()) ++with_def;   // read-only Definition (reale Wrapper-Namen) vorhanden
        std::cout << "    " << (found ? "" : "[UNKLASSIFIZIERT] ") << l.axis << " : "
                  << (found ? std::string{ex::observer_kind_name(k)} : std::string{"?"})
                  << "  (" << l.values.size() << " Wrapper)\n";
    }
