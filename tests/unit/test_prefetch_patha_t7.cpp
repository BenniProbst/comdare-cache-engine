...
static an::ComdareSegmentLatencyV2 measure_patha(char const* name) {
    ...
    if (!v3) { tr(std::string(name) + ": IMeasurableWorkloadV3 (run_workload_segmented_v2) vorhanden", false); return out; }
    // 3 Batches a 4096 Ops + Warmup (intern) → seg_ns aufsummiert; genug Arbeit, dass jeder steady_clock-Tick > 0 ist.
    std::uint64_t const got = v3->run_workload_segmented_v2(/*ops_per_batch=*/4096, /*batches=*/3, /*seed=*/0xABCDEFull, &out);
    tr(std::string(name) + ": run_workload_segmented_v2 lieferte batches_measured>0", got > 0 && out.batches_measured > 0);
    ...
    bool others_nonneg = true; int neg_t = -1;
    for (int t = 0; t < 19; ++t) if (t != 7 && out.seg_ns[t] < 0) { others_nonneg = false; if (neg_t < 0) neg_t = t; }
    tr(std::string(name) + ": die anderen 18 Segmente unberuehrt (kein < 0 durch T7-Re-Grounding)" + (others_nonneg ? "" : (" (neg bei T" + std::to_string(neg_t) + ")")), others_nonneg);

    std::cout << "    " << name << " seg_ns[T6,T7,T8] = " << out.seg_ns[6] << "," << out.seg_ns[7] << "," << out.seg_ns[8]
              << "  total=" << out.total_ns << "  batches=" << out.batches_measured << "\n";
    return out;
}
...
static void test_t7_real_store_addresses() {
    ...
    T7Store store;
    for (std::size_t i = 0; i < 4096; ++i) store.append_slot(static_cast<std::uint64_t>(i) * 2654435761ull + 1u, i * 7u + 1u);
    tr("(C) lokaler T7-Store hat reale Slots + Backing", store.slot_count() == 4096 && store.slot_address(0) != nullptr);
    ...
    tr("(C) hardware/distance/path: > 0 reale _mm_prefetch auf echte Store-Adressen (T7 re-grounded)", ih > 0 && id > 0 && ip > 0);
...
int main() {
    ...
    (void)sn; (void)sh; (void)sd; (void)sp;
