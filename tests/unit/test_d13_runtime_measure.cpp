...
struct MockTier final : ana::IObservableTier, ana::IResourceControllableTier {
    ...
    [[nodiscard]] bool tier_lookup(std::uint64_t k, std::uint64_t* o) const noexcept override {
        ++lookups; auto it = data.find(k); if (it != data.end()) { if (o) *o = it->second; return true; } return false; }
...
int main() {
    ...
    tr("Label enthaelt 'thread_count='", !pts.empty() && pts.front().setting_label.find("thread_count=") != std::string::npos);
    ...
    tr("je 3 Punkte mit repeat_index 0/1/2 (KF-10 separat)", rep_counts[0] == 3 && rep_counts[1] == 3 && rep_counts[2] == 3);
