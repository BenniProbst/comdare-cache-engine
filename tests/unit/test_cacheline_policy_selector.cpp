...
static void dump(char const* tag, an::ComdareResourceControlV1 const& r) {
    std::cout << "    " << tag
              << ": thread=" << r.thread_count
              << " prefetch=" << r.prefetch_distance
              << " pool=" << r.pool_budget_bytes
              << " batch=" << r.batch_size
              << " inline=" << r.inline_threshold_bytes << "\n";
}
...
public:
    ...
    std::uint64_t tier_apply_resource_control(an::ComdareResourceControlV1 const* in) noexcept override {
        ...
        auto a1 = [&](std::uint64_t v, std::uint64_t cap, std::uint64_t& dst) {
            if (v != 0) { dst = (cap != 0 && v > cap) ? cap : v; ++n; }
        };
...
int main(int argc, char** argv) {
    ...
    std::cout << "  scan_share(scan)=" << prof_scan.scan_share()
              << "  scan_share(point)=" << prof_point.scan_share() << "\n";
    ...
    check("applied scan != applied point (geklammert distinkt)",
          !(tier_scan.applied == tier_point.applied));
    ...
    auto prof_mixed = cp::WorkloadProfileAggregate::from_shares(
        0.30, 0.40, 0.0, 0.30, 0.0, 100000, 65536);  // scan_share=0.30 -> "gemischt"
    ...
    std::filesystem::path csv = (argc > 1)
        ? std::filesystem::path(argv[1])
        : std::filesystem::path(
              "C:/Users/benja/OneDrive/Desktop/Diplomarbeit - Datenbanken/Code/external/"
    ...
    if (rows < 0) {
        ...
        for (auto const& [algo, agg] : by_algo) {
            ...
            std::cout << "    search_algo=" << algo
                      << "  scan_share=" << agg.scan_share()
                      << "  ws=" << agg.working_set_n
                      << "  -> prefetch=" << d.prefetch_distance
                      << " batch=" << d.batch_size << "\n";
        }
