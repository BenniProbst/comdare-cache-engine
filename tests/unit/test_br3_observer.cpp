...
int main() {
    ...
    check_true("observable_axis_count >= 1 (R5.B: mind. search_algo real)",
               measured.observer.observable_axis_count >= 1);
    ...
    ex::push_static_axis<L0>(lv, "search_algo");
    ex::push_static_axis<L1>(lv, "cache_traversal");
    ex::push_static_axis<L2>(lv, "mapping");
    ex::push_static_axis<L3>(lv, "path_compression");
    ex::push_static_axis<L4>(lv, "node_type");
    ex::push_static_axis<L5>(lv, "memory_layout");
    ex::push_static_axis<L6>(lv, "allocator");
    ...
    check_eq("ungemessen: search_insert_count == 0 (Default)", tree.node_value(path).observer.search_insert_count,
             std::uint64_t{0});
    ...
    check_eq("Knoten-Snapshot == gemessener insert_count", read.observer.search_insert_count,
    ...
    if (rec)
        check_eq("Definition trägt alle 17 Achsen (achse,wrapper; INC-2d)", rec->definition.size(), std::size_t{17});
