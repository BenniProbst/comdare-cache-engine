// S7-7 (#261/#234) -- START-Pool allocator parameter + T6 DEG hook.
...
using DefaultStore = lkc::StartTrieNodePoolStore<>;
using DefaultOrgan = lkc::StartTrieOrgan;
using StartHull    = lkc::ObservableComposedContainer<DefaultOrgan>;

static_assert(lkc::StartTrieNodePool<DefaultStore>);
...
static_assert(requires(StartHull const& hull) { hull.store_allocator_statistics(); });
...
struct PoolFlipComposition {
    ...
    static constexpr std::string_view paper_id = "S7-7 START pool allocator DEG";
    static constexpr std::string_view name     = "S77StartPoolComposition";
};
...
TEST(S77StartPoolAllocatorDeg, DirectPoolStatsAreReal) {
    StartHull              hull;
    std::vector<U64> const keys{
        0x0001u, 0x0101u, 0x0201u, 0x0301u, 0x0401u, 0x0501u, 0x0601u, 0x0701u,
        0x1002u, 0x2002u, 0x3002u, 0x4002u, 0x5002u, 0x6002u, 0x7002u,
    };
...
TEST(S77StartPoolAllocatorDeg, AbiObserverRoutesPoolAllocatorStatsToT6) {
    using C       = PoolFlipComposition<lk::OriginalStartSearchAlgo>;
    ...
    std::vector<U64> const keys{
        0x0001u, 0x0101u, 0x0201u, 0x0301u, 0x0401u, 0x0501u, 0x0601u, 0x0701u,
        0x0801u, 0x0901u, 0x0A01u, 0x0B01u, 0x0C01u, 0x0D01u, 0x0E01u,
    };
    ...
    std::cout << "S7-7 T6-DEG alloc_cnt=" << snap.axis_stats[6][2] << " bytes_alloc=" << snap.axis_stats[6][0]
...
TEST(S77StartPoolAllocatorDeg, DefaultStoreRoundtripIsUnchanged) {
    DefaultOrgan           organ;
    std::vector<U64> const keys{5u, 0x0105u, 0x0205u, 1u, 3u, 0x0305u, 9u, 4u, 6u, 0x0405u};
...
TEST(S77StartPoolAllocatorDeg, NeutralityGuardsStayIntact) {
