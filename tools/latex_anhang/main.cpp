...
[[nodiscard]] int write_latex(std::string const& path,
                                std::vector<CsvRow> const& rows,
                                std::string const& caption_text,
    ...
    for (auto const& r : rows) {
        ...
        out << escape_latex(r.permutation_id)
            << " & " << r.op_count
            << " & " << r.total_cycles
            << " & " << r.cache_misses_l1
            << " & " << r.cache_misses_l2
...
[[nodiscard]] int write_latex_full_metrics(std::string const& path,
                                            std::vector<CsvRow> const& rows,
                                            std::string const& caption_text,
                                            std::string const& label) {
    std::ofstream out{path};
...
void print_help() {
    std::cout
        << "latex_anhang - Konvertiert measurements.csv zu LaTeX-Tabelle\n\n"
...
int main(int argc, char const* const* argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (k == "--input")    input_path   = std::string(v);
        else if (k == "--output")   output_path  = std::string(v);
        else if (k == "--caption")  caption      = std::string(v);
        else if (k == "--label")    label        = std::string(v);
