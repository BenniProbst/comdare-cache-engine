...
#include "ycsb_cli.hpp" // die EINE Deklarationsstelle (auch fuer comdare_ycsb_cli_lib + den Test)
...
[[nodiscard]] int parse_key_dist(std::string_view s, wg::KeyDistribution& out) noexcept {
    if (s == "uniform") {
        out = wg::KeyDistribution::Uniform;
        return 0;
    }
    if (s == "zipfian") {
        out = wg::KeyDistribution::Zipfian;
        return 0;
    }
    if (s == "sequential") {
        out = wg::KeyDistribution::Sequential;
        return 0;
    }
    if (s == "latest") {
        out = wg::KeyDistribution::Latest;
        return 0;
    }
    return 4;
}
...
[[nodiscard]] int parse_args(int argc, char const* const* argv, CliConfig& cfg) noexcept {
    for (int i = 1; i < argc; ++i) {
        ...
        try {
            if (key == "--workload") {
                if (parse_workload(val, cfg.workload) != 0) return 4;
            } else if (key == "--num-keys") {
                cfg.num_keys = std::stoull(std::string(val));
            } else if (key == "--num-ops") {
                cfg.num_ops = std::stoull(std::string(val));
            } else if (key == "--key-size") {
                cfg.key_size = static_cast<std::uint32_t>(std::stoul(std::string(val)));
            } else if (key == "--value-size") {
                cfg.value_size = static_cast<std::uint32_t>(std::stoul(std::string(val)));
            } else if (key == "--key-dist") {
                if (parse_key_dist(val, cfg.key_dist) != 0) return 4;
            } else if (key == "--zipfian-theta") {
                cfg.zipfian_theta = std::stod(std::string(val));
            } else if (key == "--seed") {
                cfg.seed = std::stoull(std::string(val));
            } else if (key == "--output") {
                cfg.output = std::filesystem::path{std::string(val)};
            } else if (key == "--format") {
                if (parse_format(val, cfg.format) != 0) return 4;
            } else
                return 4;
        } catch (std::exception const&) { return 4; }
    }
    return 0;
}
...
[[nodiscard]] int write_tsv(std::filesystem::path const& p, std::span<wg::Operation const> ops) noexcept {
    ...
    for (auto const& op : ops) { out << op_kind_name(op.op) << '\t' << op.key_id << '\t' << op.scan_length << '\n'; }
    return out.good() ? 0 : 10;
}
...
[[nodiscard]] int write_json(std::filesystem::path const& p, std::span<wg::Operation const> ops) noexcept {
    ...
    for (auto const& op : ops) {
        if (!first) out << ",\n";
        first = false;
        out << "    {\"op\": \"" << op_kind_name(op.op) << "\", \"key\": " << op.key_id
...
void print_help() {
    std::cout << "ycsb_cli - YCSB-Generator-CLI fuer comdare-cache-engine\n\n"
