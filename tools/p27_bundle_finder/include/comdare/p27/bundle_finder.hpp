...
public:
    void add_function(std::uint64_t addr, std::string name) {
        ...
        if (!call_graph_.contains(addr)) { call_graph_[addr] = {}; }
    }

    void add_call(std::uint64_t from, std::uint64_t to) { call_graph_[from].insert(to); }

    [[nodiscard]] const std::map<std::uint64_t, std::set<std::uint64_t>>& edges() const noexcept { return call_graph_; }

    [[nodiscard]] const std::map<std::uint64_t, std::string>& names() const noexcept { return addr_name_; }
...
std::map<std::uint64_t, SubtreeMetric> compute_subtree_metrics(const CallGraph&                              graph,
...
std::vector<BundleCandidate> find_bundle_candidates(const CallGraph&                              graph,
                                                    ...
                                                    std::uint64_t threshold_bytes, bool demangle = false);
