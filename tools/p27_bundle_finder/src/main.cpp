...
std::map<std::uint64_t, FunctionEntry> parse_symbol_table(std::string_view text) {
    std::map<std::uint64_t, FunctionEntry> result;
    std::size_t                            line_start = 0;
    while (line_start < text.size()) {
        std::size_t      line_end = text.find('\n', line_start);
        std::string_view line = (line_end == std::string_view::npos) ? text.substr(line_start)
...
CallGraph parse_disassembly(std::string_view text) {
    ...
    while (line_start < text.size()) {
        std::size_t line_end = text.find('\n', line_start);
        std::string line =
            std::string((line_end == std::string_view::npos) ? text.substr(line_start)
                                                             : text.substr(line_start, line_end - line_start));
        line_start = (line_end == std::string_view::npos) ? text.size() : line_end + 1;
...
compute_subtree_metrics(const CallGraph& graph, const std::map<std::uint64_t, std::uint64_t>& addr_size) {
    ...
    auto dfs_for_source = [&](std::uint64_t source) -> SubtreeMetric {
        ...
        while (!stack.empty()) {
            ...
            for (std::uint64_t child : it->second) { stack.push_back(child); }
        }
        return m;
    };

    for (const auto& [node, _] : edges) { result.emplace(node, dfs_for_source(node)); }
    return result;
}

std::vector<BundleCandidate> find_bundle_candidates(const CallGraph&                              graph,
                                                    const std::map<std::uint64_t, FunctionEntry>& symbols,
                                                    std::uint64_t threshold_bytes, bool /*demangle*/) {
    ...
    std::sort(sorted.begin(), sorted.end(),
              [](const auto& a, const auto& b) { return a.second.footprint_bytes > b.second.footprint_bytes; });
    ...
    for (const auto& [addr, m] : sorted) {
        ...
        c.demangled_name =
            (name != graph.names().end()) ? name->second : (sym != symbols.end() ? sym->second.name : "?");
...
int main(int argc, char** argv) {
    ...
    std::uint64_t     threshold  = (argc == 4) ? std::strtoull(argv[3], nullptr, 0) : 240'000ull;
    ...
    for (const auto& c : candidates) {
        std::printf("\n0x%llx 0x%llx [0x%llx]: %-70s subtree=(%llu, %llu) ", static_cast<unsigned long long>(c.address),
                    static_cast<unsigned long long>(c.size), static_cast<unsigned long long>(c.task_id),
                    c.demangled_name.c_str(), static_cast<unsigned long long>(c.subtree.function_count),
