...
| K4 | tier_scan No-Op | store-traversierbare Tiere scannen über container_ | `f6152ac` | anatomy_observer 18/18 |
...
## 4. M8 (Befund-2-Weg-A) — search_organ_-Driving für store-traversierbare Tiere eliminiert ✅

- **Basis (`8aee2b8`):** store-traversierbare LinearScan-In-Process-Verifikation `tests/unit/test_m8_storetrav_segment.cpp`
  (belegt A2.5 für store-traversierbare Tiere) + **realer seg_ns-Fix** (Key-Ernte aus container_ statt search_organ_ →
  keine nk=1-Degeneration für Nicht-MementoAxis-Tiere wie LinearScan).
- **Rest (`a39c62e`):** 6 Daten-/Timing-Sites in `abi_adapter.hpp` via `if constexpr StoreTraversableSearchAlgo` auf
  ...
  behebt nebenbei K9-(d)-uint16-Truncation für diese Tiere. Weg-B (Trie/Pool) unverändert.
...
1. **K9-b LayoutAwareChunkedStore-Compile-Bug:** A2.3 machte `Chunk` zum Struct `{data,used,capacity}`, aber 7
   ...
   kompilierte nicht für store-traversierbare Tiere → hätte A2.8 blockiert. Fix: `c.data`/`c.used` (`ae31b63`).
...
- **Index-320-Voll-Matrix (SUPPLEMENTÄR, läuft):** `-MaxBinaries 320 -SelectMode index`, Task `b3i763gc1`. Stand 14.06
  ...
  schwankend + zunehmend langsamer (10:38→15:43: ~16 min/Tier; 15:43→21:45: ~52 min/Tier — die Index-Mode-Kompositionen
  sind im Schnitt schwerer als die k_ary-F15-Tiere). Hochrechnung daher unsicher, potenziell **mehrere Tage**.
