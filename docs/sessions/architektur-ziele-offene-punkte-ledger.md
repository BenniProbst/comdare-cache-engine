...
|----|-------|------|-----------------|---------|------------|------------------|
| **R8** (=R8.prt-art / F.6-Phase-B+C / Task #8,#31) | prt-art als Pruefling einbinden (Stufe 1/2/3) + optional_prt_art_impl-Slotfüllung | large-multi-session | **open** (Foundation gelegt, Code-Integration fehlt) | Doku 19 Reaudit: 44 Header klassifiziert (10 already_covered→löschen, 14 basis_missing→migrieren, 11 spezifisch→Slot, 9 deprecated→löschen). Slots existieren physisch aber leer (verifiziert: nur 2 `optional_prt_art_impl`-Vorkommen im Topics-Tree). Tasks #8/#31 pending | Doku 19/21/23 §5; Code/external/comdare-prt-art | Phase A: 14 basis_missing migrieren. Phase B: 11 spezifische in optional_prt_art_impl-Slots. Phase C: 18 (9 deprec.+ ggf. already_covered) löschen |
| **R7.1.b–d** (Task #27) | Goldstandard-Restachsen: axis_08-Concurrency-Test + fehlende is_original-Header (axis_03a ART/HOT/START, axis_14, axis_q1) | large-multi-session | **in-progress** | 6 is_original-Header erzeugt (mimalloc/jemalloc/snmalloc/rpmalloc/lrmalloc/dlmalloc in build/generated). axis_08 nur 8 Wrapper, KEINE volle Test-Coverage. axis_03a-Original-Codes ohne is_original-Header | Task #27 pending; build/.../legacy_code/paper_a*_is_original.hpp | CMake-Codegen für alle Klasse-A-Wrapper; axis_08_concurrency_olc Variante + Test; Introspection auf weitere Achsen |
