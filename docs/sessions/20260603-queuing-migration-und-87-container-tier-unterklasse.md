# Session 2026-06-03 — queuing-Migration 17→19 (#88/#89 fertig) + #87 Container-Lebewesen-Unterklasse (in Arbeit)
...
Ebene 2 — LEBEWESEN-UNTERKLASSE = fester Achsen-Satz UNTER dem Interface (eine konkrete „Lebewesen-Art")
Ebene 3 — ACHSEN           = Organe (Sub-Aufgaben). ALLE PFLICHT + UNIFORM getrieben.
                              KEINE Achse ist optional. Eine nicht-puffernde Lebewesen-Unterklasse wählt
...
**Aktueller Bau-Stand:** Es ist **nur EINE Lebewesen-Unterklasse** real gebaut — die der **SearchAlgorithm**-Gattung.
...
- Jeder Lebewesen-Binary nutzt **durchgängig dieselben** Achsen-Interfaces wie alle anderen (uniform).
...
- **Explizit je Lebewesen** (kein Auto-Anhängen): jede Lebewesen-Quelle endet jetzt mit `…, NoBuffer, LazyFlush)`.
...
- 10 Lebewesen-Quellen + 11 benannte Compositions (art/hot/masstree/start/surf/wormhole +_paper_binding) je +queuing_q1/q2.
...
- Mess-Pfad: 8/8 Lebewesen, 19 Achsen, exit 0 (`obs_axes` 4→6).
...
+ über alle Architektur-Dokumente ab Dok 10 (26-Agenten-Konsistenz-Sweep: queuing≠Gattung + Lebewesen-Unterklasse-Terminologie).
Doku 14 §25 verbatim erhalten + 7 Schärfungs-Notizen. Memory: `project_axis03a_monolith_violation_delegation_fix`.

---

## 4. #87+#90 — Container-GATTUNG + Lebewesen-Unterklassen (WIEDERAUFSATZPUNKT, korr. 2026-06-03)

> ⚠️ **ZWEI frühere Designs in dieser Doku waren FALSCH (User-Korrekturen):**
> - „inner+ordering als Container-Gattung" — Ebenen-Fehler: Container ist die **Gattung**, Adapter eine **Lebewesen-Unterklasse** darunter.
...
**Bereits erledigt (verifiziert):** `anatomy_base.hpp` — `AnatomyGattung{SearchAlgorithm,Container,Graph}` + `gattung_of()` + `gattung_name()` eingeführt (nicht-brechend, kompiliert via test_v41_anatomy_base). AnatomyGenus als Lebewesen-Unterklasse re-dokumentiert.
...
- **#90 (User-Freigabe nötig):** codebase-weite `AnatomyGenus`→3-Ebenen-Benennung (Gattung/Lebewesen-Unterklasse). Groß, separat.
...
# Mess-Pfad (Lebewesen): tests/unit/thesis_tiere/build_and_measure_thesis_tiere.ps1
