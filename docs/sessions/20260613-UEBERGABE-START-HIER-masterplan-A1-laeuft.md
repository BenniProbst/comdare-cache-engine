...
- **3-Ebenen (Doc 30 §8.0, AUTORITATIV):** Gattung=Außen-Interface/Prüf-Dock (SearchAlgorithm/Container/Graph) · Tier-Unterklasse=fester Achsen-Satz · **Achsen=Organe, ALLE Pflicht, in jedem Tier-Binary uniform** (NoBuffer/NonePrefetch=Durchreich-Algos). queuing q1/q2=Pflicht-SA-Achsen (17→19). Per-Gattung: SA 17(+3 Build), Adapter 13, Set 15, Sequence 11, View 7.
...
- **B+-Experiment-Baum (Doc 26/27/29 + Code):** Achsen=Baum-Ebenen, Pfad=binary_id=Tier, Mixed-Radix-Bijektion (`StaticBinaryView`), **Achsen-Austausch = `flat_index`-Ziffernwechsel (genau 1 Achse diff)**. ⇒ **Achsen-Austausch gehört IN DEN BAUM/cache-engine, NIE flach im Eval-Tool.**
