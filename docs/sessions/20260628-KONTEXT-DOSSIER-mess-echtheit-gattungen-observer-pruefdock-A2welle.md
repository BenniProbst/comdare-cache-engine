...
2. **EytzingerTraversal-Organ — ENTSCHIEDEN (User 2026-06-28): Option (b) — der Store wird ECHT Eytzinger-/BFS-arrangiert** (NICHT organ-lokaler Index; der „echte Weg", nah am Paper, cache-freundlich). **Konsequenz: 4a ist GRÖSSER als ein reines Organ** — der `LayoutAwareChunkedStore` bekommt eine **Eytzinger/BFS-Slot-Ordnungs-Variante**, und EytzingerTraversal navigiert diese physische Ordnung. **Erster 4a-Schritt = das Store-Arrangement-Design:** Eytzinger als **Slot-ORDNUNG** (nicht Record-Feld-Layout) — Abgrenzung zur memory_layout-aos/soa-Achse (#167) klären (Eytzinger ordnet die Slot-Reihenfolge, aos/soa ordnen die Feld-Anordnung INNERHALB eines Slots; ggf. orthogonal kombinierbar). **Bruch-Radius:** Slot-Anordnung + die geordnete Iteration (`scan_into`/lower_bound) müssen weiter **std::map-konform** bleiben (Konformitäts-Gate `test_conformance_gate` #223 + scan-Test `test_v41_scan_range_organ.cpp` sind die Bars). KAryTraversal (Schritt 1) bleibt organ-lokal über den sortierten Store; nur Eytzinger braucht die Arrangement-Variante.
...
## §19 #188-4a-C IMPLEMENTIERUNGS-PLAN — K-bewusstes store-traversierbares k-ary via SEPARATEM iterable-Kanal (User-Entscheid 2026-06-29)

> **Warum:** #188-4a (k-ary store-traversierbar) OHNE K-Bewusstsein wäre ein Phantom-Tausch — Weg-B-Spiegel (Speicher-Achsen) RAUS, dafür die iterierbare K∈{2,4,8,16}-Variation STUMM (fixes K=4) — gegen Meta-Lehre #3. **User-Entscheid 2026-06-29: K-bewusstes Organ JETZT bauen (Option C); K reitet einen SEPARATEN iterable-Kanal, NICHT den RC-POD** (algorithmischer Such-Parameter ≠ Resource Control → saubere Semantik-Trennung). Der User-Hinweis: dynamische Subachsen mit State sind im Permutations-B+-Baum ein gesanktioniertes Muster → ein K-State bricht die Achseneigenschaften NICHT (State wohnt im Container/Permutations-Pfad, nicht im Organ).

**Baseline (im Working-Tree, Marker AUS = kompiliert; ggf. schon committet):** `KAryTraversal`-Organ (`axes/lookup/composable/k_ary_traversal_organ.hpp`) existiert + ist CI-konformitätsbeweisbar (`test_conformance_gate` KAryComposedTier + `test_v41_scan_range_organ` KAry). FIXES K=4. Marker `axis_03a_store_traversable` NOCH AUS (k-ary = Weg-B) — Aktivierung = dieser Plan. Mapping `traversal_for_search_algo<KArySearchAlgo>=KAryTraversal` + static_assert schon gesetzt (mit Marker-aus unused-aber-bereit). Codex-Review: lookup_in korrekt (A), Concept OK (B), #211-frei (D), Test-Hülle OK (E).

**Grounded Mechanismus (verifiziert 2026-06-29):**
- **RC-Kanal** (RuntimeVariableLoop KF-7, `runtime_variable_loop.hpp`): `DynamicDim → set_field(var→ComdareResourceControlV1-Feld) → clamp → tier_apply_resource_control(&applied)`. Konsum heute = Null-Object `applied_rc_` (#221). **NICHT für K** (User: separater Kanal).
- **iterable-Kanal** (`permutation_engine.hpp:209-252`): `HasIterableAspect<V>` / `for_each_aspect<V>(visitor)` / `aspect_count<V>()` — DEFINIERT, aber im Mess-Treiber (perm_runner/iterator) **UNVERDRAHTET** (kein Aufruf; verifiziert via Grep). Die search_algo-iterable-Achse: `KArySearchAlgo` `iterable_aspect_t=unsigned`, `kIterableArities{2,4,8,16}`, `iterable_values()`, `set_iterable_aspect(arity_)` (`axis_03a_search_algo_k_ary.hpp:74-78,191`). **DAS ist der zu verdrahtende separate Kanal.**

**SOLL:** k-ary läuft Weg-A (container_ = KAryTraversal über den realen node/layout/allocator-Store) UND K∈{2,4,8,16} wird treu gemessen — über einen SEPARATEN iterable-Laufzeit-Kanal (parallel zu RC). Organ bleibt STATELESS (K = Parameter); der K-State wohnt im Container (dynamische Subachse), geliefert über den iterable-Kanal.

**Implementierungs-Schritte (ausführungsreif; je Inkrement Codex + contract:conformance-CI / #162-Harness; NIE halb committen):**
- **C1 — Organ K-parametrisieren (stateless):** `KAryTraversal::lookup_in<Store>(Store const&, key, unsigned K = kArity)` — K als Parameter mit Default 4 (bestehende Aufrufer/Tests unverändert). Additives Concept `ConfigurableTraversalOrgan` (analog `ScannableTraversalOrgan`): Organ mit `lookup_in(Store const&, key, aspect_value)`. Self-proof static_assert im Organ-File.
- **C2 — ComposedSearch K-tragend:** ComposedSearch (oder store-traversierbare Hülle) hält einen Laufzeit-`iterable_aspect_value_` (Default = Organ-kArity); `lookup` reicht ihn an `Traversal::lookup_in(store_, k, aspect_)` durch (nur falls `ConfigurableTraversalOrgan`, sonst unverändert); Setter `set_iterable_aspect(value)`. Observable-Hülle spiegelt das.
- **C3 — Separater iterable-ABI-Kanal:** NEUE additive Tier-Schnittstelle (parallel zu `tier_apply_resource_control`), z.B. `IIterableAspectTier::tier_apply_iterable_aspect(uint64 axis_id, uint64 value) noexcept` (axis_id adressiert die iterable-Achse; search_algo=0). `abi_adapter` implementiert sie → routet value an `container_.set_iterable_aspect` (k_ary). ABI-additiv (eigene Schnittstelle, `dynamic_cast` 1× kalt wie IObservableTier; KEIN RC-POD-Eingriff). Loader/Major-Konvention beachten (anatomy_module_abi).
- **C4 — iterable-Laufzeit-Loop (Builder):** analog RuntimeVariableLoop ein `IterableAspectLoop` (oder Erweiterung): iteriert `iterable_values()` der aktiven iterable-Achse über die GELADENE Binary via `tier_apply_iterable_aspect`, ruft je Wert den Mess-Callback. `profile_to_tree`: die iterable-Achse (`k_ary.arity`) als EIGENE dynamische Dimension emittieren (eigener iterable-Dim-Typ, NICHT RC-DynamicDim). Pfad-Signatur/Label trägt „k_ary.arity=K".
- **C5 — Weg-A aktivieren:** Marker `axis_03a_store_traversable=true` (`axis_03a_search_algo_k_ary.hpp`) + registry-static_assert auf POSITIV flippen (`StoreTraversableSearchAlgo<KArySearchAlgo>`, `axis_03a_search_algo_registry.hpp:90-91`) + stale Kommentare aktualisieren (registry:86-91, `abi_adapter` tier_search_routes_through_store ~:1516-1518). container_traversal_t routet k_ary → KAryTraversal.
- **C6 — Tests:** `test_conformance_gate`: KAryComposedTier je K∈{2,4,8,16} durch `run_conformance_gate` (ALLE bestehen = Korrektheit für jedes K). `test_v41_scan_range_organ`: KAry-Scan je K. NEU: K-Distinktheits-Test (verschiedene K → andere Separator-/Vergleichs-Pfade, aber IDENTISCHE Ergebnisse = Mess-Echtheit statt Phantom).

**#221-Beziehung:** PARALLELER Mechanismus, NICHT vereinheitlicht — K reitet den iterable-Kanal (C3/C4), RC reitet den RC-Kanal (`tier_apply_resource_control`). Beide = „dynamische Subachse → Container/Organ-Konsum"; C ist die iterable-Hälfte, #221 die RC-Hälfte (getrennte Kanäle, gemeinsames Muster).

**Verifikation:** C1/C2/C6-Konformität sind via `test_conformance_gate` (contract:conformance, Linux-g++, reine lib-Header) ECHT CI-verifizierbar. C3/C4/C5 (ABI-Schnittstelle + Loop + abi_adapter + profile_to_tree) teils harness-gated (#162) = der bekannte Harness-CI-Gap (#230). #210-CI-Status vor „grün" literal prüfen.
