...
|---|---|---|---|
| **4 XML-Definition** | `comdare_thesis_profile`-Schema + `xml_config_parser` + Ist-Profile vs. Diplomarbeit-Methodik; verlustfreie Weitergabe an Ebene 3 | 🟡 teils-OK + 1 Befund | B4-1: genutzte dyn-Dims (thread_count/hw_prefetcher/repetition) verlustfrei; 4 POD-Felder gar nicht XML-definierbar |
| **3 B+-Baum / Gattung** | `profile_to_tree` + `experiment_tree`; StaticAxisNode/DynamicVariableNode-Verwebung; **Abstract Factory PRO GATTUNG** | ⏳ offen | |
| **2 StaticAxisNode → Tier-Binaries** | CEB-Gen + Iterator + Achsen-Organe konsumieren ihre POD-Feld-Parameter (NICHT tot) | ⏳ offen | |
| **1 DynamicVariableNode → RC** | `RuntimeVariableLoop` + `tier_apply_resource_control` → Organ-Verdrahtung (Ebene-1 echt aktivieren) | ⏳ offen | |
