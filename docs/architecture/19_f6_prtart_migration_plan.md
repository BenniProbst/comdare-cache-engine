...
| `identity/prt_art_identity.hpp` | PermutationFlags-Bit-Kombination zur Identifikation der PRT-ART-Binary (11 Achsen-Banks) | `cache_engine/anatomy/identity.hpp` (o.ä.) als Golden-Standard-Permutation für die SearchAlgorithm-Tier-Unterklasse (korr. 2026-06-03, s. Doc 30 §8.0: „Gattung" hier ersetzt — die 17/19-Achsen-Komposition/Permutation ist die Tier-Unterklassen-Ebene UNTER dem SearchAlgorithm-Interface, NICHT die Gattung; Gattung = das Außen-Interface SearchAlgorithm/Container/Graph) | Echtes Basis-Konzept: ausgereiftes Permutation-Flags-System; `prt_art_permutation_flags()` + `prt_art_identifier()` als generisches Pattern verfügbar machen. R5/R7-Goldstandard-Anatomien als Vorlage. |
