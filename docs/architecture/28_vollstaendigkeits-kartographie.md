# 28 — Vollständigkeits-Kartographie: SOLL-Zustand für ALLE 22 Achsen + 5 Lebewesen-Unterklassen + 4 Brücken + Skalierung (2026-06-02)
...
> (2) **LEBEWESEN-UNTERKLASSE = liegt UNTER dem Gattungs-Interface und verwendet einen FESTEN Achsen-Satz.** SearchAlgorithm (std::map-ähnlich, 17/19-Achsen) ist die einzige bisher gebaute Lebewesen-Unterklasse (unter dem SearchAlgorithm-**Interface**); Set/Sequence/Adapter/View sind Lebewesen-Unterklassen UNTER dem **Container-Interface**.
> (3) **ACHSEN = Organe der Lebewesen-Unterklasse**, keine ist optional (Durchreich-Algorithmus statt Weglassen).
> Daher: „5 Gattungen" = **5 Lebewesen-Unterklassen** (verteilt auf 2 Interfaces SearchAlgorithm + Container); die feste Slot-/Achsen-Zahl pro Zeile lebt auf der Lebewesen-Unterklassen-Ebene. Wo „Gattung" das **Außen-Interface/Prüf-Dock** meint (z.B. „Prüf-Dock je Gattung", „Interface der Gattung", „ABI-Interface der API der Gattung"), ist es KORREKT und bleibt. Die Gattungs-Invariante (feste Slot-Zahl, `AdHocComposition<17>` als ABI-Identität) bleibt sachlich richtig — sie ist eine **Lebewesen-Unterklassen**-Invariante.
...
> (FIFO/LIFO/Priority)" für die Adapter-Lebewesen-Unterklasse war noch teilweise GERATEN und ist hiermit VERWORFEN. Autoritativ
> (Doku 14 **§28 Invertebrate** + **§26.4**, in Doc 30 §8.1 umgesetzt+verifiziert): die Adapter-Lebewesen-Unterklasse hat
...
- **Autoritative Gesamtzahl (Doc 27 §0/§0.1):** **15 Topics / 22 Achsen** (frühere „18" verworfen). „17" = NUR die SearchAlgorithm-Komposition (`AdHocComposition<17>`/`ObserverAggregate<17>`, Invariante der SearchAlgorithm-**Lebewesen-Unterklasse** — korr. 2026-06-03, s. Doc 30 §8.0: feste Achsen-Zahl lebt auf der Lebewesen-Unterklassen-Ebene, NICHT auf der Gattungs-/Interface-Ebene) — NIEMALS als Gesamtzahl missverstehen (kein Wegschrumpfen).
- **Achsen-Set-Constraint (vormals „Gattungs-Constraint", Doku 14 §32; korr. 2026-06-03, s. Doc 30 §8.0):** Joins zwischen disjunkten Lebewesen-Unterklassen sind type-system-mathematisch unmöglich (disjunkte Achsen-Sets, inkompatible Concepts). Jede **Lebewesen-Unterklasse** = eigene Komposition/Anatomie/ABI-Adapter/Observer + eigene `PermutationEngine`-Subklasse; das **Prüf-Dock + ABI-Interface der API** liegt auf der **Gattungs-(Interface-)Ebene** (SearchAlgorithm/Container/Graph), Doku 14 §29.2/§32.6.
...
**5 Achsen außerhalb der 17 composition_factory-Slots (T0..T16) — alle weiterhin Achsen der SearchAlgorithm-Lebewesen-Unterklasse:** 3 Build-Achsen (page_type/09b/12 = Build-/Definition-Variante) + 2 reguläre mandatorische SearchAlgorithm-Achsen (q1/q2_queuing). Zusammen mit den 17 Slot-Achsen = **19 SearchAlgorithm-Achsen** (+ 3 Build-Achsen = 22). Weder eigene Gattung (Interface) noch eigene Lebewesen-Unterklasse (korr. 2026-06-03, s. Doc 30 §8.0).
...
| 21 | axis_q1_queuing | queuing | EnabledStrategies | BufferStrategy: put/get/emplace/peek_front/peek_back/size/is_empty/clear | **ObservableAxis** (put/get/peak_occupancy) | **reguläre mandatorische Achse der SearchAlgorithm-Lebewesen-Unterklasse** (queuing-Organ; nicht-pufferndes Lebewesen wählt `NoBuffer`, durchreichend); Achse — weder Gattung noch Lebewesen-Unterklasse |
| 22 | axis_q2_queuing | queuing | EnabledPolicies | FlushPolicy: should_flush()→{NoFlush/Partial/Full}, on_flush_complete() | **ObservableAxis** | **reguläre mandatorische Achse der SearchAlgorithm-Lebewesen-Unterklasse** (queuing-Organ; nicht-flushendes Lebewesen wählt `NoFlush`, durchreichend); Achse — weder Gattung noch Lebewesen-Unterklasse (#75) |
...
## §2 Die 5 Lebewesen-Unterklassen (+ Viren) — SOLL je Lebewesen-Unterklasse

> **(korr. 2026-06-03, s. Doc 30 §8.0):** Diese Tabelle listet **5 Lebewesen-Unterklassen** (NICHT „5 Gattungen"), verteilt auf **2 Gattungen/Interfaces**: SearchAlgorithm-Zeile = einzige Lebewesen-Unterklasse unter dem **SearchAlgorithm-Interface**; Adapter/Set/Sequence/View = Lebewesen-Unterklassen unter dem **Container-Interface**. Viren liegen außerhalb (Graph-Interface bzw. Nicht-Lebewesen). Die Spalten „Slots SOLL / Eigene Achsen / Anatomie / ABI / Observer" beschreiben die jeweilige **Lebewesen-Unterklasse**; nur das **Dock** (+ ABI-Interface der API) gehört zur Gattungs-/Interface-Ebene. „AnatomyGenus" im Code ist die historische Bezeichnung dieser Lebewesen-Unterklassen-Aufzählung.

Quelle: `anatomy_base.hpp:44-50` (AnatomyGenus — historischer Name der Lebewesen-Unterklassen-Enum), Doku 14 §26-§40, Doc 24 §8.8, abhaengigkeitskette.

| Lebewesen-Unterklasse | Gattung/Interface | Lebewesen (Doku 14 §27) | K/V-Modell | Slots SOLL | Eigene Achsen | Anatomie / ABI / Observer / Dock | IST |
|---------|---------|--------------------|-----------|-----------|---------------|----------------------------------|-----|
...
**queuing-Einordnung (präzise, korrigiert; verfeinert korr. 2026-06-03, s. Doc 30 §8.0):** q1/q2 sind reguläre **mandatorische SearchAlgorithm-Achsen** (Organe), nicht Organe einer eigenen Lebewesen-Unterklasse/Gattung — sie liegen lediglich außerhalb der 17 composition_factory-Slots (T0..T16), gehören aber zur **SearchAlgorithm-Lebewesen-Unterklasse** (unter dem SearchAlgorithm-**Interface**) und werden in JEDEM Lebewesen-Binary uniform getrieben (nicht-pufferndes/-flushendes Lebewesen wählt `NoBuffer`/`NoFlush`, durchreichend — keine Achse ist optional). Die **Doku-14-§7-Sicht („q1/q2 = stabile Organe") war KORREKT** (hiermit rehabilitiert); **nicht Doku 14, sondern Doc 27 §0.1 ist die revidierte Fehl-„Präzisierung"**, die queuing fälschlich zur Container-Gattung erhob. Die **Adapter-Lebewesen-Unterklasse** (`std::queue/stack/priority_queue`, unter dem **Container-Interface**) besteht aus den **13 §28-Achsen** (9 delegiert + 3 aktiv + `inner_container`; **KEINE „ordering"-Achse** — FIFO/LIFO = API-Nutzung §26.4, korr. 2026-06-03 s. Doc 30 §8.1) und nutzt queuing NICHT; §32 (Disjunkte-Achsen-Sets, vormals „Cross-Genus") verbietet den Join disjunkter Lebewesen-Unterklassen, macht aber keine Achse zur Gattung/Lebewesen-Unterklasse. Grep-verifiziert: **kein** Set/Sequence/View-Anatomy/Composition/Dock im Code (0 Treffer) → die 3 weiteren Container-Lebewesen-Unterklassen (Set/Sequence/View) + produktiver Viren-Pfad (Graph-Interface) sind nur Doku-SOLL.
...
3. Jedes statische Blatt → reale `AdHocComposition`, als Lebewesen-Binary baubar (BR-4).
...
- **#75 (Container-Gattung/Interface — Adapter-Lebewesen-Unterklasse; korr. 2026-06-03, s. Doc 30 §8.0/§8.1):** echte Adapter-Komposition (`AdapterComposition<T0..T11, Inner>` der **Adapter-Lebewesen-Unterklasse** aus den **13 §28-Achsen** = 9 delegiert + 3 aktiv + `inner_container` — KEINE „ordering"-Achse, FIFO/LIFO = API-Nutzung §26.4; NICHT queuing) + Adapter-DLL-Pfad (`AdapterAbiAdapter` + COMDARE_DEFINE-Analogon + Loader + observe über DLL-Grenze; Dock + ABI-Interface = Container-Gattungs-/Interface-Ebene). (Typnamen `Adapter*`/`adapter_*` seit Rename 2026-06-03, vorher `Container*`/`container_*`; in Doc 30 §8.1 umgesetzt+verifiziert. queuing q1/q2 = SearchAlgorithm-Achsen, gehören nicht hierher → AdHocComposition 17→19.)
- **#76 (weitere Container-Lebewesen-Unterklassen; korr. 2026-06-03, s. Doc 30 §8.0):** Set (14 Achsen) / Sequence (9+axis_growth) / View (7+axis_extent/layout/accessor) — je eine **Lebewesen-Unterklasse** unter dem Container-Interface — je Anatomie+Composition+ABI+Observer+PermutationEngine (+ gemeinsames Container-Dock auf Interface-Ebene) + GenusBindingTraits-Spezialisierung; (Viren-Pfad IVirusExecutionEngine produktiv, Graph-Interface).
...
**Gate-3/4 gelten heute nur unter Pilot- bzw. Klassifikations-Vorbehalt.** „ABSOLUTE VOLLSTÄNDIGKEIT gegen ALLE 22 Achsen + alle 5 Lebewesen-Unterklassen (verteilt auf die Gattungen/Interfaces SearchAlgorithm + Container, plus Graph für Viren; korr. 2026-06-03, s. Doc 30 §8.0)" = Phase D, bestätigt durch Phase E (finaler adversarialer Audit).
