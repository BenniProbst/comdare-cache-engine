...
[[nodiscard]] std::optional<tool::CompositionDescriptor> parse_external_composition(std::string_view spec) {
...
int main(int argc, char** argv) {
    ...
    if (!lib_type_ptr) {
        std::cerr << "ERROR: --library-type must be SHARED or STATIC (got: " << library_type << ").\n";
        return 1;
    }
    ...
    std::cout << "comdare-anatomy-codegen-tool: wrote " << selected.size() << " composition entries to " << output_path
