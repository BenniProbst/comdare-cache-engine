...
[[nodiscard]] PermutationResult run_single_experiment(
    std::string const& permutation_id,
    std::uint64_t      fingerprint,
    Strategy&          strategy,
    std::span<workload_generator::Operation const> ops,
    std::uint32_t      value_size_bytes)
{
        ...
        switch (op.op) {
            ...
            case workload_generator::OperationKind::ReadModifyWrite: {
                ...
                if (p) {
                    ...
                    runner.record_event(handle,
                        benchmark_suite::EventKind::Allocation, value_size_bytes);
                }
                break;
            }
            case workload_generator::OperationKind::Erase:
                if (!allocations.empty()) {
                    ...
                    runner.record_event(handle,
                        benchmark_suite::EventKind::Deallocation, value_size_bytes);
                }
                break;
    ...
    auto const ns_taken = std::chrono::duration_cast<std::chrono::nanoseconds>(
        end_cycles - start_cycles).count();
